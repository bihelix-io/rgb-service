...
impl Router for FixedRouter {
    fn find_route(
        ...
        self.route
            ...
            .unwrap()
            .take()
            .ok_or("Can't use this router to return multiple routes")
    }
...
impl Path {
    ...
    pub fn has_trampoline_hops(&self) -> bool {
        self.blinded_tail
            .as_ref()
            .map_or(false, |bt| !bt.trampoline_hops.is_empty())
    }
}
...
impl Readable for RouteParameters {
    fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
        ...
        if let Payee::Clear {
            ref mut final_cltv_expiry_delta,
            ..
...
impl RouteParametersConfig {
    ...
    pub fn with_max_total_routing_fee_msat(self, fee_msat: u64) -> Self {
        Self {
            max_total_routing_fee_msat: Some(fee_msat),
            ..self
        }
    }
...
impl<'a> PaymentPath<'a> {
    ...
    fn get_cost_msat(&self) -> u64 {
        self.get_total_fee_paid_msat()
            .saturating_add(self.get_path_penalty_msat())
    }
