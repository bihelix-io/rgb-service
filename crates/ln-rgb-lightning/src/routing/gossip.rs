    ...
    pub(super) fn forward_gossip_msg(&self, mut ev: MessageSendEvent) {
        match &mut ev {
            MessageSendEvent::BroadcastChannelAnnouncement {
                ...
                if update_msg
                    .as_ref()
                    .map(|msg| msg.contents.excess_data.len())
                    .unwrap_or(0)
...
fn message_sha256d_hash<M: Writeable>(msg: &M) -> Sha256dHash {
    ...
    msg.write(&mut engine)
        .expect("In-memory structs should not fail to serialize");
    ...
    fn get_next_channel_announcement(
        ...
        for (_, ref chan) in channels.range(starting_point..) {
            if chan.announcement_message.is_some() {
                ...
                return Some((
                    chan_announcement,
                    one_to_two_announcement,
                    two_to_one_announcement,
                ));
            } else {
...
impl<'a> DirectedChannelInfo<'a> {
    ...
    pub fn effective_capacity(&self) -> EffectiveCapacity {
        ...
        let capacity_msat = self
            .channel
            .capacity_sats
            .map(|capacity_sats| capacity_sats * 1000);
    ...
    L::Target: Logger,
{
    fn read<R: io::Read>(reader: &mut R, logger: L) -> Result<NetworkGraph<L>, DecodeError> {
        ...
        for (_, chan) in channels.unordered_iter_mut() {
            chan.node_one_counter = nodes
                .get(&chan.node_one)
                .ok_or(DecodeError::InvalidValue)?
                .node_counter;
            chan.node_two_counter = nodes
                .get(&chan.node_two)
                .ok_or(DecodeError::InvalidValue)?
                .node_counter;
        }
    ...
    L::Target: Logger,
{
    fn eq(&self, other: &Self) -> bool {
        ...
        let a = if ord {
            (&self.channels, &self.nodes)
        } else {
            (&other.channels, &other.nodes)
        };
        let b = if ord {
            (&other.channels, &other.nodes)
        } else {
    ...
    L::Target: Logger,
{
        ...
        #[cfg(any(test, fuzzing))]
        {
            ...
            for (_, chan) in channels.unordered_iter() {
                assert_eq!(
                    ...
                    nodes.get(&chan.node_one).unwrap().node_counter
                );
                assert_eq!(
                    chan.node_two_counter,
                    nodes.get(&chan.node_two).unwrap().node_counter
                );
            }
        }
    }
    ...
    pub fn update_node_from_unsigned_announcement(
        &self,
        msg: &msgs::UnsignedNodeAnnouncement,
    ) -> Result<(), LightningError> {
    ...
    fn update_node_from_announcement_intern(
        &self,
        msg: &msgs::UnsignedNodeAnnouncement,
        full_msg: Option<&msgs::NodeAnnouncement>,
    ) -> Result<(), LightningError> {
        ...
        match nodes.get_mut(&msg.node_id) {
            None => {
                core::mem::drop(nodes);
                self.pending_checks
                    .check_hold_pending_node_announcement(msg, full_msg)?;
                Err(LightningError {
        ...
        U::Target: UtxoLookup,
    {
        ...
        let utxo_value =
            self.pending_checks
                .check_channel_announcement(utxo_lookup, msg, full_msg)?;
        ...
        log_gossip!(
            ...
            if !msg.excess_data.is_empty() {
                " with excess uninterpreted data!"
            } else {
                ""
            }
        );
    ...
    fn channel_failed_permanent_with_time(
        ...
        if let Some(chan) = channels.remove(&short_channel_id) {
            ...
            self.removed_channels
                .lock()
                .unwrap()
                .insert(short_channel_id, current_time_unix);
    ...
    pub fn remove_stale_channels_and_tracking_with_time(&self, current_time_unix: u64) {
        ...
        if !scids_to_remove.is_empty() {
            ...
            self.removed_node_counters
                .lock()
                .unwrap()
                .reserve(channels_removed_bulk.len());
        ...
        self.removed_channels
            .lock()
            .unwrap()
            .retain(|_, time| should_keep_tracking(time));
        self.removed_nodes
            .lock()
            .unwrap()
            .retain(|_, time| should_keep_tracking(time));
    ...
    pub fn update_channel_unsigned(
        &self,
        msg: &msgs::UnsignedChannelUpdate,
    ) -> Result<(), LightningError> {
    ...
    fn update_channel_internal(
        &self,
        msg: &msgs::UnsignedChannelUpdate,
        full_msg: Option<&msgs::ChannelUpdate>,
        sig: Option<&secp256k1::ecdsa::Signature>,
        only_verify: bool,
    ) -> Result<(), LightningError> {
        ...
        let mut node_pubkey = None;
        {
            ...
            match channels.get(&msg.short_channel_id) {
                None => {
                    core::mem::drop(channels);
                    self.pending_checks
                        .check_hold_pending_channel_update(msg, full_msg)?;
                    return Err(LightningError {
    ...
    fn remove_channel_in_nodes_callback<RM: FnMut(IndexedMapOccupiedEntry<NodeId, NodeInfo>)>(
        &self,
        nodes: &mut IndexedMap<NodeId, NodeInfo>,
        chan: &ChannelInfo,
        short_channel_id: u64,
        mut remove_node: RM,
    ) {
            ...
            ($node_id: expr) => {
                if let IndexedMapEntry::Occupied(mut entry) = nodes.entry($node_id) {
                    entry
                        .get_mut()
                        .channels
                        .retain(|chan_id| short_channel_id != *chan_id);
                    if entry.get().channels.is_empty() {
                        self.removed_node_counters
                            .lock()
                            .unwrap()
                            .push(entry.get().node_counter);
                        remove_node(entry);
                    }
                } else {
                    panic!(
...
pub(crate) mod tests {
    ...
    fn handling_node_announcements() {
        ...
        let announcement_with_data = get_signed_node_announcement(
            |unsigned_announcement| {
                ...
                unsigned_announcement
                    .excess_data
                    .resize(MAX_EXCESS_BYTES_FOR_RELAY + 1, 0);
            },
            node_1_privkey,
            &secp_ctx,
        );
    ...
    fn handling_channel_announcements() {
        ...
        *chain_source.utxo_ret.lock().unwrap() = UtxoResult::Sync(Ok(TxOut {
        ...
        #[cfg(feature = "std")]
        {
            ...
            gossip_sync
                .network_graph()
                .node_failed_permanent(&node_1_pubkey);
            ...
            .handle_channel_announcement(Some(node_1_pubkey), &incorrect_chain_announcement)
        {
            ...
            Err(e) => {
                assert_eq!(
                    e.err,
                    "Channel announcement chain hash does not match genesis hash"
                )
            }
        };
    }
    ...
    fn handling_channel_update() {
        ...
        network_graph
            .verify_channel_update(&valid_channel_update)
            .unwrap();
...

        {
            match network_graph.read_only().channels().get(&short_channel_id) {
                ...
                Some(channel_info) => {
                    assert_eq!(
                        channel_info.one_to_two.as_ref().unwrap().cltv_expiry_delta,
                        144
                    );
        ...
        match gossip_sync.handle_channel_update(Some(node_1_pubkey), &valid_channel_update) {
            ...
            Err(e) => assert_eq!(
                e.err,
                "htlc_maximum_msat is larger than maximum possible msats"
            ),
        };
        ...
        match gossip_sync.handle_channel_update(Some(node_1_pubkey), &incorrect_chain_update) {
            ...
            Err(e) => assert_eq!(
                e.err,
                "Channel update chain hash does not match genesis hash"
            ),
        };
    }
    ...
    fn test_channel_timeouts() {
        ...
        gossip_sync
            .handle_channel_update(Some(node_1_pubkey), &valid_channel_update_2)
            .unwrap();
        assert!(network_graph
            .read_only()
            .channels()
        ...
        #[cfg(feature = "std")]
        {
            ...
            network_graph.remove_stale_channels_and_tracking_with_time(
                announcement_time
                    + 1
                    + STALE_CHANNEL_UPDATE_AGE_LIMIT_SECS
    ...
    fn calling_sync_routing_table() {
        ...
        // It should ignore if gossip_queries feature is not enabled
        {
            ...
            gossip_sync
                .peer_connected(node_id_1, &init_msg, true)
                .unwrap();
    ...
    fn displays_node_alias() {
        ...
        assert_eq!(
            format_str_alias("I\u{1F496}LDK!\0\u{26A1}"),
            "I\u{1F496}LDK!"
        );
        assert_eq!(
            format_str_alias("I\u{1F496}LDK!\t\u{26A1}"),
        ...
        assert_eq!(
            format_bytes_alias(b"\xFFI <heart> LDK!"),
            "\u{FFFD}I <heart> LDK!"
        );
        assert_eq!(
            format_bytes_alias(b"\xFFI <heart>\0LDK!"),
            "\u{FFFD}I <heart>"
        );
        assert_eq!(
            format_bytes_alias(b"\xFFI <heart>\tLDK!"),
            "\u{FFFD}I <heart>\u{FFFD}LDK!"
        );
    }
    ...
    fn channel_info_is_readable() {
        ...
        assert!(chan_update_info
            .write(&mut encoded_chan_update_info)
            .is_ok());
    ...
    fn node_info_is_readable() {
        ...
        assert!(valid_node_ann_info
            .write(&mut encoded_valid_node_ann_info)
            .is_ok());
    ...
    fn is_tor_only_node() {
        ...
        let announcement =
            get_signed_channel_announcement(|_| {}, node_1_privkey, node_2_privkey, &secp_ctx);
        gossip_sync
            .handle_channel_announcement(Some(node_1_pubkey), &announcement)
            .unwrap();
        ...
        gossip_sync
            .handle_node_announcement(Some(node_1_pubkey), &announcement)
            .unwrap();
        assert!(!network_graph
            .read_only()
            .node(&node_1_id)
...
pub mod benches {
    ...
    pub fn write_network_graph(bench: &mut Criterion) {
        ...
        bench.bench_function("write_network_graph", |b| {
            b.iter(|| black_box(&net_graph).encode())
        });
    }
}
