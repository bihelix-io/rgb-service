...
pub(crate) fn write_chanmon_internal<Signer: EcdsaChannelSigner, W: Writer>(
    channel_monitor: &ChannelMonitorImpl<Signer>,
    _is_stub: bool,
    writer: &mut W,
) -> Result<(), Error> {
    ...
    let redeem_script = channel_monitor
        .funding
        .channel_parameters
    ...
    for (ref txid, ref htlc_infos) in channel_monitor
        .funding
        .counterparty_claimable_outpoints
        .iter()
    {
    ...
    writer.write_all(&byte_utils::be48_to_array(
        channel_monitor.current_holder_commitment_number,
    ))?;
    ...
    for ref entry in channel_monitor
...
impl WithChannelMonitor {
    ...
    pub(crate) fn from_impl<'a, L: Deref, S: EcdsaChannelSigner>(
        logger: &'a L,
        monitor_impl: &ChannelMonitorImpl<S>,
        payment_hash: Option<PaymentHash>,
    ) -> WithContext<'a, L>
    where
        L::Target: Logger,
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
    ...
    pub(crate) fn get_cur_counterparty_commitment_number(&self) -> u64 {
        self.inner
            .lock()
            .unwrap()
            .get_cur_counterparty_commitment_number()
    }
    ...
    pub(crate) fn get_cur_holder_commitment_number(&self) -> u64 {
        self.inner
            .lock()
            .unwrap()
            .get_cur_holder_commitment_number()
    }
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
    ...
    pub(crate) fn get_onchain_failed_outbound_htlcs(&self) -> HashMap<HTLCSource, PaymentHash> {
        ...
        let confirmed_txid = us.funding_spend_confirmed.or_else(|| {
            us.onchain_events_awaiting_threshold_conf
                .iter()
                .find_map(|event| {
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
    ...
    fn provide_initial_counterparty_commitment_tx(
		...
		// RGB commitment transactions can contain extra outputs which cannot be reconstructed from
		// the compact downgrade fields above. Store the full transaction before any debug-time
		// reconstruction so monitors for RGB channels keep the exact txid.
    ...
    fn update_counterparty_commitment_data(
        ...
        let htlcs_for_commitment = |commitment: &CommitmentTransaction| {
            ...
            let dust_htlcs = htlc_data.dust_htlcs.iter().map(|(htlc, source)| {
                (
                    htlc.clone(),
                    source.as_ref().map(|source| Box::new(source.clone())),
                )
            });
    ...
    fn update_holder_commitment_data(
        ...
        self.verify_matching_commitment_transactions(
            commitment_txs
                .iter()
                .map(|holder_commitment_tx| holder_commitment_tx.deref()),
        )?;

        let current_funding_commitment_tx = commitment_txs.first().unwrap();
        self.current_holder_commitment_number = current_funding_commitment_tx.commitment_number();
        self.onchain_tx_handler
            .provide_latest_holder_tx(current_funding_commitment_tx.clone());
        ...
        for (claimed_htlc_id, claimed_preimage) in claimed_htlcs {
            ...
            self.counterparty_fulfilled_htlcs
                .insert(*claimed_htlc_id, *claimed_preimage);
        }
    ...
    fn promote_funding(&mut self, new_funding_txid: Txid) -> Result<(), ()> {
        ...
        self.onchain_tx_handler
            .update_after_renegotiated_funding_locked(
			...
			if let Some(per_commitment_claimable_data) = per_commitment_option {
				for (htlc, _) in per_commitment_claimable_data {
					if let Some(transaction_output_index) = htlc.transaction_output_index {
						...
						let rgb_claim = htlc.amount_rgb.map(|(contract_id, amount_rgb)| crate::rgb::RgbClaimableOutpoint {
							outpoint: BitcoinOutPoint { txid: commitment_txid, vout: transaction_output_index },
							contract_id,
							amount_rgb,
							purpose: crate::rgb::RgbClaimPurpose::Htlc,
						});
						...
						).with_rgb_claimable_outpoint(rgb_claim);
						claimable_outpoints.push(justice_package);
					}
				}
			}
            ...
            .filter_map(|(htlc, _)| {
                if let Some(transaction_output_index) = htlc.transaction_output_index {
                    if htlc.offered && htlc.payment_hash == matching_payment_hash {
                        ...
                        let rgb_claim = htlc.amount_rgb.map(|(contract_id, amount_rgb)| {
                            crate::rgb::RgbClaimableOutpoint {
                                outpoint: BitcoinOutPoint {
                                    txid: commitment_txid,
        ...
        for (idx, outp) in tx.output.iter().enumerate() {
            if outp.script_pubkey == revokeable_p2wsh {
                to_counterparty_output_info = Some((
                    idx.try_into().expect("Can't have > 2^32 outputs"),
                    outp.value,
                ));
            }
        }
        ...
        for &(ref htlc, _) in per_commitment_claimable_data.iter() {
            if let Some(transaction_output_index) = htlc.transaction_output_index {
                ...
                if preimage.is_some() || !htlc.offered {
                    ...
                    .with_rgb_claimable_outpoint(htlc.amount_rgb.map(
                        |(contract_id, amount_rgb)| crate::rgb::RgbClaimableOutpoint {
                            outpoint: BitcoinOutPoint {
                                txid: commitment_txid,
                                vout: transaction_output_index,
                            },
    ...
    L::Target: Logger,
{
    fn filtered_block_connected(&self, header: &Header, txdata: &TransactionData, height: u32) {
        self.0
            .block_connected(header, txdata, height, &*self.1, &*self.2, &self.3);
    }

    fn blocks_disconnected(&self, fork_point: BestBlock) {
        self.0
            .blocks_disconnected(fork_point, &*self.1, &*self.2, &self.3);
    }
}
    ...
    L::Target: Logger,
{
    fn transactions_confirmed(&self, header: &Header, txdata: &TransactionData, height: u32) {
        self.0
            .transactions_confirmed(header, txdata, height, &*self.1, &*self.2, &self.3);
    }

    fn transaction_unconfirmed(&self, txid: &Txid) {
        self.0
            .transaction_unconfirmed(txid, &*self.1, &*self.2, &self.3);
    }

    fn best_block_updated(&self, header: &Header, height: u32) {
        self.0
            .best_block_updated(header, height, &*self.1, &*self.2, &self.3);
    }
			...
			() => {
				{
					...
					HTLCOutputInCommitment {
						offered, amount_msat, amount_rgb: None, cltv_expiry, payment_hash,
						transaction_output_index
					}
				}
			}
