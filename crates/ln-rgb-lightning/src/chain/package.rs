...
pub struct PackageTemplate {
    ...
    // Explicit RGB assignments for the package inputs. LN must provide these from channel state;
    // package finalization must never infer RGB amounts from Bitcoin outputs.
    rgb_claimable_outpoints: Vec<crate::rgb::RgbClaimableOutpoint>,
...
impl PackageTemplate {
    ...
    pub(crate) fn outpoints_and_creation_heights(
        ...
        self.inputs
            .iter()
            .map(|(o, p)| (o, p.input_confirmation_height()))
    }
    ...
    pub(crate) fn split_package(&mut self, split_outp: &BitcoinOutPoint) -> Option<PackageTemplate> {
		match self.malleability {
			PackageMalleability::Malleable(cluster) => {
				...
				self.inputs.retain(|outp| {
					if *split_outp == outp.0 {
						split_package = Some(PackageTemplate {
							...
							rgb_claimable_outpoints: self.rgb_claimable_outpoints.iter()
								.filter(|claim| claim.outpoint == outp.0)
								.cloned()
								.collect(),
				...
				self.rgb_claimable_outpoints.retain(|claim| *split_outp != claim.outpoint);
    ...
    pub(crate) fn merge_package(
        &mut self,
        mut merge_from: PackageTemplate,
        cur_height: u32,
        ...
        self.rgb_claimable_outpoints
            .append(&mut merge_from.rgb_claimable_outpoints);
    ...
    pub(crate) fn maybe_finalize_malleable_package<L: Logger, Signer: EcdsaChannelSigner>(
		...
		let mut rgb_claims = self.rgb_claimable_outpoints.clone();
		if rgb_claims.is_empty() {
			rgb_claims = onchain_handler.current_holder_commitment_tx().rgb_claimable_outpoints().iter()
				.filter(|claim| self.inputs.iter().any(|(outpoint, _)| *outpoint == claim.outpoint))
				.cloned()
				.collect();
		}
		let rgb_claim_outpoint = if !rgb_claims.is_empty() {
			assert_eq!(rgb_claims.len(), 1, "RGB daemon LN on-chain claim compose expects a single RGB source outpoint");
			Some(rgb_claims[0].outpoint)
		} else {
			if onchain_handler.funding_rgb_transfer.is_some() {
				self.inputs.iter()
					.find(|(_, outp)| matches!(outp, PackageSolvingData::RevokedOutput(_)))
					.map(|(outpoint, _)| *outpoint)
			} else {
				None
			}
		};
		if let Some(rgb_claim_outpoint) = rgb_claim_outpoint {
			assert_eq!(bumped_tx.output.len(), 1, "RGB LN sweep expects exactly one output");
			let funding_ref = onchain_handler.funding_rgb_transfer.clone()
				.expect("RGB on-chain claim requires the funding RGB daemon reference");
			bumped_tx = crate::rgb::get_rgb_ln_tx_composer()
				.compose_onchain_claim(crate::rgb::RgbLnOnchainClaimComposeRequest {
					channel_id: funding_ref.channel_id.expect("RGB on-chain claim requires a channel id"),
					commitment_txid: rgb_claim_outpoint.txid,
					vout: rgb_claim_outpoint.vout,
					unsigned_tx: bumped_tx,
				})
				.expect("RGB daemon LN on-chain claim compose failed");
		}
    ...
    pub(crate) fn build_package(
        ...
        PackageTemplate {
            inputs,
            rgb_claimable_outpoints: Vec::new(),
            malleability,
    ...
    pub(crate) fn with_rgb_claimable_outpoint(
        mut self,
        claim: Option<crate::rgb::RgbClaimableOutpoint>,
    ) -> Self {
        if let Some(claim) = claim {
            self.rgb_claimable_outpoints.push(claim);
        }
        self
    }
}
...
impl Writeable for PackageTemplate {
    fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
        (self.inputs.len() as u64).write(writer)?;
        ...
        write_tlv_fields!(writer, {
            ...
            (8, self.rgb_claimable_outpoints, optional_vec)
        });
        Ok(())
    }
}
...
impl Readable for PackageTemplate {
    ...
    fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		let mut rgb_claimable_outpoints = None;
		read_tlv_fields!(reader, {
			...
			(8, rgb_claimable_outpoints, optional_vec),
		});
		...
		() => {
			{
				...
				let htlc = HTLCOutputInCommitment { offered: false, amount_msat: 1_000_000, amount_rgb: None, cltv_expiry: 0, payment_hash: hash, transaction_output_index: None };
		...
		($amt: expr, $expiry: expr, $features: expr) => {
			{
				...
				let dumb_point = PublicKey::from_secret_key(&secp_ctx, &dumb_scalar);
				let hash = PaymentHash([1; 32]);
				let htlc = HTLCOutputInCommitment { offered: true, amount_msat: $amt, amount_rgb: None, cltv_expiry: $expiry, payment_hash: hash, transaction_output_index: None };
				let mut channel_parameters = ChannelTransactionParameters::test_dummy(0);
		...
		($amt: expr, $features: expr) => {
			{
				...
				let htlc = HTLCOutputInCommitment { offered: false, amount_msat: $amt, amount_rgb: None, cltv_expiry: 0, payment_hash: hash, transaction_output_index: None };
				let mut channel_parameters = ChannelTransactionParameters::test_dummy(0);
				channel_parameters.channel_type_features = $features;
				PackageSolvingData::CounterpartyOfferedHTLCOutput(
