...
impl<
        ...
        BI: Deref + MaybeSend + MaybeSync + 'static,
        FE: Deref + MaybeSend + MaybeSync + 'static,
    > Persist<<SP::Target as SignerProvider>::EcdsaSigner>
    for AsyncPersister<K, S, L, ES, SP, BI, FE>
where
    K::Target: KVStore + MaybeSync,
    L::Target: Logger,
    ES::Target: EntropySource + Sized,
    ...
    <SP::Target as SignerProvider>::EcdsaSigner: MaybeSend + 'static,
{
    fn persist_new_channel(
        ...
        self.persister
            .spawn_async_persist_new_channel(monitor_name, monitor, notifier);
    ...
    fn update_persisted_channel(
        ...
        let notifier = Arc::clone(&self.event_notifier);
        self.persister
            .spawn_async_update_channel(monitor_name, monitor_update, monitor, notifier);
        ChannelMonitorUpdateStatus::InProgress
    }

    fn archive_persisted_channel(&self, monitor_name: MonitorName) {
        self.persister
            .spawn_async_archive_persisted_channel(monitor_name);
    }
    ...
    ES::Target: EntropySource,
{
        ...
        if let Some(height) = best_height {
            ...
            if new_height > old_height {
                self.highest_chain_height
                    .store(new_height, Ordering::Release);
            }
        }
    }
        ...
        FN: Fn(&ChannelMonitor<ChannelSigner>, &TransactionData) -> Vec<TransactionOutputs>,
    {
        ...
        if has_pending_claims || get_partition_key(channel_id) % partition_factor == 0 {
            ...
            match self
                .persister
                .update_persisted_channel(monitor.persistence_key(), None, monitor)
            {
        ...
        hash_map_from_iter(
            self.monitors
                ...
                .map(|(channel_id, holder)| {
                    (
                        *channel_id,
                        holder.pending_monitor_updates.lock().unwrap().clone(),
                    )
                }),
        )
    }
    ...
    pub fn archive_fully_resolved_channel_monitors(&self) {
        ...
        for monitor_holder in self.monitors.read().unwrap().values() {
            ...
            let (is_fully_resolved, needs_persistence) = monitor_holder
                .monitor
                .check_and_update_full_resolution_status(&logger);
        ...
        if have_monitors_to_prune {
            ...
            monitors.retain(|channel_id, monitor_holder| {
                let logger = WithChannelMonitor::from(&self.logger, &monitor_holder.monitor, None);
                let (is_fully_resolved, _) = monitor_holder
                    .monitor
                    .check_and_update_full_resolution_status(&logger);
                if is_fully_resolved {
                    log_info!(
    ...
    fn send_peer_storage(&self, their_node_id: PublicKey) {
        ...
        log_debug!(
            self.logger,
            "Sending Peer Storage to {}",
            log_pubkey!(their_node_id)
        );
        let send_peer_storage_event = MessageSendEvent::SendPeerStorage {
            node_id: their_node_id,
            msg: PeerStorage {
                data: cipher.into_vec(),
            },
        };

        self.pending_send_only_events
            .lock()
            .unwrap()
            .push(send_peer_storage_event)
    }
    ...
    ES::Target: EntropySource,
{
    fn watch_channel(
        ...
        let persist_res = self
            .persister
            .persist_new_channel(monitor.persistence_key(), &monitor);
        match persist_res {
            ChannelMonitorUpdateStatus::InProgress => {
                log_info!(
                    logger,
    ...
    ES::Target: EntropySource,
{
        ...
        H::Target: EventHandler,
    {
        for monitor_state in self.monitors.read().unwrap().values() {
            match monitor_state
                .monitor
                .process_pending_events(&handler, &self.logger)
            {
                Ok(()) => {}
                Err(ReplayEvent()) => {
                    self.event_notifier.notify();
...
mod tests {
    ...
    fn test_async_ooo_offchain_updates() {
        ...
        let persistences = chanmon_cfgs[1]
            .persister
            .offchain_monitor_updates
            .lock()
            .unwrap()
        ...
        #[cfg(c_bindings)]
        let pending_chan_updates = &pending_updates
            .iter()
        ...
        let next_update = update_iter.next().unwrap().clone();
        node_b_mon
            .channel_monitor_updated(channel_id, next_update)
            .unwrap();
        ...
        nodes[1]
            .node
            .handle_commitment_signed_batch_test(node_a_id, &as_first_update);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[1]
            .node
            .handle_commitment_signed_batch_test(node_a_id, &as_second_update);
        check_added_monitors!(nodes[1], 1);
    ...
    fn update_during_chainsync_poisons_channel() {
        ...
        *nodes[0].connect_style.borrow_mut() = ConnectStyle::FullBlockViaListen;
        ...
        chanmon_cfgs[0]
            .persister
            .set_update_ret(ChannelMonitorUpdateStatus::UnrecoverableError);
