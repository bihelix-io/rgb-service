...
impl BlindedPaymentPath {
    ...
    pub fn public_introduction_node_id<'a>(
        &self,
        network_graph: &'a ReadOnlyNetworkGraph,
    ) -> Option<&'a NodeId> {
        ...
        NS::Target: NodeSigner,
    {
        ...
        let encrypted_control_tlvs = &self
            .inner_path
            .blinded_hops
            .get(0)
            .ok_or(())?
...
pub(super) fn blinded_hops<T: secp256k1::Signing + secp256k1::Verification>(
    ...
    let tlvs = intermediate_nodes
        ...
        .chain(core::iter::once(BlindedPaymentTlvsRef::Receive(
            &payee_tlvs,
        )));

    let path = pks.zip(tlvs.map(|tlv| BlindedPathWithPadding {
        tlvs: tlv,
...
pub(crate) fn amt_to_forward_msat(
    ...
    let post_base_fee_inbound_amt = if let Some(amt) = inbound_amt.checked_sub(base) {
        amt
    } else {
        return None;
    };
...
pub(super) fn compute_payinfo(
    intermediate_nodes: &[PaymentForwardNode],
    payee_tlvs: &UnauthenticatedReceiveTlvs,
    ...
    for node in intermediate_nodes.iter() {
        ...
        if node
            .tlvs
            .features
            .requires_unknown_bits_from(&BlindedHopFeatures::empty())
        {
            return Err(());
        }
        ...
        htlc_minimum_msat = amt_to_forward_msat(
            core::cmp::max(
                node.tlvs.payment_constraints.htlc_minimum_msat,
                htlc_minimum_msat,
            ),
    ...
    htlc_minimum_msat = core::cmp::max(
        payee_tlvs.payment_constraints.htlc_minimum_msat,
        htlc_minimum_msat,
    );
...
impl Readable for PaymentConstraints {
    fn read<R: io::Read>(r: &mut R) -> Result<Self, DecodeError> {
        ...
        Ok(Self {
            max_cltv_expiry,
            htlc_minimum_msat: htlc_minimum_msat.0,
        })
    }
}
