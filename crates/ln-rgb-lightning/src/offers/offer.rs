    ...
    for OfferBuilder<'a, DerivedMetadata, secp256k1::All>
{
    fn from(builder: OfferWithDerivedMetadataBuilder<'a>) -> Self {
        let OfferWithDerivedMetadataBuilder {
            offer,
            metadata_strategy,
            secp_ctx,
        } = builder;
...
impl OfferContents {
    pub fn chains(&self) -> Vec<ChainHash> {
        self.chains
            .as_ref()
            .cloned()
            .unwrap_or_else(|| vec![self.implied_chain()])
    }
    ...
    pub fn metadata(&self) -> Option<&Vec<u8>> {
        self.metadata
            .as_ref()
            .and_then(|metadata| metadata.as_bytes())
    }
    ...
    pub fn description(&self) -> Option<PrintableString<'_>> {
        self.description
            .as_ref()
            .map(|description| PrintableString(description))
    }
    ...
    pub fn issuer(&self) -> Option<PrintableString<'_>> {
        self.issuer
            .as_ref()
            .map(|issuer| PrintableString(issuer.as_str()))
    }
...
impl core::fmt::Display for CurrencyCodeError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(
            f,
            "invalid currency code: must be 3 uppercase ASCII letters (ISO 4217)"
        )
    }
}
...
mod tests {
    ...
    fn builds_offer_with_chains() {
        ...
        assert_eq!(
            offer.as_tlv_stream().0.chains,
            Some(&vec![mainnet, testnet])
        );
    }
    ...
    fn builds_offer_with_amount() {
        ...
        let invalid_amount = Amount::Bitcoin {
    ...
    fn builds_offer_with_description() {
        ...
        assert_eq!(
            offer.as_tlv_stream().0.description,
    ...
    fn builds_offer_with_features() {
        ...
        assert_eq!(
            offer.as_tlv_stream().0.features,
            Some(&OfferFeatures::unknown())
        );
    ...
    fn builds_offer_with_absolute_expiry() {
        ...
        assert_eq!(
            offer.as_tlv_stream().0.absolute_expiry,
            Some(future_expiry.as_secs())
        );
    ...
    fn parses_offer_with_experimental_tlv_records() {
        ...
        let mut encoded_offer = Vec::new();
        offer.write(&mut encoded_offer).unwrap();
        BigSize(EXPERIMENTAL_OFFER_TYPES.start + 1)
            .write(&mut encoded_offer)
            .unwrap();
        BigSize(32).write(&mut encoded_offer).unwrap();
        [42u8; 32].write(&mut encoded_offer).unwrap();
        ...
        let mut encoded_offer = Vec::new();
        offer.write(&mut encoded_offer).unwrap();
        BigSize(EXPERIMENTAL_OFFER_TYPES.start)
            .write(&mut encoded_offer)
            .unwrap();
        BigSize(32).write(&mut encoded_offer).unwrap();
        [42u8; 32].write(&mut encoded_offer).unwrap();
    ...
    fn fails_parsing_offer_with_out_of_range_tlv_records() {
        ...
        let mut encoded_offer = Vec::new();
        offer.write(&mut encoded_offer).unwrap();
        BigSize(EXPERIMENTAL_OFFER_TYPES.end)
            .write(&mut encoded_offer)
            .unwrap();
        BigSize(32).write(&mut encoded_offer).unwrap();
        [42u8; 32].write(&mut encoded_offer).unwrap();
