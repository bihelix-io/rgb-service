    ...
    pub fn derive_from<W: Writeable, T: secp256k1::Signing>(
        ...
        match self {
            ...
            Metadata::Derived(metadata_material) => (
                Metadata::Bytes(metadata_material.derive_metadata(iv_bytes, tlv_stream)),
                None,
            ),
...
fn verify_metadata<T: secp256k1::Signing>(
    ...
    if metadata.len() == Nonce::LENGTH {
        ...
        #[allow(unused_mut)]
        let mut ok = fixed_time_eq(
            &signing_pubkey.serialize(),
            &derived_keys.public_key().serialize(),
        );
        #[cfg(fuzzing)]
        if metadata[0] & 1 == 0 {
            ok = true;
...
pub(crate) fn hmac_for_payment_tlvs(
    receive_tlvs: &UnauthenticatedReceiveTlvs,
    nonce: Nonce,
    expanded_key: &ExpandedKey,
) -> Hmac<Sha256> {
