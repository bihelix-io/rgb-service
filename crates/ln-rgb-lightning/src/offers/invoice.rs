    ...
    ($self: ident, $self_type: ty) => {
        ...
        pub fn build($self: $self_type) -> Result<UnsignedBolt12Invoice, Bolt12SemanticError> {
            ...
            #[cfg(not(feature = "std"))]
            {
                if $self
                    .invoice
                    .is_offer_or_refund_expired_no_std($self.invoice.created_at())
                {
                    return Err(Bolt12SemanticError::AlreadyExpired);
                }
            }
...
impl InvoiceContents {
    ...
    fn verify<T: secp256k1::Signing>(
        ...
        let invreq_records = TlvStream::new(bytes)
        ...
        let tlv_stream = offer_records
            .chain(invreq_records)
            .chain(experimental_records);
        ...
        (
            InvoiceTlvStreamRef {
                paths: Some(Iterable(
                    self.payment_paths
                        .iter()
                        .map(|path| path.inner_blinded_path()),
                )),
                blindedpay: Some(Iterable(
                    self.payment_paths.iter().map(|path| &path.payinfo),
                )),
                created_at: Some(self.created_at.as_secs()),
                relative_expiry: self
                    .relative_expiry
...
impl TryFrom<ParsedMessage<FullInvoiceTlvStream>> for Bolt12Invoice {
    ...
    fn try_from(invoice: ParsedMessage<FullInvoiceTlvStream>) -> Result<Self, Self::Error> {
        ...
        let signature = signature.ok_or(Bolt12ParseError::InvalidSemantics(
...
pub(super) fn construct_payment_paths(
    blinded_payinfos: Option<Vec<BlindedPayInfo>>,
...
pub(super) fn check_invoice_signing_pubkey(
    invoice_signing_pubkey: &PublicKey,
    offer_tlv_stream: &OfferTlvStream,
...
mod tests {
    ...
    fn builds_invoice_for_offer_with_defaults() {
        ...
        assert_eq!(
            unsigned_invoice.chain(),
            ChainHash::using_genesis_block(Network::Bitcoin)
        );
        assert_eq!(unsigned_invoice.amount_msats(), 1000);
        assert_eq!(
            unsigned_invoice.invoice_request_features(),
        ...
        assert_eq!(
            unsigned_invoice.invoice_features(),
        ...
        assert_eq!(
            invoice.invoice_request_features(),
            &InvoiceRequestFeatures::empty()
        );
        ...
        assert!(secp_ctx
            .verify_schnorr(&invoice.signature, &digest, &pubkey)
            .is_ok());
        ...
        assert_eq!(
            ...
            (
                PayerTlvStreamRef {
                    metadata: Some(&encrypted_payment_id.to_vec())
                },
                OfferTlvStreamRef {
                    chains: None,
                    metadata: None,
                ...
                SignatureTlvStreamRef {
                    signature: Some(&invoice.signature())
                },
                ExperimentalOfferTlvStreamRef {
    ...
    fn builds_invoice_from_offer_using_derived_keys() {
        ...
        let blinded_path = BlindedMessagePath::from_blinded_path(
            ...
            vec![
                BlindedHop {
                    blinded_node_id: pubkey(42),
                    encrypted_payload: vec![0; 43],
                },
                BlindedHop {
                    blinded_node_id: node_id,
    ...
    fn builds_invoice_with_relative_expiry() {
        ...
        assert_eq!(
            tlv_stream.relative_expiry,
            Some(one_hour.as_secs() as u32 - 1)
        );
    }
    ...
    fn fails_signing_invoice() {
            ...
            .sign(payer_sign)
        {
            ...
            Err(e) => assert_eq!(
                e,
                SignError::Verification(secp256k1::Error::IncorrectSignature)
            ),
        }
    }
    ...
    fn parses_invoice_with_payment_paths() {
        ...
        let mut tlv_stream = invoice.as_tlv_stream();
        tlv_stream.3.paths = Some(Iterable(
            empty_payment_paths
                .iter()
                .map(|path| path.inner_blinded_path()),
        ));
    ...
    fn parses_invoice_with_fallback_address() {
        ...
        fallbacks.push(FallbackAddress {
            version: 1,
            program: vec![0u8; 41],
        });
        fallbacks.push(FallbackAddress {
    ...
    fn parses_invoice_with_node_id_from_blinded_path() {
        ...
        let paths = [
            BlindedMessagePath::from_blinded_path(
                ...
                vec![
                    BlindedHop {
                        blinded_node_id: pubkey(43),
                        encrypted_payload: vec![0; 43],
                    },
                    BlindedHop {
                        blinded_node_id: pubkey(44),
                        encrypted_payload: vec![0; 44],
                    },
                ],
            ),
            BlindedMessagePath::from_blinded_path(
                ...
                vec![
                    BlindedHop {
                        ...
                        encrypted_payload: vec![0; 45],
                    },
                    BlindedHop {
                        blinded_node_id: pubkey(46),
                        encrypted_payload: vec![0; 46],
                    },
                ],
            ),
        ];
    ...
    fn parses_invoice_with_unknown_tlv_records() {
        ...
        match Bolt12Invoice::try_from(encoded_invoice) {
            ...
            Err(e) => assert_eq!(
                e,
                Bolt12ParseError::Decode(DecodeError::UnknownRequiredFeature)
            ),
        }
    }
    ...
    fn parses_invoice_with_experimental_tlv_records() {
        ...
        unsigned_invoice
            .experimental_bytes
            .extend_from_slice(&unknown_bytes);
