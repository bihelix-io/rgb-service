...
impl AsyncReceiveOfferCache {
    ...
    pub(super) fn should_build_offer_with_paths(
        &self,
        offer_paths: &[BlindedMessagePath],
        offer_paths_absolute_expiry_secs: Option<u64>,
        slot: u16,
        duration_since_epoch: Duration,
    ) -> bool {
        ...
        // `MIN_OFFER_PATHS_RELATIVE_EXPIRY_SECS`.
        let min_offer_paths_absolute_expiry = duration_since_epoch
            .as_secs()
            .saturating_add(MIN_OFFER_PATHS_RELATIVE_EXPIRY_SECS);
        ...
        self.offers_with_idx()
            .all(|(_, offer)| offer.offer.paths() != offer_paths)
    }
    ...
    pub(super) fn cache_pending_offer(
        ...
        match self.offers.get_mut(slot as usize) {
            ...
            None => {
                debug_assert!(
                    false,
                    "Slot in cache was invalid but should'be been checked above"
                );
                return Err(());
            }
        }
    ...
    fn slot_needs_offer(&self, slot: u16, duration_since_epoch: Duration) -> bool {
        match self.offers.get(slot as usize) {
            ...
            None => {
                debug_assert!(
                    false,
                    "Got offer paths for a non-existent slot in the cache"
                );
                false
            }
        }
    ...
    fn needs_new_offer_idx(&self, duration_since_epoch: Duration) -> Option<usize> {
        ...
        let no_replaceable_offers = self.offers_with_idx().all(|(_, offer)| {
            matches!(
                offer.status,
                OfferStatus::Used { .. } | OfferStatus::Pending
            )
        });
        ...
        let num_payable_offers = self
            ...
            .filter(|(_, offer)| {
                matches!(
                    offer.status,
                    OfferStatus::Used { .. } | OfferStatus::Ready { .. }
                )
            })
            .count();
    ...
    pub(super) fn static_invoice_persisted(&mut self, context: AsyncPaymentsContext) -> bool {
        ...
        if let Some(Some(ref mut offer)) = offer_entry {
            match offer.status {
                OfferStatus::Used {
                    invoice_created_at: ref mut inv_created_at,
                }
                | OfferStatus::Ready {
                    invoice_created_at: ref mut inv_created_at,
                } => {
