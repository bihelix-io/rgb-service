...
impl Notifier {
    ...
    pub fn get_future(&self) -> Future {
        ...
        if let Some(existing_state) = &lock.1 {
            Future {
                state: Arc::clone(&existing_state),
                self_idx,
            }
        } else {
...
impl Drop for Future {
    fn drop(&mut self) {
        self.state
            ...
            .retain(|(idx, _)| *idx != self.self_idx);
    }
}
...
impl<'a> StdFuture for Future {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if state.complete {
            ...
            state
                .std_future_callbacks
                .push((self.self_idx, StdWaker(waker)));
            Poll::Pending
        }
    }
}
...
impl Sleeper {
    ...
    pub fn from_single_future(future: &Future) -> Self {
        Self {
            notifiers: vec![Arc::clone(&future.state)],
        }
    }
    /// Constructs a new sleeper from two futures, allowing blocking on both at once.
    pub fn from_two_futures(fut_a: &Future, fut_b: &Future) -> Self {
        Self {
            notifiers: vec![Arc::clone(&fut_a.state), Arc::clone(&fut_b.state)],
        }
    }
    ...
    pub fn new(futures: Vec<Future>) -> Self {
        Self {
            notifiers: futures.into_iter().map(|f| Arc::clone(&f.state)).collect(),
        }
    }
        ...
        let notified_fut_mtx = Arc::new(Mutex::new(None));
        {
            for notifier_mtx in self.notifiers.iter() {
                ...
                notifier
                    ...
                    .push(Box::new(move |notifier_ref| {
    ...
    pub fn wait(&self) {
        ...
        let notified_fut = cv
            .wait_while(notified_fut_mtx.lock().unwrap(), |fut_opt| {
                fut_opt.is_none()
            })
            .unwrap()
            .take()
...
mod tests {
    ...
    unsafe fn waker_clone(ptr: *const ()) -> RawWaker {
        let p = ptr as *const Arc<AtomicBool>;
        RawWaker::new(
            Box::into_raw(Box::new(Arc::clone(&*p))) as *const (),
            &WAKER_V_TABLE,
        )
    }
    ...
    fn test_multi_future_sleep() {
        ...
        assert!(
            !Sleeper::from_two_futures(&notifier_a.get_future(), &notifier_b.get_future())
