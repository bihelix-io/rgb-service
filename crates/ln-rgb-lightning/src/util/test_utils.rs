...
impl<'a> TestRouter<'a> {
    pub fn new(
        network_graph: Arc<NetworkGraph<&'a TestLogger>>,
        logger: &'a TestLogger,
    ...
    fn find_route(
        ...
        if let Some((find_route_query, find_route_res)) = next_route_opt {
            ...
            if let Some(res) = find_route_res {
                ...
                route_res = self
                    .router
                    .find_route(payer, params, first_hops, inflight_htlcs);
            }
        } else {
            route_res = self
                .router
...
impl<'a> TestChainMonitor<'a> {
    pub fn new(
        ...
        broadcaster: &'a dyn SyncBroadcaster,
        logger: &'a TestLogger,
        fee_estimator: &'a TestFeeEstimator,
        persister: &'a dyn SyncPersist,
    ...
    pub fn complete_sole_pending_chan_update(&self, channel_id: &ChannelId) {
        let (_, latest_update) = self
            ...
            .clone();
        self.chain_monitor
            .channel_monitor_updated(*channel_id, latest_update)
            .unwrap();
    }
    ...
    pub fn load_existing_monitor(
        ...
        self.added_monitors
            .lock()
            .unwrap()
            .push((channel_id, monitor));
        self.chain_monitor
            .load_existing_monitor(channel_id, new_monitor)
    }
...
impl<Signer: sign::ecdsa::EcdsaChannelSigner> Persist<Signer> for WatchtowerPersister {
    fn persist_new_channel(
        ...
        let initial_counterparty_commitment_tx = data
    ...
    fn update_persisted_channel(
        ...
        let res = self
            .persister
            .update_persisted_channel(monitor_name, update, data);
...
impl TestStore {
    ...
    pub fn complete_async_writes_through(
        ...
        let pending_writes = writes_lock
            .get_mut(&key)
            .expect("No pending writes for given key");
        pending_writes.retain(|(id, res, data)| {
            if *id <= write_id {
                let namespace = persisted_lock
                    .entry(prefix.clone())
                    .or_insert(new_hash_map());
    ...
    pub fn complete_all_async_writes(&self) {
        let pending_writes: Vec<String> = self
            .pending_async_writes
            .lock()
            .unwrap()
            .keys()
...
impl TestChainSource {
    pub fn new(network: Network) -> Self {
        ...
        let utxo_ret = Mutex::new(UtxoResult::Sync(Ok(TxOut {
            value: Amount::MAX,
            script_pubkey,
        })));
        Self {
    ...
    pub fn remove_watched_txn_and_outputs(&self, outpoint: OutPoint, script_pubkey: ScriptBuf) {
        self.watched_outputs
            .lock()
            .unwrap()
            .remove(&(outpoint, script_pubkey.clone()));
        self.watched_txn
            .lock()
            .unwrap()
            .remove(&(outpoint.txid, script_pubkey));
    }
}
...
impl TestWalletSource {
    ...
    pub fn remove_utxo(&self, outpoint: bitcoin::OutPoint) {
        self.utxos
            .lock()
            .unwrap()
            .retain(|utxo| utxo.outpoint != outpoint);
    }
    ...
    pub fn sign_tx(
        ...
        for i in 0..tx.input.len() {
            if let Some(utxo) = utxos
                .iter()
                .find(|utxo| utxo.outpoint == tx.input[i].previous_output)
            {
