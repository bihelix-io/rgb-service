    ...
    FE::Target: FeeEstimator,
{
    ...
    > {
        ...
        for monitor_key in monitor_list {
            let result = self
                .0
                .maybe_read_channel_monitor_with_updates(monitor_key.as_str())
                .await?;
    ...
    FE::Target: FeeEstimator,
{
    ...
    > {
        ...
        if monitor_cursor
            .get_ref()
            .starts_with(MONITOR_UPDATING_PERSISTER_PREPEND_SENTINEL)
        {
    ...
    async fn read_monitor_update(
        ...
        let update_bytes = self
            .kv_store
            .read(primary, monitor_key, update_name.as_str())
            .await?;
        ChannelMonitorUpdate::read(&mut &update_bytes[..]).map_err(|e| {
            ...
            io::Error::new(
                io::ErrorKind::InvalidData,
                "Failed to read ChannelMonitorUpdate",
            )
        })
    }
    ...
    async fn cleanup_stale_updates_for_monitor_to(
        ...
        for update in updates {
            ...
            if update_name.0 <= latest_update_id {
                self.kv_store
                    .remove(primary, monitor_key, update_name.as_str(), lazy)
                    .await?;
            }
        }
        Ok(())
    ...
    fn persist_new_channel<ChannelSigner: EcdsaChannelSigner>(
        ...
        let secondary = CHANNEL_MONITOR_PERSISTENCE_SECONDARY_NAMESPACE;
        self.kv_store
            .write(primary, secondary, monitor_key.as_str(), monitor_bytes)
    }
    ...
    async fn archive_persisted_channel(&self, monitor_name: MonitorName) {
        ...
        match self
            .kv_store
            .write(primary, secondary, &monitor_key, monitor.encode())
            .await
        {
            Ok(()) => {}
            Err(_e) => return,
        ...
        let _ = self
            .kv_store
            .remove(primary, secondary, &monitor_key, true)
            .await;
    }
    ...
    async fn cleanup_in_range(&self, monitor_name: MonitorName, start: u64, end: u64) {
        ...
        for update_id in start..=end {
            ...
            let res = self
                .kv_store
                .remove(primary, &monitor_key, update_name.as_str(), true)
                .await;
            if let Err(e) = res {
                log_error!(
                    self.logger,
    ...
    fn from_str(monitor_key: &str) -> Result<Self, io::Error> {
        ...
        if let Some(part) = parts.next() {
            ...
            let bytes = <[u8; 32]>::from_hex(id).map_err(|_| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "Invalid channel ID in stored key",
                )
            })?;
    ...
    pub fn new(name: String) -> Result<Self, io::Error> {
        match name.parse::<u64>() {
            ...
            Err(_) => Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "cannot parse u64 from update name",
            )),
        }
    }
