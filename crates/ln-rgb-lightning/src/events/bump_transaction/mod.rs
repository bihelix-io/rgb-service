...
impl AnchorDescriptor {
    ...
    pub fn previous_utxo(&self) -> TxOut {
        ...
        let script_pubkey = if tx_params
    ...
    L::Target: Logger + MaybeSend,
{
    ...
    pub fn new(source: W, logger: L) -> Self {
        Self {
            source,
            logger,
            locked_utxos: Mutex::new(new_hash_map()),
        }
    }
    ...
    async fn select_confirmed_utxos_internal(
        ...
        utxos: &[Utxo],
        claim_id: ClaimId,
        force_conflicting_utxo_spend: bool,
        tolerate_high_network_feerates: bool,
        target_feerate_sat_per_1000_weight: u32,
        preexisting_tx_weight: u64,
        input_amount_sat: Amount,
        target_amount_sat: Amount,
        max_tx_weight: u64,
        ...
        let change_output = if change_output_amount < change_script.minimal_non_dust() {
            log_debug!(
                self.logger,
                "Coin selection attempt did not yield change output"
            );
            None
        } else {
            Some(TxOut {
    ...
    fn select_confirmed_utxos<'a>(
        ...
        Box::pin(async move {
            ...
            let total_output_size: u64 = must_pay_to
                ...
                .sum();
            let total_satisfaction_weight: u64 = must_spend
                .iter()
            ...
            let input_amount_sat = must_spend
                .iter()
                .map(|input| input.previous_utxo.value)
                .sum();
    ...
    L::Target: Logger,
{
    ...
    async fn handle_channel_close(
        ...
        loop {
            ...
            let must_spend_amount = must_spend
                .iter()
                .map(|input| input.previous_utxo.value)
                .sum::<Amount>();
            ...
            let input_satisfaction_weight: u64 = coin_selection
                .confirmed_utxos
                .iter()
                ...
                < package_target_feerate_sat_per_1000_weight.into()
            {
                ...
                {
                    debug_assert!(
                        anchor_psbt.unsigned_tx.output[0]
                            .script_pubkey
                            .is_op_return(),
            ...
            if channel_type.supports_anchors_zero_fee_htlc_tx() {
                ...
                let channel_parameters = &anchor_descriptor
                    .channel_derivation_parameters
                    .transaction_parameters;
            ...
            self.broadcaster
                .broadcast_transactions(&[&commitment_tx, &anchor_tx]);
            return Ok(());
        }
    }
    ...
    async fn handle_htlc_resolution(
        ...
        while broadcasted_htlcs < htlc_descriptors.len() {
            ...
            #[cfg(debug_assertions)]
            let must_spend_satisfaction_weight = must_spend
                .iter()
                .map(|input| input.satisfaction_weight)
                .sum::<u64>();
            #[cfg(debug_assertions)]
            let must_spend_amount = must_spend
                .iter()
                .map(|input| input.previous_utxo.value.to_sat())
                .sum::<u64>();
            ...
            #[cfg(debug_assertions)]
            let input_value: u64 = coin_selection
                .confirmed_utxos
                .iter()
                ...
                (
                    ...
                    CoinSelection {
                        confirmed_utxos: vec![Utxo {
                            outpoint: OutPoint {
                                txid: Txid::from_byte_array([44; 32]),
                                vout: 0,
                            },
                            output: TxOut {
