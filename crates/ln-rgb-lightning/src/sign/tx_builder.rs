...
use crate::ln::chan_utils::{
    commit_tx_fee_sat, commit_tx_fee_sat_with_rgb, htlc_success_tx_weight, htlc_timeout_tx_weight,
...
use crate::prelude::*;
use crate::rgb::{ContractId, RgbFundingRef};
...
impl HTLCAmountDirection {
    fn is_dust(
        ...
        let htlc_tx_fee_sat = if self.outbound == local {
            timeout_tx_fee_sat
        } else {
            success_tx_fee_sat
        };
...
fn commit_plus_htlc_tx_fees_msat(
    local: bool,
    next_commitment_htlcs: &[HTLCAmountDirection],
    dust_buffer_feerate: u32,
    feerate: u32,
    broadcaster_dust_limit_satoshis: u64,
    channel_type: &ChannelTypeFeatures,
) -> (u64, u64) {
    ...
    let second_stage_fees_sat = htlc_tx_fees_sat(
        feerate,
        accepted_nondust_htlcs,
        offered_nondust_htlcs,
        channel_type,
    );
    ...
    let extra_accepted_htlc_second_stage_fees_sat = htlc_tx_fees_sat(
        feerate,
        accepted_nondust_htlcs + 1,
        offered_nondust_htlcs,
        channel_type,
    );
...
fn subtract_addl_outputs(
    ...
    value_to_self_after_htlcs_msat: u64,
    value_to_remote_after_htlcs_msat: u64,
    channel_type: &ChannelTypeFeatures,
) -> Result<(u64, u64), ()> {
    ...
    if is_outbound_from_holder {
        Ok((
            value_to_self_after_htlcs_msat
                .checked_sub(total_anchors_sat * 1000)
                .ok_or(())?,
...
pub(crate) trait TxBuilder {
    ...
    fn subtract_non_htlc_outputs(
        &self,
        is_outbound_from_holder: bool,
        value_to_self_after_htlcs: u64,
        value_to_remote_after_htlcs: u64,
        channel_type: &ChannelTypeFeatures,
    ) -> (u64, u64);
...
impl SpecTxBuilder {
    pub(crate) fn build_commitment_transaction_with_rgb<L: Deref>(
        &self,
        local: bool,
        commitment_number: u64,
        per_commitment_point: &PublicKey,
    ...
    where
        L::Target: Logger,
    {
        self.build_commitment_transaction_internal(
            local,
            commitment_number,
    ...
    fn build_commitment_transaction_internal<L: Deref>(
        &self,
        local: bool,
        commitment_number: u64,
        per_commitment_point: &PublicKey,
        ...
        secp_ctx: &Secp256k1<secp256k1::All>,
        value_to_self_msat: u64,
        mut htlcs_in_tx: Vec<HTLCOutputInCommitment>,
        feerate_per_kw: u32,
        broadcaster_dust_limit_satoshis: u64,
        to_each_value_rgb: Option<(ContractId, u64, u64)>,
        funding_rgb_transfer: Option<RgbFundingRef>,
        ...
        L::Target: Logger,
    {
        ...
        let commit_tx_fee_sat = commit_tx_fee_sat_with_rgb(
            ...
            to_each_value_rgb.is_some(),
        );
        let value_to_self_after_htlcs_msat = value_to_self_msat
            .checked_sub(local_htlc_total_msat)
            .unwrap();
        ...
        let mut to_broadcaster_value_sat = if local {
            value_to_self
        } else {
            value_to_remote
        };
        let mut to_countersignatory_value_sat = if local {
            value_to_remote
        } else {
            value_to_self
        };
        ...
        if to_broadcaster_value_sat >= broadcaster_dust_limit_satoshis {
            ...
            if let Some((_, broadcaster_rgb, _)) = to_each_value_rgb.as_ref() {
                debug_assert_eq!(*broadcaster_rgb, 0);
            }
        ...
        if to_countersignatory_value_sat >= broadcaster_dust_limit_satoshis {
            ...
            if let Some((_, _, countersignatory_rgb)) = to_each_value_rgb.as_ref() {
                debug_assert_eq!(*countersignatory_rgb, 0);
            }
        ...
        let tx = if to_each_value_rgb.is_some() {
            CommitmentTransaction::new_with_rgb(
                commitment_number,
                per_commitment_point,
                to_broadcaster_value_sat,
                to_countersignatory_value_sat,
...
impl TxBuilder for SpecTxBuilder {
    fn get_next_commitment_stats(
        ...
        let inbound_htlcs_count = next_commitment_htlcs
            .iter()
        ...
        let inbound_htlcs_value_msat: u64 = next_commitment_htlcs
            ...
            .filter_map(|htlc| (!htlc.outbound).then_some(htlc.amount_msat))
            .sum();
        let value_to_holder_after_htlcs_msat = value_to_holder_msat
            .checked_sub(outbound_htlcs_value_msat)
            .ok_or(())?;
        let value_to_counterparty_after_htlcs_msat = value_to_counterparty_msat
