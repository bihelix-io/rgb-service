...
impl StaticPaymentOutputDescriptor {
        ...
        self.channel_transaction_parameters
            ...
            .and_then(|channel_params| {
                    ...
                    .supports_anchors_zero_fee_htlc_tx()
                {
                    ...
                    Some(chan_utils::get_to_countersigner_keyed_anchor_redeemscript(
                        &payment_point,
                    ))
                } else {
                    None
    ...
    pub fn needs_csv_1_for_spend(&self) -> bool {
        ...
        chan_params.map_or(false, |p| {
            p.channel_type_features.supports_anchors_zero_fee_htlc_tx()
        })
    }
}
...
impl SpendableOutputDescriptor {
    ...
    pub fn create_spendable_outputs_psbt<T: secp256k1::Signing>(
        ...
        let psbt_inputs = descriptors
            .iter()
            .map(|d| d.to_psbt_input(&secp_ctx))
            .collect::<Vec<_>>();
        let psbt = Psbt {
            inputs: psbt_inputs,
...
impl InMemorySigner {
    ...
    pub fn sign_counterparty_payment_input<C: Signing>(
        ...
        let (remotepubkey, payment_key) = if spk_v1 == descriptor.output.script_pubkey {
            (
                bitcoin::PublicKey::new(payment_point_v1),
                &self.payment_key_v1,
            )
        } else {
            ...
            (
                bitcoin::PublicKey::new(payment_point_v2),
                &self.payment_key_v2,
            )
        };
...
impl ChannelSigner for InMemorySigner {
    fn get_per_commitment_point(
        ...
        let commitment_secret = SecretKey::from_slice(&chan_utils::build_commitment_secret(
    ...
    fn release_commitment_secret(&self, idx: u64) -> Result<[u8; 32], ()> {
        Ok(chan_utils::build_commitment_secret(
            &self.commitment_seed,
            idx,
        ))
    }
    ...
    fn pubkeys(&self, secp_ctx: &Secp256k1<secp256k1::All>) -> ChannelPublicKeys {
        ...
        let payment_key = if self.v2_remote_key_derivation {
            &self.payment_key_v2
        } else {
            &self.payment_key_v1
        };
    ...
    fn new_funding_pubkey(
        ...
        self.funding_key(Some(splice_parent_funding_txid))
            .public_key(secp_ctx)
    }
...
impl EcdsaChannelSigner for InMemorySigner {
    fn sign_counterparty_commitment(
        ...
        assert!(
            channel_parameters.is_populated(),
            "Channel parameters must be fully populated"
        );
        ...
        let counterparty_keys = channel_parameters
    ...
    fn sign_holder_htlc_transaction(
        ...
        let channel_parameters = &htlc_descriptor
            .channel_derivation_parameters
            .transaction_parameters;
        assert!(
            channel_parameters.is_populated(),
    ...
    fn sign_closing_transaction(
        ...
        let funding_key = self.funding_key(channel_parameters.splice_parent_funding_txid);
        let funding_pubkey = funding_key.public_key(secp_ctx);
        let counterparty_funding_key = &channel_parameters
            .counterparty_pubkeys()
            .expect(MISSING_PARAMS_ERR)
            .funding_pubkey;
        let channel_funding_redeemscript =
...
impl KeysManager {
        ...
        match Xpriv::new_master(Network::Testnet, seed) {
            Ok(master_key) => {
                    ...
                    match master_key.derive_priv(&secp_ctx, &DESTINATION_SCRIPT_INDEX) {
                        Ok(destination_key) => {
                            let wpubkey_hash = WPubkeyHash::hash(
                                &Xpub::from_priv(&secp_ctx, &destination_key)
                                    .to_pub()
                                    .to_bytes(),
                            );
                            Builder::new()
                ...
                let mut res = KeysManager {
                    ...
                    peer_storage_key: PeerStorageKey {
                        inner: peer_storage_key.secret_bytes(),
                    },
    ...
    pub fn possible_v2_counterparty_closed_balance_spks<C: Signing>(
        ...
        for idx in 0..STATIC_PAYMENT_KEY_COUNT {
            ...
            res.push(get_countersigner_payment_script(
...
impl NodeSigner for KeysManager {
    ...
    fn sign_invoice(
        ...
        Ok(self
            .secp_ctx
            .sign_ecdsa_recoverable(&hash_to_message!(&hash), secret))
    }
