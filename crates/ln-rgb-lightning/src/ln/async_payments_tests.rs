...
fn pass_static_invoice_server_messages(
    ...
    server
        ...
        .lock()
        .unwrap()
        .push(recipient.node.get_our_node_id());
    recipient
        .message_router
        .peers_override
        .lock()
        .unwrap()
        .push(server.node.get_our_node_id());
...
fn pass_async_payments_oms(
    ...
    sender: &Node,
    always_online_recipient_counterparty: &Node,
    recipient: &Node,
    recipient_id: Vec<u8>,
    invoice_request_path: BlindedMessagePath,
) -> (msgs::OnionMessage, msgs::OnionMessage) {
    ...
    let invreq_om = sender
        .onion_messenger
        .next_onion_message_for_peer(always_online_node_id)
        .unwrap();
    always_online_recipient_counterparty
        .onion_messenger
        .handle_onion_message(sender_node_id, &invreq_om);

    let mut events = always_online_recipient_counterparty
        .node
    ...
    sender
        .onion_messenger
        .handle_onion_message(always_online_node_id, &static_invoice_om);
    ...
    let held_htlc_available_om_0_1 = sender
        .onion_messenger
        .next_onion_message_for_peer(always_online_node_id)
        .unwrap();
    always_online_recipient_counterparty
    ...
    let release_held_htlc = recipient
        .onion_messenger
        .next_onion_message_for_peer(sender_node_id)
        .unwrap();
...
fn create_static_invoice<T: secp256k1::Signing + secp256k1::Verification>(
    always_online_counterparty: &Node,
    recipient: &Node,
    relative_expiry: Option<Duration>,
...
fn extract_invoice_request_om<'a>(
    ...
    extract_expected_om(
        ...
        |peeled_onion| {
            matches!(
                peeled_onion,
                &PeeledOnion::Offers(OffersMessage::InvoiceRequest(_), _, _)
            )
        },
        |_| false,
    )
    ...
    F2: Fn(&PeeledOnion<Infallible>) -> bool,
{
    ...
    while let Some((pk, msg)) = pk_to_msg.pop() {
        let node = next_hop_nodes
            .iter()
            .find(|node| node.node.get_our_node_id() == pk)
            .unwrap();
...
fn build_async_offer_and_init_payment(
    ...
    let inv_server_paths = invoice_server
        .node
        .blinded_paths_for_async_recipient(recipient_id.clone(), None)
        .unwrap();
    recipient
        .node
        .set_paths_to_static_invoice_server(inv_server_paths)
        .unwrap();
    ...
    invoice_server
        .onion_messenger
        .handle_onion_message(peer_id, &invreq_om);
...
fn lock_in_htlc_for_static_invoice(
    static_invoice_om: &msgs::OnionMessage,
    om_peer: PublicKey,
    sender: &Node,
    sender_lsp: &Node,
) -> PaymentHash {
    ...
    sender
        .onion_messenger
        .handle_onion_message(om_peer, &static_invoice_om);
    ...
    sender_lsp
        .node
        .handle_update_add_htlc(sender.node.get_our_node_id(), &update_add);
...
fn invalid_keysend_payment_secret() {
    ...
    let chan_upd_1_2 = create_announced_chan_between_nodes_with_value(&nodes, 1, 2, 1_000_000, 0)
        .0
        .contents;
...
fn ignore_unexpected_static_invoice() {
    ...
    nodes[1]
        .onion_messenger
        .handle_onion_message(nodes[0].node.get_our_node_id(), &invreq_om);
    ...
    let unexpected_static_invoice_om = nodes[1]
        ...
        .next_onion_message_for_peer(nodes[0].node.get_our_node_id())
        .unwrap();
    nodes[0].onion_messenger.handle_onion_message(
        nodes[1].node.get_our_node_id(),
        &unexpected_static_invoice_om,
    );
...
fn ignore_duplicate_invoice() {
    ...
    let invreq_om = always_online_node
        .onion_messenger
        .next_onion_message_for_peer(async_recipient_id)
        .unwrap();
    let peeled_msg = async_recipient
        .onion_messenger
        .peel_onion_message(&invreq_om)
        .unwrap();
    assert!(matches!(
        peeled_msg,
        PeeledOnion::Offers(OffersMessage::InvoiceRequest(_), _, _)
    ...
    let static_invoice_om = always_online_node
        .onion_messenger
        .next_onion_message_for_peer(sender_node_id)
        .unwrap();
    let peeled_msg = sender
        .onion_messenger
        .peel_onion_message(&static_invoice_om)
        .unwrap();
    assert!(matches!(
        peeled_msg,
        PeeledOnion::Offers(OffersMessage::StaticInvoice(_), _, _)
    ));

    // Handling the `invoice_request` from the async recipient we should get back an invoice.
    async_recipient
        .onion_messenger
        .handle_onion_message(always_online_node_id, &invreq_om);
    let invoice_om = async_recipient
        .onion_messenger
        .next_onion_message_for_peer(sender_node_id)
        .unwrap();
    ...
    always_online_node
        .onion_messenger
        .handle_onion_message(sender_node_id, &held_htlc_available_om_0_1);
    let held_htlc_available_om_1_2 = always_online_node
        .onion_messenger
        .next_onion_message_for_peer(async_recipient_id)
        .unwrap();
    async_recipient
    ...
    let release_held_htlc_om = async_recipient
        .onion_messenger
        .next_onion_message_for_peer(sender_node_id)
        .unwrap();
    sender
        .onion_messenger
        .handle_onion_message(async_recipient_id, &release_held_htlc_om);
    ...
    assert_eq!(
        res,
        Some(PaidBolt12Invoice::StaticInvoice(static_invoice.clone()))
    );
    ...
    sender
        .onion_messenger
        .handle_onion_message(async_recipient_id, &invoice_om);
...
fn timeout_unreleased_payment() {
    ...
    server
        .onion_messenger
        .handle_onion_message(sender.node.get_our_node_id(), &invreq_om);
    ...
    sender
        .onion_messenger
        .handle_onion_message(server.node.get_our_node_id(), &static_invoice_om);
...
fn async_receive_mpp() {
    ...
    let node_chanmgrs = create_node_chanmgrs(
        ...
        &[
            None,
            Some(allow_priv_chan_fwds_cfg.clone()),
            Some(allow_priv_chan_fwds_cfg),
            None,
        ],
    );
...
fn amount_doesnt_match_invreq() {
    ...
    nodes[0]
        .node
        .test_modify_pending_payment(&payment_id, |pmt| {
...
fn invalid_async_receive_with_retry<F1, F2>(
    mut modify_outbounds_for_failure: F1,
    mut modify_outbounds_for_success: F2,
) where
    ...
    F2: FnMut(&Node, PaymentId),
{
    ...
    *nodes[2].keys_manager.override_random_bytes.lock().unwrap() = Some(hardcoded_random_bytes);
    ...
    nodes[2]
        .router
        .expect_blinded_payment_paths(static_invoice_paths);
...
fn expired_static_invoice_payment_path() {
    ...
    let final_max_cltv_expiry = {
        let mut blinded_path = static_invoice_expired_paths
            .payment_paths()
            .to_vec()
            .pop()
            .unwrap();
        blinded_path
            ...
            .unwrap();
        match blinded_path
            .decrypt_intro_payload(&nodes[2].keys_manager)
            .unwrap()
            .0
        {
    ...
    connect_blocks(
        &nodes[2],
        final_max_cltv_expiry - nodes[2].best_block_info().1,
    );
...
fn ignore_expired_offer_paths_request() {
    ...
    assert!(matches!(
        server
            .onion_messenger
            .peel_onion_message(&offer_paths_req)
            .unwrap(),
        PeeledOnion::AsyncPayments(AsyncPaymentsMessage::OfferPathsRequest(_), _, _)
    ));
    ...
    advance_time_by(
        OFFER_PATHS_REQ_RELATIVE_EXPIRY + Duration::from_secs(1),
        server,
    );
    server
        .onion_messenger
...
fn ignore_expired_offer_paths_message() {
    ...
    assert!(matches!(
        server
            .onion_messenger
            .peel_onion_message(&serve_static_invoice)
            .unwrap(),
    ...
    advance_time_by(
        TEST_TEMP_REPLY_PATH_RELATIVE_EXPIRY + Duration::from_secs(1),
        recipient,
    );
    recipient
        .onion_messenger
...
fn offer_cache_round_trip_ser() {
    ...
    let chan_id = create_unannounced_chan_between_nodes_with_value(&nodes, 0, 1, 1_000_000, 0)
        .0
        .channel_id;
    let server = &nodes[0];
    let recipient = &nodes[1];
...
fn refresh_static_invoices_for_pending_offers() {
    ...
    for _ in 0..10 {
        ...
        pending_oms
            ...
            .iter()
            .find(
                |msg| match server.onion_messenger.peel_onion_message(&msg).unwrap() {
                    PeeledOnion::AsyncPayments(
                        AsyncPaymentsMessage::ServeStaticInvoice(_),
                        _,
                        _,
                    ) => true,
                    PeeledOnion::AsyncPayments(
...
fn refresh_static_invoices_for_used_offers() {
    ...
    assert_eq!(
        res.0,
        Some(PaidBolt12Invoice::StaticInvoice(updated_invoice))
    );
}
...
fn ignore_expired_static_invoice() {
    ...
    advance_time_by(
        TEST_DEFAULT_ASYNC_RECEIVE_OFFER_EXPIRY + Duration::from_secs(1),
        server,
    );
...
fn refresh_unused_offers() {
    ...
    advance_time_by(
        TEST_OFFER_REFRESH_THRESHOLD - Duration::from_secs(2),
        recipient,
    );
    assert!(recipient
        .onion_messenger
    ...
    assert_eq!(
        ...
        TEST_MAX_CACHED_OFFERS_TARGET as u16 - 1
    );

    // If an offer is used, we shouldn't replace it after the threshold time.
    let offer = recipient.node.get_async_receive_offer().unwrap();
    advance_time_by(
        TEST_OFFER_REFRESH_THRESHOLD + Duration::from_secs(100),
        recipient,
    );
    ...
    let peeled_om = server
        .onion_messenger
        .peel_onion_message(&pending_oms[0])
        .unwrap();
    match peeled_om {
...
fn invoice_server_is_not_channel_peer() {
    ...
    let node_chanmgrs = create_node_chanmgrs(
        4,
        &node_cfgs,
        &[None, Some(allow_priv_chan_fwds_cfg), None, None],
    );
...
fn async_payment_e2e() {
    ...
    recipient
        .node
        .peer_disconnected(invoice_server.node.get_our_node_id());
    recipient
        .onion_messenger
        .peer_disconnected(invoice_server.node.get_our_node_id());
    invoice_server
        .node
        .peer_disconnected(recipient.node.get_our_node_id());
    invoice_server
        .onion_messenger
        .peer_disconnected(recipient.node.get_our_node_id());
    ...
    let held_htlc_om_to_inv_server = sender
        ...
        .unwrap();
    invoice_server.onion_messenger.handle_onion_message(
        sender_lsp.node.get_our_node_id(),
        &held_htlc_om_to_inv_server,
    );
    ...
    invoice_server
        .onion_messenger
        .process_pending_events(&|e| Ok(events_rc.borrow_mut().push(e)));
    ...
    let held_htlc_om = events
        ...
        .find_map(|ev| {
            if let Event::OnionMessageIntercepted { message, .. } = ev {
                ...
                let peeled_onion = recipient
                    .onion_messenger
                    .peel_onion_message(&message)
                    .unwrap();
                if matches!(
                    peeled_onion,
    ...
    invoice_server
        .onion_messenger
        .process_pending_events(&|e| Ok(events.borrow_mut().push(e)));
    assert_eq!(events.borrow().len(), 1);
    assert!(matches!(
        events.into_inner().pop().unwrap(),
    ...
    let (peer_id, release_htlc_om) =
        extract_release_htlc_oms(recipient, &[sender, sender_lsp, invoice_server])
            .pop()
            .unwrap();
    sender_lsp
        .onion_messenger
        .handle_onion_message(peer_id, &release_htlc_om);
...
fn held_htlc_timeout() {
    ...
    recipient
        .onion_messenger
        .handle_onion_message(peer_id, &held_htlc_om);
...
fn fail_held_htlc_on_reconnect() {
    ...
    sender
        .node
        .peer_disconnected(sender_lsp.node.get_our_node_id());
    sender_lsp
        .node
        .peer_disconnected(sender.node.get_our_node_id());
...
fn intercepted_hold_htlc() {
    ...
    *recipient.keys_manager.override_random_bytes.lock().unwrap() = Some(hardcoded_random_bytes);
    ...
    offer_nonce
        .0
        .copy_from_slice(&hardcoded_random_bytes[..Nonce::LENGTH]);
    ...
    recipient
        .router
        .expect_blinded_payment_paths(blinded_payment_path_with_jit_channel_scid);
    ...
    let (peer_id, held_htlc_om) =
        extract_held_htlc_available_oms(sender, &[lsp, recipient, &nodes[2]])
            .pop()
            .unwrap();
    recipient
        .onion_messenger
        .handle_onion_message(peer_id, &held_htlc_om);
    let (peer_id, release_htlc_om) = extract_release_htlc_oms(recipient, &[sender, lsp])
        .pop()
        .unwrap();
    lsp.onion_messenger
        .handle_onion_message(peer_id, &release_htlc_om);
...
fn async_payment_mpp() {
    ...
    sender
        .onion_messenger
        .handle_onion_message(peer_id, &static_invoice_om);
