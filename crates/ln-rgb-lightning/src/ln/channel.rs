...
use crate::ln::chan_utils::{
    commit_tx_fee_sat_with_rgb, get_commitment_transaction_number_obscure_factor, max_htlcs,
...
use crate::rgb::{ContractId, RgbContext};
    ...
    <SP::Target as SignerProvider>::EcdsaSigner: ChannelSigner,
{
        ...
        L::Target: Logger,
    {
        let is_resumable = match &mut self.phase {
            ...
            ChannelPhase::Funded(chan) => chan
                .remove_uncommitted_htlcs_and_mark_paused(logger)
                .is_ok(),
        ...
        L::Target: Logger,
    {
        ...
        log_info!(
            logger,
            "Failed interactive transaction negotiation: {reason}"
        );
    ...
    pub fn tx_complete<L: Deref>(
        ...
        (
            Option<InteractiveTxMessageSend>,
        ...
        L::Target: Logger,
    {
        ...
        let (should_ack, splice_funding_failed) = match &mut self.phase {
            ...
            ChannelPhase::UnfundedV2(pending_v2_channel) => {
                let had_constructor = pending_v2_channel
        ...
        let tx_abort = should_ack.then(|| {
            ...
            log_info!(
                logger,
                "Counterparty failed interactive transaction negotiation: {reason}"
            );
            msgs::TxAbort {
        ...
        let (interactive_tx_constructor, commitment_signed) = match &mut self.phase {
            ChannelPhase::UnfundedV2(chan) => {
                ...
                let commitment_signed = chan
                    .context
                    .get_initial_commitment_signed_v2(&chan.funding, &&logger);
        ...
        F::Target: FeeEstimator,
    {
        match &self.phase {
            ...
            ChannelPhase::UnfundedOutboundV1(chan) => chan
                .context
                .get_available_balances_for_scope(&chan.funding, fee_estimator),
            ChannelPhase::UnfundedInboundV1(chan) => chan
                .context
                .get_available_balances_for_scope(&chan.funding, fee_estimator),
            ChannelPhase::UnfundedV2(chan) => chan
                .context
                .get_available_balances_for_scope(&chan.funding, fee_estimator),
        }
    }
...
impl FundingScope {
    ...
    pub fn get_funding_tx_confirmations(&self, height: u32) -> u32 {
        ...
        height
            .checked_sub(self.funding_tx_confirmation_height)
            .map_or(0, |c| c + 1)
    }
    ...
    fn for_splice<SP: Deref>(
        prev_funding: &Self,
        context: &ChannelContext<SP>,
        our_funding_contribution: SignedAmount,
        their_funding_contribution: SignedAmount,
    ...
    SP::Target: SignerProvider,
{
    ...
    /// RGB state committed into this channel, if this is an RGB-carrying channel.
    pub rgb_context: Option<RgbContext>,
}
    ...
    SP::Target: SignerProvider,
{
		...
		L::Target: Logger
	{
		...
		let funding_rgb_transfer = context.rgb_context.as_ref()
			.and_then(|rgb_context| rgb_context.funding_ref.clone());
		...
		let channel_monitor = ChannelMonitor::new(
			...
			context.is_manual_broadcast, funding_rgb_transfer,
		);
    ...
    SP::Target: SignerProvider,
{
    ...
    fn new_for_inbound_channel<'a, ES: Deref, F: Deref, L: Deref>(
		...
		msg_push_msat: u64,
		funding_rgb: Option<(ContractId, u64)>,
			...
			SP::Target: SignerProvider,
	{
		...
		let channel_context = ChannelContext {
			...
			rgb_context: funding_rgb.map(|(contract_id, amount)| {
				RgbContext::new_for_inbound_channel(contract_id, amount)
			}),
		};
			...
			L::Target: Logger,
	{
		...
		let channel_context = Self {
			...
			interactive_tx_signing_session: None,
			rgb_context: funding_rgb.map(|(contract_id, amount)| {
				RgbContext::new_for_outbound_channel(contract_id, amount)
			}),
		};
    ...
    fn get_next_commitment_htlcs(
        &self,
        local: bool,
        htlc_candidate: Option<HTLCAmountDirection>,
        ...
        let pending_inbound_htlcs = self
            ...
            .map(
                |&InboundHTLCOutput { amount_msat, .. }| HTLCAmountDirection {
                    outbound: false,
                    amount_msat,
                },
            );
        ...
        let pending_outbound_htlcs = self
            ...
            .map(
                |&OutboundHTLCOutput { amount_msat, .. }| HTLCAmountDirection {
                    outbound: true,
                    amount_msat,
                },
            );
    ...
    fn get_next_local_commitment_stats(
        ...
        let mut ret = SpecTxBuilder {}.get_next_commitment_stats(
            true,
            funding.is_outbound(),
        ...
        if self.rgb_context.is_some() {
            ret.commit_tx_fee_sat = commit_tx_fee_sat_with_rgb(
                feerate_per_kw,
                ret.nondust_htlc_count,
                funding.get_channel_type(),
                true,
            );
        }
        ...
        #[cfg(any(test, fuzzing))]
        {
            if addl_nondust_htlc_count == 0 {
                *funding.next_local_fee.lock().unwrap() = PredictedNextFee {
                ...
                let mut predicted_stats = SpecTxBuilder {}
                ...
                if self.rgb_context.is_some() {
                    predicted_stats.commit_tx_fee_sat = commit_tx_fee_sat_with_rgb(
                        feerate_per_kw,
    ...
    fn validate_commitment_signed<F: Deref, L: Deref>(
        ...
        (
            ...
            Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>,
        ),
        ChannelError,
    >
    where
        ...
        L::Target: Logger,
    {
        ...
        for (htlc, counterparty_sig) in commitment_data
            .tx
            .nondust_htlcs()
            .iter()
            .zip(msg.htlc_signatures.iter())
        {
        ...
        L::Target: Logger,
    {
                    ...
                    * 1000
        {
            ...
            log_debug!(
                logger,
                "Cannot afford to send new feerate at {}",
                feerate_per_kw
            );
            return false;
        }
        ...
        L::Target: Logger,
    {
        ...
        let fee_spike_buffer_htlc = if funding
            .get_channel_type()
        ...
        let feerate = cmp::max(
            self.feerate_per_kw,
            self.pending_update_fee.map(|(fee, _)| fee).unwrap_or(0),
        );
		...
		where L::Target: Logger
	{
		...
		let mut value_to_self_claimed_rgb = 0;
		let mut value_to_remote_claimed_rgb = 0;
		let mut outbound_htlc_total_rgb = 0;
		let mut inbound_htlc_total_rgb = 0;
		...
		for htlc in self.pending_inbound_htlcs.iter() {
			if htlc.state.included_in_commitment(generated_by_local) {
				...
				inbound_htlc_total_rgb += htlc.amount_rgb.map(|(_, amount)| amount).unwrap_or(0);
				...
				if let Some(preimage) = htlc.state.preimage() {
					...
					value_to_self_claimed_rgb +=
						htlc.amount_rgb.map(|(_, amount)| amount).unwrap_or(0);
				}
			}
		};
		...
		for htlc in self.pending_outbound_htlcs.iter() {
			...
			if htlc.state.included_in_commitment(generated_by_local) {
				...
				outbound_htlc_total_rgb += htlc.amount_rgb.map(|(_, amount)| amount).unwrap_or(0);
		...
		let (to_each_value_rgb, funding_rgb_transfer) =
			if let Some(rgb_context) = self.rgb_context.as_ref() {
				let holder_rgb_before_htlcs = (rgb_context.to_self as i64)
					+ (value_to_self_claimed_rgb as i64)
					- (value_to_remote_claimed_rgb as i64);
				assert!(holder_rgb_before_htlcs >= 0);
				assert!(holder_rgb_before_htlcs <= rgb_context.funding_rgb as i64);
				let holder_rgb = holder_rgb_before_htlcs - outbound_htlc_total_rgb as i64;
				assert!(holder_rgb >= 0);
				let counterparty_rgb = rgb_context
					.funding_rgb as i64
					- holder_rgb_before_htlcs
					- inbound_htlc_total_rgb as i64;
				assert!(counterparty_rgb >= 0);
				let (broadcaster_rgb, countersignatory_rgb) = if local {
					(holder_rgb as u64, counterparty_rgb as u64)
				} else {
					(counterparty_rgb as u64, holder_rgb as u64)
				};
				match rgb_context.funding_ref.clone() {
					Some(transfer) => (
						Some((rgb_context.contract_id, broadcaster_rgb, countersignatory_rgb)),
						Some(transfer),
					),
					None => (None, None),
				}
		...
		let (tx, stats) = SpecTxBuilder {}.build_commitment_transaction_with_rgb(
			local,
			commitment_number,
    ...
    pub fn get_dust_buffer_feerate(&self, outbound_feerate_update: Option<u32>) -> u32 {
        ...
        cmp::max(
            feerate_per_kw.saturating_add(2530),
            feerate_plus_quarter.unwrap_or(u32::MAX),
        )
    }
    ...
    pub fn unbroadcasted_batch_funding_txid(&self, funding: &FundingScope) -> Option<Txid> {
        self.unbroadcasted_funding_txid(funding)
            .filter(|_| self.is_batch_funding())
    }
    ...
    fn force_shutdown(
        &mut self,
        funding: &FundingScope,
        mut closure_reason: ClosureReason,
    ) -> ShutdownResult {
                    ...
                    for update in update.update.updates.iter() {
                        let have_htlc = match update {
                            ChannelMonitorUpdateStep::LatestCounterpartyCommitment {
                                ...
                                dust.chain(nondust)
                                    .any(|source| source == Some(&htlc.source))
                            }
        ...
        if let ClosureReason::HolderForceClosed {
            ref mut broadcasted_latest_txn,
            ..
        } = &mut closure_reason
        {
...
pub(crate) fn get_holder_selected_channel_reserve_satoshis(
    ...
    let counterparty_chan_reserve_prop_mil = config
    ...
    cmp::min(
        channel_value_satoshis,
        cmp::max(calculated_reserve, MIN_THEIR_CHAN_RESERVE_SATOSHIS),
    )
}
...
pub(super) struct FundingNegotiationContext {
    ...
    #[allow(dead_code)]
    // TODO(dual_funding): Remove once contribution to V2 channels is enabled.
        ...
        ES::Target: EntropySource,
    {
        ...
        if let Some(change_value) = change_value_opt {
            ...
            let change_output_fee = fee_for_weight(
                self.funding_feerate_sat_per_1000_weight,
    ...
    <SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
    ...
    fn get_closing_transaction_weight(
        &self,
        a_scriptpubkey: Option<&Script>,
        b_scriptpubkey: Option<&Script>,
    ) -> u64 {
    ...
    fn build_closing_transaction(
        &self,
        proposed_total_fee_satoshis: u64,
        skip_remote_output: bool,
        ...
        let value_to_each_rgb = self.context.rgb_context.as_ref().and_then(|rgb_context| {
            rgb_context.funding_ref.as_ref().map(|_| {
                (
                    rgb_context.contract_id,
                    rgb_context.to_self,
                    rgb_context.funding_rgb.saturating_sub(rgb_context.to_self),
                )
            })
        });
        let holder_rgb = value_to_each_rgb
            .as_ref()
            .map(|(_, holder_rgb, _)| *holder_rgb)
            .unwrap_or(0);
        let counterparty_rgb = value_to_each_rgb
            .as_ref()
            .map(|(_, _, counterparty_rgb)| counterparty_rgb)
            .copied()
            .unwrap_or(0);
        ...
        let counterparty_shutdown_script = self
            .context
            .counterparty_shutdown_scriptpubkey
            .clone()
            .unwrap();
        ...
        let closing_transaction = if value_to_each_rgb.is_some() {
            let funding_rgb_transfer = self
                .context
                .rgb_context
                .as_ref()
                .and_then(|rgb_context| rgb_context.funding_ref.clone());
            ClosingTransaction::new_with_rgb(
    ...
    pub fn claim_htlc_while_disconnected_dropping_mon_update_legacy<L: Deref>(
        &mut self,
        htlc_id_arg: u64,
        payment_preimage_arg: PaymentPreimage,
        logger: &L,
    ) where
        L::Target: Logger,
    {
        ...
        L::Target: Logger,
    {
        ...
        if !self.context.channel_state.can_generate_new_commitment() {
            ...
            self.context
                ...
                .push(HTLCUpdateAwaitingACK::ClaimHTLC {
        ...
        match self.get_update_fulfill_htlc(
            ...
            UpdateFulfillFetch::NewClaim {
                ...
                if release_cs_monitor && !update_blocked {
                    ...
                    monitor_update
                        .updates
                        .append(&mut additional_update.updates);
                } else {
    ...
    pub fn update_add_htlc<F: Deref>(
		...
		if let Some((contract_id, amount_rgb)) = msg.amount_rgb {
			let Some(rgb_context) = self.context.rgb_context.as_ref() else {
				return Err(ChannelError::close(
					"Remote sent RGB HTLC over a non-RGB channel".to_owned(),
				));
			};
			if contract_id != rgb_context.contract_id {
				return Err(ChannelError::close(
					"Remote sent RGB HTLC with a contract different from channel funding"
						.to_owned(),
				));
			}
			if amount_rgb == 0 {
				return Err(ChannelError::close(
					"Remote sent a 0-amount RGB HTLC".to_owned(),
				));
			}
			if rgb_context.funding_ref.is_none() {
				return Err(ChannelError::close(
					"Remote sent RGB HTLC before the funding RGB daemon reference was registered"
						.to_owned(),
				));
			}
		}
        ...
        L::Target: Logger,
    {
        ...
        let monitor_update = ChannelMonitorUpdate {
            ...
            updates: vec![ChannelMonitorUpdateStep::RenegotiatedFunding {
                channel_parameters: pending_splice_funding
                    .channel_transaction_parameters
                    .clone(),
                holder_commitment_tx,
        ...
        L::Target: Logger,
    {
        ...
        for funding in core::iter::once(&self.funding).chain(self.pending_funding().iter()) {
            let funding_txid = funding
                .get_funding_txid()
                .expect("Funding txid must be known for pending scope");
    ...
    fn commitment_signed_update_monitor<L: Deref>(
        &mut self,
        mut update: ChannelMonitorUpdateStep,
        logger: &L,
    ) -> Result<Option<ChannelMonitorUpdate>, ChannelError>
    where
        L::Target: Logger,
        ...
        L::Target: Logger,
    {
            ...
            || self.context.holding_cell_update_fee.is_some()
        {
            ...
            mem::swap(
                &mut htlc_updates,
                &mut self.context.holding_cell_htlc_updates,
            );
            ...
            for htlc_update in htlc_updates.drain(..) {
                ...
                let fail_htlc_res = match &htlc_update {
                    ...
                    &HTLCUpdateAwaitingACK::ClaimHTLC {
                        ...
                        monitor_update
                            .updates
                            .append(&mut additional_monitor_update.updates);
                        None
                    }
                    &HTLCUpdateAwaitingACK::FailHTLC {
                        htlc_id,
            ...
            let update_fee = self
                ...
                .take()
                .and_then(|feerate| self.send_update_fee(feerate, false, fee_estimator, logger));
        ...
        L::Target: Logger,
    {
        ...
        if !self.context.channel_state.is_awaiting_remote_revoke() {
            ...
            return Err(ChannelError::close(
                "Received an unexpected revoke_and_ack".to_owned(),
            ));
        }
        ...
        let mut value_to_self_rgb_diff: i64 = 0;

        {
            ...
            pending_inbound_htlcs.retain(|htlc| {
                if let &InboundHTLCState::LocalRemoved(ref reason) = &htlc.state {
                    log_trace!(
                        logger,
                        " ...removing inbound LocalRemoved {}",
                        &htlc.payment_hash
                    );
                    if let &InboundHTLCRemovalReason::Fulfill(_, _) = reason {
                        value_to_self_msat_diff += htlc.amount_msat as i64;
                        value_to_self_rgb_diff +=
                            htlc.amount_rgb.map(|(_, amount)| amount).unwrap_or(0) as i64;
                    }
        ...
        if let Some(rgb_context) = self.context.rgb_context.as_mut() {
            rgb_context.to_self = (rgb_context.to_self as i64 + value_to_self_rgb_diff) as u64;
        }
        ...
        macro_rules! return_with_htlcs_to_fail {
            ($htlcs_to_fail: expr) => {
                if !release_monitor {
                    self.context
                        .blocked_monitor_updates
                        .push(PendingChannelMonitorUpdate {
        ...
        self.context
            ...
            .append(&mut pending_update_adds);
        ...
        L::Target: Logger,
    {
        ...
        if let Some(pending_splice) = self.pending_splice.as_mut() {
                ...
                pending_splice.funding_negotiation.take()
            {
                ...
                let funding_txo = funding
                    .get_funding_txo()
                    .expect("funding outpoint should be set");
                ...
                if let Some(splice_txid) = splice_locked
                    .as_ref()
                    .map(|splice_locked| splice_locked.splice_txid)
                {
                    log_info!(
                        logger,
        ...
        L::Target: Logger,
    {
        ...
        debug_assert_eq!(
            ...
            shared_input_signature.is_some()
        );
        ...
        L::Target: Logger,
    {
            ...
            if let Some(signing_session) = self.context.interactive_tx_signing_session.as_mut() {
                if signing_session.has_received_tx_signatures() {
                    return Err(ChannelError::Ignore(
                        "Ignoring duplicate tx_signatures".to_owned(),
                    ));
                }
                ...
                return Err(ChannelError::Ignore(
                    "Ignoring unexpected tx_signatures".to_owned(),
                ));
            };
        ...
        let (holder_tx_signatures, funding_tx) = signing_session
            .received_tx_signatures(msg)
    ...
    fn monitor_updating_paused(
        &mut self,
        resend_raa: bool,
        resend_commitment: bool,
        resend_channel_ready: bool,
        ...
        self.context
            ...
            .append(&mut pending_forwards);
        self.context
            .monitor_pending_failures
            .append(&mut pending_fails);
        self.context
        ...
        CBP: Fn(u64) -> BlindedMessagePath,
    {
        ...
        self.holder_commitment_point
            .try_resolve_pending(signer, &self.context.secp_ctx, logger);
        ...
        F::Target: FeeEstimator,
    {
        ...
        let mut proposed_max_feerate = if self.funding.is_outbound() {
            normal_feerate
        } else {
            u32::max_value()
        };
    ...
    pub fn maybe_propose_closing_signed<F: Deref, L: Deref>(
        ...
        (
            ...
            Option<(Transaction, ShutdownResult)>,
        ),
        ChannelError,
    >
    where
    ...
    > {
        ...
        let mut not_broadcasted_initial_funding = matches!(
            self.context.channel_state,
            ChannelState::NegotiatingFunding(_)
        );
        if matches!(
        ...
        if not_broadcasted_initial_funding {
            ...
            return Err(ChannelError::close(
                "Shutdown before funding was broadcasted".to_owned(),
            ));
        }
        for htlc in self.context.pending_inbound_htlcs.iter() {
        ...
        self.context
            .holding_cell_htlc_updates
            .retain(|htlc_update| match htlc_update {
                &HTLCUpdateAwaitingACK::AddHTLC {
                    ref payment_hash,
                    ref source,
                    ..
    ...
    fn build_signed_closing_transaction(
        ...
        tx.input[0]
            .witness
            .push(self.funding.get_funding_redeemscript().into_bytes());
        tx
    }
        ...
        L::Target: Logger,
    {
        ...
        if sig.is_none() {
            log_trace!(
                logger,
                "Closing transaction signature unavailable, waiting on signer"
            );
        ...
        self.context.last_sent_closing_fee = Some((
            fee_satoshis,
            skip_remote_output,
            fee_range.clone(),
            sig.clone(),
        ...
        L::Target: Logger,
    {
        ...
        match self.context.secp_ctx.verify_ecdsa(
            ...
            Err(_e) => {
                ...
                closing_tx = self
                    .build_closing_transaction(msg.fee_satoshis, skip_remote_output)?
                    .0;
                let sighash = closing_tx
                    .trust()
                    .get_sighash_all(&funding_redeemscript, self.funding.get_value_satoshis());
        ...
        if let Some(msgs::ClosingSignedFeeRange {
            min_fee_satoshis,
            max_fee_satoshis,
        }) = msg.fee_range
        {
    ...
    pub fn get_inbound_scid(&self) -> Option<u64> {
        self.context
            ...
            .or(self.funding.get_short_channel_id())
    }
    ...
    fn check_funding_meets_minimum_depth(&self, funding: &FundingScope, height: u32) -> bool {
        self.context
            .check_funding_meets_minimum_depth(funding, height)
    }
    ...
    pub fn best_block_updated<NS: Deref, L: Deref>(
        &mut self,
        height: u32,
        highest_header_time: Option<u32>,
        chain_hash: ChainHash,
        node_signer: &NS,
        user_config: &UserConfig,
        logger: &L,
    ...
    fn maybe_get_my_current_funding_locked(&self) -> Option<msgs::FundingLocked> {
        self.pending_splice
            ...
            .or_else(|| {
                self.is_our_channel_ready()
                    .then(|| self.funding.get_funding_txid())
                    .flatten()
            })
            .map(|txid| {
        ...
        L::Target: Logger,
    {
        ...
        .map_err(|e| APIError::APIMisuseError {
            err: format!(
                "Channel {} cannot be {}; {}",
                self.context.channel_id(),
                if our_funding_contribution.is_positive() {
                    "spliced in"
                } else {
                    "spliced out"
                },
    ...
    pub fn validate_splice_init(
        ...
        if !self.context.channel_state.is_quiescent() {
            return Err(ChannelError::WarnAndDisconnect(
                "Quiescence needed to splice".to_owned(),
            ));
        }
    ...
    fn validate_splice_contributions(
        ...
        let (holder_balance_remaining, counterparty_balance_remaining) = self
            .get_holder_counterparty_balances_floor_incl_fee(&self.funding)
            .map_err(|e| {
                format!(
                    "Channel {} cannot be spliced; {}",
                    self.context.channel_id(),
                    e
                )
            })?;
        ...
        L::Target: Logger,
    {
        ...
        debug_assert!(interactive_tx_constructor
            .take_initiator_first_message()
            .is_none());
    ...
    fn validate_splice_ack(&self, msg: &msgs::SpliceAck) -> Result<FundingScope, ChannelError> {
        ...
        let (funding_negotiation_context, new_holder_funding_key) = match &pending_splice
            .funding_negotiation
        {
            Some(FundingNegotiation::AwaitingAck {
                context,
        ...
        L::Target: Logger,
    {
        ...
        let pending_splice = match self.pending_splice.as_mut() {
            ...
            None => {
                return Err(ChannelError::Ignore(
                    "Channel is not in pending splice".to_owned(),
                ));
            }
        };
        ...
        L::Target: Logger,
    {
        ...
        .map_err(|err| {
            debug_assert!(
                err.0.is_temporary(),
                "Queuing HTLC should return temporary error"
            );
            err
        })
    }
        ...
        L::Target: Logger,
    {
        ...
        if amount_msat == 0 {
            return Err((
                ...
                "Cannot send 0-msat HTLC".to_owned(),
            ));
        }
        ...
        if let Some((contract_id, rgb_amount)) = amount_rgb {
            let Some(rgb_context) = self.context.rgb_context.as_ref() else {
                return Err((
                    LocalHTLCFailureReason::HTLCMaximum,
                    "Cannot send RGB HTLC over a non-RGB channel".to_owned(),
                ));
            };
            if contract_id != rgb_context.contract_id {
                return Err((
                    LocalHTLCFailureReason::HTLCMaximum,
                    "Cannot send RGB HTLC with a contract different from channel funding"
                        .to_owned(),
                ));
            }
            if rgb_amount == 0 {
                return Err((
                    LocalHTLCFailureReason::ZeroAmount,
                    "Cannot send 0-amount RGB HTLC".to_owned(),
                ));
            }
            if rgb_context.funding_ref.is_none() {
                return Err((
                    LocalHTLCFailureReason::ChannelNotReady,
                    "Cannot send RGB HTLC before the funding RGB daemon reference is registered"
                        .to_owned(),
                ));
            }
            let rgb_htlc_feerate_per_kw = self
                .context
                .get_commitment_feerate(&self.funding, true)
                .max(self.context.get_commitment_feerate(&self.funding, false));
            let (htlc_success_tx_fee_sat, htlc_timeout_tx_fee_sat) =
                second_stage_tx_fees_sat(self.funding.get_channel_type(), rgb_htlc_feerate_per_kw);
            let rgb_htlc_dust_limit_satoshis = (self.context.holder_dust_limit_satoshis
                + htlc_timeout_tx_fee_sat)
                .max(self.context.counterparty_dust_limit_satoshis + htlc_success_tx_fee_sat);
            if amount_msat / 1000 < rgb_htlc_dust_limit_satoshis {
                return Err((
					LocalHTLCFailureReason::HTLCMinimum,
					format!(
						"Cannot send RGB HTLC below commitment dust: amount_msat={amount_msat} requires at least {} msat",
						rgb_htlc_dust_limit_satoshis * 1000
					),
				));
            }
            let pending_outbound_rgb = self
                .context
                .pending_outbound_htlcs
                .iter()
                .map(|htlc| htlc.amount_rgb.map(|(_, amount)| amount).unwrap_or(0))
                .sum::<u64>();
            let holding_outbound_rgb = self
                .context
                .holding_cell_htlc_updates
                .iter()
                .map(|update| match update {
                    HTLCUpdateAwaitingACK::AddHTLC { amount_rgb, .. } => {
                        amount_rgb.map(|(_, amount)| amount).unwrap_or(0)
                    }
                    _ => 0,
                })
                .sum::<u64>();
            let available_rgb = rgb_context
                .to_self
                .saturating_sub(pending_outbound_rgb)
                .saturating_sub(holding_outbound_rgb);
            if rgb_amount > available_rgb {
                return Err((
                    LocalHTLCFailureReason::HTLCMaximum,
                    format!(
						"Cannot send RGB HTLC with amount {rgb_amount}; available RGB is {available_rgb}"
					),
                ));
            }
        }
        ...
        if force_holding_cell {
            self.context
                .holding_cell_htlc_updates
                .push(HTLCUpdateAwaitingACK::AddHTLC {
                    amount_msat,
                    amount_rgb,
                    payment_hash,
                    cltv_expiry,
    ...
    fn build_commitment_no_status_check<L: Deref>(&mut self, logger: &L) -> ChannelMonitorUpdate where L::Target: Logger {
		...
		let update = if self.pending_funding().is_empty() {
			...
			if self.context.rgb_context.is_some() {
                ...
                historical_scids
                    ...
                    .zip(
                        historical_scids
                            .iter()
                            .skip(1)
                            .chain(core::iter::once(&current_scid)),
                    )
                    .filter(|(_, next_scid)| {
    ...
    SP::Target: SignerProvider,
{
	      ...
	      L::Target: Logger,
	{
		Self::new_with_rgb(
			...
			channel_value_satoshis, push_msat, None, user_id, config, current_chain_height,
			outbound_scid_alias, temporary_channel_id, logger
		)
	}
    ...
    pub fn new_with_rgb<ES: Deref, F: Deref, L: Deref>(
		fee_estimator: &LowerBoundedFeeEstimator<F>, entropy_source: &ES, signer_provider: &SP, counterparty_node_id: PublicKey, their_features: &InitFeatures,
		channel_value_satoshis: u64, push_msat: u64, funding_rgb: Option<(ContractId, u64)>,
		user_id: u128, config: &UserConfig, current_chain_height: u32,
		outbound_scid_alias: u64, temporary_channel_id: Option<ChannelId>, logger: L
	) -> Result<OutboundV1Channel<SP>, APIError>
	where ES::Target: EntropySource,
	      F::Target: FeeEstimator,
    ...
    SP::Target: SignerProvider,
{
    fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
        ...
        let pending_splice = self
            .pending_splice
            .as_ref()
            .filter(|_| !self.should_reset_pending_splice_state(false));
        ...
        write_tlv_fields!(writer, {
            ...
            (26233_1, self.context.rgb_context, option),
        });
    ...
    SP::Target: SignerProvider,
{
    fn read<R: io::Read>(
        reader: &mut R,
        args: (&'a ES, &'b SP, &'c ChannelTypeFeatures),
        ...
        let mut rgb_context: Option<RgbContext> = None;
        ...
        let holder_commitment_point = {
            ...
            match (
                holder_commitment_point_next_opt,
                holder_commitment_point_pending_next_opt,
            ) {
    ...
    impl SignerProvider for Keys {
        ...
        fn get_shutdown_scriptpubkey(&self) -> Result<ShutdownScript, ()> {
            ...
            let channel_close_key =
                SecretKey::from_slice(&<Vec<u8>>::from_hex(hex).unwrap()[..]).unwrap();
            Ok(ShutdownScript::new_p2wpkh_from_pubkey(
    ...
    fn upfront_shutdown_script_incompatibility() {
        ...
        match res {
            Err(APIError::IncompatibleShutdownScript { script }) => {
                assert_eq!(
                    script.into_inner(),
                    non_v0_segwit_shutdown_script.into_inner()
                );
            }
    ...
    fn blinding_point_skimmed_fee_malformed_ser() {
        ...
        let decoded_chan =
            FundedChannel::read(&mut reader, (&&keys_provider, &&keys_provider, &features))
                .unwrap();
        assert_eq!(
            decoded_chan.context.pending_outbound_htlcs,
            pending_outbound_htlcs
        );
        assert_eq!(
            decoded_chan.context.holding_cell_htlc_updates,
            holding_cell_htlc_updates
        );
    }
