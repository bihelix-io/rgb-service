...
trait OnionPayload<'a, 'b> {
    ...
    fn new_receive(
        recipient_onion: &'a RecipientOnionFields,
...
impl<'a, 'b> OnionPayload<'a, 'b> for msgs::OutboundTrampolinePayload<'a> {
    ...
    fn new_blinded_receive(
        ...
        intro_node_blinding_point: Option<PublicKey>,
        keysend_preimage: Option<PaymentPreimage>,
        _invoice_request: Option<&'a InvoiceRequest>,
        custom_tlvs: &'a Vec<(u64, Vec<u8>)>,
    ) -> Self {
        Self::BlindedReceive {
            sender_intended_htlc_amt_msat,
    ...
    fn new_trampoline_entry(
        _total_msat: u64,
        _amt_to_forward: u64,
        _outgoing_cltv_value: u32,
        _recipient_onion: &'a RecipientOnionFields,
        _packet: msgs::TrampolineOnionPacket,
    ) -> Result<Self::ReceiveType, APIError> {
        Err(APIError::InvalidRoute {
...
fn construct_onion_keys_generic<'a, T, H>(
    ...
    hops: &'a [H],
    blinded_tail: Option<&'a BlindedTail>,
    ...
    H: HopInfo,
{
    ...
    unblinded_hops
        .chain(blinded_pubkeys)
        .enumerate()
        .map(move |(idx, (pubkey, route_hop_opt))| {
            ...
            blinded_priv = blinded_priv
                ...
                .expect(
                    "Blinding are never invalid as we picked the starting private key randomly",
                );
...
fn build_onion_payloads_callback<'a, 'b, H, B, F, OP>(
    hops: H,
    mut blinded_tail: Option<BlindedTailDetails<'a, B>>,
    total_msat: u64,
    recipient_onion: &'a RecipientOnionFields,
    starting_htlc_offset: u32,
    keysend_preimage: &Option<PaymentPreimage>,
    ...
    for (idx, hop) in hops.rev().enumerate() {
        ...
        if cur_value_msat >= 21000000 * 100000000 * 1000 {
            return Err(APIError::InvalidRoute {
                err: "Channel fees overflowed?".to_owned(),
            });
        }
        cur_cltv = cur_cltv.saturating_add(hop.cltv_expiry_delta() as u32);
        if cur_cltv >= 500000000 {
            return Err(APIError::InvalidRoute {
                err: "Channel CLTV overflowed?".to_owned(),
            });
        }
...
pub(crate) fn set_max_path_length(
    route_params: &mut RouteParameters,
    ...
    let cltv_expiry_delta = core::cmp::min(
...
pub(super) fn construct_onion_packet(
    payloads: Vec<msgs::OutboundOnionPayload>,
    ...
    debug_assert_eq!(
        payloads.len(),
        onion_keys.len(),
        "Payloads and keys must have equal lengths"
    );
...
pub(super) fn construct_trampoline_onion_packet(
    payloads: Vec<msgs::OutboundTrampolinePayload>,
    onion_keys: Vec<OnionKeys>,
    prng_seed: [u8; 32],
    associated_data: &PaymentHash,
    length: Option<u16>,
    ...
    let packet_length = length
        .map(|l| usize::from(l))
        .unwrap_or(minimum_packet_length);
    debug_assert!(
...
pub(crate) fn payloads_serialized_length<HD: Writeable>(payloads: &Vec<HD>) -> usize {
    payloads
        .iter()
        .map(|p| p.serialized_length() + 32 /* HMAC */)
        .sum()
}
...
fn construct_onion_packet_with_init_noise<HD: Writeable, P: Packet>(
    ...
    let filler = {
        ...
        for (i, (payload, keys)) in payloads.iter().zip(onion_keys.iter()).enumerate() {
            ...
            payload
                .write(&mut payload_len)
                .expect("Failed to calculate length");
    ...
    Ok(P::new(
        onion_keys.first().unwrap().ephemeral_pubkey,
        packet_data,
        hmac_res,
    ))
}
...
fn build_unencrypted_failure_packet(
    ...
    let mut packet = OnionErrorPacket {
        data: writer.0,
...
fn update_attribution_data(
    onion_error_packet: &mut OnionErrorPacket,
    shared_secret: &[u8],
    hold_time: u32,
) {
    ...
    // failures.
    let attribution_data = onion_error_packet
        .attribution_data
        .get_or_insert(AttributionData::new());
...
fn process_onion_failure_inner<T: secp256k1::Signing, L: Deref>(
    ...
    path: &Path,
    outer_session_priv: &SecretKey,
    inner_session_priv: Option<&SecretKey>,
    ...
    L::Target: Logger,
{
    ...
    let nontrampoline_bt = if path.has_trampoline_hops() {
        None
    } else {
    ...
    let nontrampolines =
        construct_onion_keys_generic(secp_ctx, &path.hops, nontrampoline_bt, outer_session_priv)
            .map(|(shared_secret, _, _, route_hop_option, _)| {
                (
                    route_hop_option.map(|rh| ErrorHop::RouteHop(rh)),
                    shared_secret,
                )
            });
    ...
    let trampolines = if path.has_trampoline_hops() {
        ...
        Some(
            construct_onion_keys_generic(secp_ctx, hops, blinded_tail, inner_session_priv).map(
    ...
    let mut iter = nontrampolines
        .chain(trampolines.into_iter().flatten())
        .enumerate()
        .peekable();
    while let Some((route_hop_idx, (route_hop_option, shared_secret))) = iter.next() {
        ...
        let error_code_slice = match err_packet.failuremsg.get(0..2) {
            ...
            None => {
                ...
                log_warn!(
                    logger,
                    "Missing error code in failure from {}",
                    route_hop.pubkey()
                );
...
impl From<&HTLCFailReason> for HTLCHandlingFailureReason {
    fn from(value: &HTLCFailReason) -> Self {
        match value.0 {
            ...
            HTLCFailReasonRepr::Reason { failure_reason, .. } => HTLCHandlingFailureReason::Local {
    ...
    pub fn set_hold_time(&mut self, hold_time: u32) {
        match self.0 {
            HTLCFailReasonRepr::LightningError {
                hold_time: ref mut current_hold_time,
                ..
            } => {
...
impl HTLCFailReason {
    pub(super) fn reason(failure_reason: LocalHTLCFailureReason, data: Vec<u8>) -> Self {
        ...
        Self(HTLCFailReasonRepr::Reason {
            data,
            failure_reason,
        })
    }
    ...
    pub(super) fn get_encrypted_failure_packet(
        ...
        secondary_shared_secret: &Option<[u8; 32]>,
...
impl Hop {
    ...
    pub(crate) fn shared_secret(&self) -> &SharedSecret {
        match self {
            ...
            Hop::TrampolineForward {
                outer_shared_secret,
                ..
            } => outer_shared_secret,
            Hop::TrampolineBlindedForward {
                outer_shared_secret,
                ..
            } => outer_shared_secret,
            Hop::Receive { shared_secret, .. } => shared_secret,
            Hop::BlindedReceive { shared_secret, .. } => shared_secret,
            Hop::TrampolineReceive {
                outer_shared_secret,
                ..
            } => outer_shared_secret,
            Hop::TrampolineBlindedReceive {
                outer_shared_secret,
                ..
            } => outer_shared_secret,
        }
    ...
    NS::Target: NodeSigner,
{
    let blinded_node_id_tweak = blinding_point.map(|bp| {
        let blinded_tlvs_ss = node_signer
            .ecdh(recipient, &bp, None)
            .unwrap()
            .secret_bytes();
...
fn decode_next_hop<T, R: ReadableArgs<T>, N: NextPacketBytes>(
    ...
    let mut chacha_stream = ChaChaReader {
        chacha: &mut chacha,
        read: Cursor::new(&hop_data[..]),
    };
    match R::read(&mut chacha_stream, read_args) {
        ...
        Ok(msg) => {
            ...
            if hmac == [0; 32] {
                ...
                chacha_stream
                    .read_exact(&mut new_packet_bytes.as_mut()[..read_pos])
                    .unwrap();
                ...
                chacha_stream
                    .chacha
                    .process_in_place(&mut new_packet_bytes.as_mut()[read_pos..]);
...
impl AttributionData {
    ...
    fn shift_right(&mut self) {
        ...
        self.hold_times
            .copy_within(..(MAX_HOPS - 1) * HOLD_TIME_LEN, HOLD_TIME_LEN);
...
fn process_failure_packet(
    onion_error: &mut OnionErrorPacket,
    shared_secret: &[u8],
    hold_time: u32,
) {
...
mod tests {
    ...
    fn test_attributable_failure_packet_onion_mutations() {
        ...
        for mutating_node in 0..5 {
                ...
                .chain(iter::once(MutationType::DropAttributionData))
            {
                ...
                assert_eq!(
                    decrypted_failure.hold_times,
                    [5, 4, 3, 2, 1][..mutating_node]
                );
            }
        }
    }
    ...
    fn test_long_route_attributable_failure() {
        ...
        for failure_pos in 0..LEGACY_MAX_HOPS {
            ...
            let expected_failed_chan = if failure_pos < MAX_HOPS {
                Some(failure_pos as u64)
            } else {
                None
            };
            assert_eq!(
                decrypted_failure.attribution_failed_channel,
                expected_failed_chan
            );
        }
    }
