...
fn do_test_counterparty_no_reserve(send_from_initiator: bool) {
    ...
    let temp_channel_id = nodes[0]
        .node
        .create_channel(node_b_id, 100_000, push, 42, None, None)
        .unwrap();
    ...
    {
        let sender_node = if send_from_initiator {
            &nodes[1]
        } else {
            &nodes[0]
        };
        let counterparty_node = if send_from_initiator {
            &nodes[0]
        } else {
            &nodes[1]
        };
...
pub fn test_channel_reserve_holding_cell_htlcs() {
    ...
    // attempt to send amt_msat > their_max_htlc_value_in_flight_msat
    {
        ...
        assert!(route.paths[0]
            .hops
            .iter()
            .rev()
            .skip(1)
            .all(|h| h.fee_msat == feemsat));
    ...
    loop {
        ...
        assert_eq!(
            stat01_.value_to_self_msat,
            stat01.value_to_self_msat - amt_msat
        );
        assert_eq!(
            stat11_.value_to_self_msat,
            stat11.value_to_self_msat + amt_msat
        );
        assert_eq!(
            stat12_.value_to_self_msat,
            stat12.value_to_self_msat - (amt_msat - feemsat)
        );
        assert_eq!(
            stat22_.value_to_self_msat,
            stat22.value_to_self_msat + (amt_msat - feemsat)
        );
        stat01 = stat01_;
        stat11 = stat11_;
        stat12 = stat12_;
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &payment_event_1.msgs[0]);
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_21, our_payment_hash_21, onion, id)
        .unwrap();
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_22, our_payment_hash_22, onion, id)
        .unwrap();
    check_added_monitors(&nodes[0], 0);
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &payment_event_1.commitment_msg);
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &as_commitment_signed);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &bs_revoke_and_ack);
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &payment_event_11.msgs[0]);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &commitment_update_2.update_add_htlcs[0]);
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &commitment_update_2.update_add_htlcs[1]);
    commitment_signed_dance!(
        nodes[1],
        nodes[0],
        &commitment_update_2.commitment_signed,
        false
    );
    expect_and_process_pending_htlcs(&nodes[1], false);
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &payment_event_3.msgs[0]);
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &payment_event_3.msgs[1]);
        ...
        Event::PaymentClaimable {
            ...
            assert_eq!(
                *receiving_channel_ids,
                vec![(chan_2.2, Some(chan_2_user_id))]
            );
            match &purpose {
    ...
    assert_eq!(
        stat0.value_to_self_msat,
        stat0.channel_reserve_msat + commit_tx_fee_1_htlc
    );
...
pub fn channel_reserve_in_flight_removes() {
    ...
    let send_1 = {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route, payment_hash_3, onion, id)
            .unwrap();
        check_added_monitors(&nodes[0], 1);
        let mut events = nodes[0].node.get_and_clear_pending_msg_events();
        assert_eq!(events.len(), 1);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_removes.update_fulfill_htlcs.remove(0));
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_removes.commitment_signed);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &send_1.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &send_1.commitment_msg);
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_cs.commitment_signed);
    check_added_monitors(&nodes[1], 1);
    let bs_raa = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, node_a_id);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_cs.update_fulfill_htlcs.remove(0));
    nodes[0]
        .node
    ...
    let send_2 = {
        ...
        nodes[1]
            .node
            .send_payment_with_route(route, payment_hash_4, onion, id)
            .unwrap();
        check_added_monitors(&nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &send_2.msgs[0]);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &send_2.commitment_msg);
    check_added_monitors(&nodes[0], 1);
    let as_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, node_b_id);
...
pub fn holding_cell_htlc_counting() {
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &initial_payment_event.msgs[0]);
    nodes[2]
        .node
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &bs_commitment_signed);
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &as_updates.commitment_signed);
    check_added_monitors(&nodes[2], 1);
    ...
    nodes[2]
        .node
        .handle_revoke_and_ack(node_b_id, &as_final_raa);
    check_added_monitors(&nodes[2], 1);
...
pub fn do_test_fee_spike_buffer(cfg: Option<UserConfig>, htlc_fails: bool) {
    ...
    nodes[1]
        .node
        .handle_commitment_signed(node_a_id, &commit_signed_msg);
...
pub fn test_update_add_htlc_bolt2_receiver_check_cltv_expiry() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, our_payment_hash, reason, id)
        .unwrap();
    check_added_monitors(&nodes[0], 1);
    let mut updates = get_htlc_update_msgs!(nodes[0], node_b_id);
    ...
    assert_eq!(
        err_msg.data,
        "Remote provided CLTV expiry in seconds instead of block height"
    );
    check_added_monitors(&nodes[1], 1);
    let reason = ClosureReason::ProcessingError { err: err_msg.data };
    check_closed_event!(nodes[1], 1, reason, [node_a_id], 100000);
...
pub fn test_update_add_htlc_bolt2_receiver_check_repeated_id_ignore() {
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &updates.commitment_signed);
    check_added_monitors(&nodes[1], 1);
...
pub fn test_update_fulfill_htlc_bolt2_update_fulfill_htlc_before_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, update_msg);
...
pub fn test_update_fulfill_htlc_bolt2_update_fail_htlc_before_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &update_msg);
...
pub fn test_update_fulfill_htlc_bolt2_update_fail_malformed_htlc_before_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fail_malformed_htlc(node_b_id, &update_msg);
...
pub fn test_update_fulfill_htlc_bolt2_incorrect_htlc_id() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, update_fulfill_msg);

    assert!(nodes[0].node.list_channels().is_empty());
    let err_msg = check_closed_broadcast!(nodes[0], true).unwrap();
    assert_eq!(
        err_msg.data,
        "Remote tried to fulfill/fail an HTLC we couldn't find"
    );
    check_added_monitors(&nodes[0], 1);
    let reason = ClosureReason::ProcessingError { err: err_msg.data };
    check_closed_event!(nodes[0], 1, reason, [node_b_id], 100000);
...
pub fn test_update_fulfill_htlc_bolt2_missing_badonion_bit_for_malformed_htlc_message() {
    ...
    assert_eq!(
        err_msg.data,
        "Got update_fail_malformed_htlc with BADONION not set"
    );
    check_added_monitors(&nodes[0], 1);
    let reason = ClosureReason::ProcessingError { err: err_msg.data };
...
pub fn do_test_dust_limit_fee_accounting(can_afford: bool) {
    ...
    let node_chanmgrs = create_node_chanmgrs(
        2,
        &node_cfgs,
        &[Some(default_config.clone()), Some(default_config)],
    );
    ...
    if !can_afford {
        ...
        nodes[1]
            .logger
            .assert_log_contains("lightning::ln::channelmanager", &err, 3);
        let events = nodes[1].node.get_and_clear_pending_msg_events();
        assert_eq!(events.len(), 2);
