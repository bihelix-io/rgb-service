...
pub fn duplicate_htlc_test() {
    ...
    *nodes[0].network_payment_count.borrow_mut() -= 1;
    assert_eq!(
        route_payment(&nodes[1], &[&nodes[3]], 1000000).0,
        payment_preimage
    );

    *nodes[0].network_payment_count.borrow_mut() -= 1;
    assert_eq!(
        route_payment(&nodes[2], &[&nodes[3], &nodes[5]], 1000000).0,
        payment_preimage
    );
...
pub fn test_duplicate_htlc_different_direction_onchain() {
    ...
    assert_eq!(preimage_tx.input.len(), 1);
    assert_eq!(
        preimage_tx.input[0].witness.last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    ); // HTLC 1 <--> 0, preimage tx
    assert_eq!(
        remote_txn[0].output[preimage_tx.input[0].previous_output.vout as usize]
            .value
            .to_sat(),
        payment_value_sats
    );
    ...
    assert_eq!(
        timeout_tx.input[0].witness.last().unwrap().len(),
        ACCEPTED_HTLC_SCRIPT_WEIGHT
    ); // HTLC 0 <--> 1, timeout tx
    check_spends!(timeout_tx, remote_txn[0]);
    assert_eq!(
        remote_txn[0].output[timeout_tx.input[0].previous_output.vout as usize]
            .value
            .to_sat(),
        900
    );
...
pub fn test_inbound_outbound_capacity_is_not_zero() {
    ...
    assert_eq!(
        ...
        95000000 - reserve * 1000
    );
    assert_eq!(
        channels1[0].outbound_capacity_msat,
        95000000 - reserve * 1000
    );

    assert_eq!(
        channels0[0].outbound_capacity_msat,
        100000 * 1000 - 95000000 - reserve * 1000
    );
    assert_eq!(
...
fn do_test_fail_back_before_backwards_timeout(post_fail_back_action: PostFailBackAction) {
    ...
    connect_blocks(
        &nodes[0],
        2 * CHAN_CONFIRM_DEPTH + 1 - nodes[0].best_block_info().1,
    );
    connect_blocks(
        &nodes[1],
        2 * CHAN_CONFIRM_DEPTH + 1 - nodes[1].best_block_info().1,
    );
    connect_blocks(
        &nodes[2],
        2 * CHAN_CONFIRM_DEPTH + 1 - nodes[2].best_block_info().1,
    );
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &update_fail_htlcs[0]);
    ...
    match post_fail_back_action {
        ...
        PostFailBackAction::FailOffChain => {
            ...
            nodes[1]
                .node
                .handle_update_fail_htlc(node_c_id, &update_fail);
        ...
        PostFailBackAction::ClaimOffChain => {
            ...
            nodes[1]
                .node
                .handle_update_fulfill_htlc(node_c_id, update_fulfill);
            let err_msg = get_err_msg(&nodes[1], &node_c_id);
            assert_eq!(err_msg.channel_id, chan_2.2);
            assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
        }
...
pub fn channel_monitor_network_test() {
    ...
    connect_blocks(
        &nodes[0],
        4 * CHAN_CONFIRM_DEPTH + 1 - nodes[0].best_block_info().1,
    );
    connect_blocks(
        &nodes[1],
        4 * CHAN_CONFIRM_DEPTH + 1 - nodes[1].best_block_info().1,
    );
    connect_blocks(
        &nodes[2],
        4 * CHAN_CONFIRM_DEPTH + 1 - nodes[2].best_block_info().1,
    );
    connect_blocks(
        &nodes[3],
        4 * CHAN_CONFIRM_DEPTH + 1 - nodes[3].best_block_info().1,
    );
    connect_blocks(
        &nodes[4],
        4 * CHAN_CONFIRM_DEPTH + 1 - nodes[4].best_block_info().1,
    );
    ...
    let node_b_reason = ClosureReason::HolderForceClosed {
        broadcasted_latest_txn: Some(true),
        message,
    };
    check_closed_event!(nodes[1], 1, node_b_reason, [node_c_id], 100000);
    ...
    let node_c_reason = ClosureReason::HolderForceClosed {
        broadcasted_latest_txn: Some(true),
        message,
    };
    check_closed_event!(nodes[2], 1, node_c_reason, [node_d_id], 100000);
    ...
    let (close_chan_update_1, close_chan_update_2) = {
        ...
        let reason = ClosureReason::HTLCsTimedOut {
            payment_hash: Some(payment_hash_2),
        };
    ...
    nodes[3]
        .gossip_sync
        .handle_channel_update(Some(node_id_4), &close_chan_update_2)
        .unwrap();
    nodes[4]
        .gossip_sync
        .handle_channel_update(Some(node_id_3), &close_chan_update_1)
        .unwrap();
    ...
    assert_eq!(
        nodes[3]
            .chain_monitor
            .chain_monitor
            .watch_channel(chan_3.2, chan_3_mon),
...
pub fn test_justice_tx_htlc_timeout() {
    ...
    assert_eq!(
        revoked_local_txn[0].input[0].previous_output.txid,
        chan_5.3.compute_txid()
    );
...
fn do_test_forming_justice_tx_from_monitor_updates(broadcast_initial_commitment: bool) {
    ...
    let chanmon_cfgs = create_chanmon_cfgs(2);
    let destination_script0 = chanmon_cfgs[0]
        .keys_manager
        .get_destination_script([0; 32])
        .unwrap();
    let destination_script1 = chanmon_cfgs[1]
        .keys_manager
        .get_destination_script([0; 32])
        .unwrap();
    let persisters = [
    ...
    let justice_tx = persisters[1]
        .justice_tx(channel_id, &revoked_commitment_tx.compute_txid())
        .unwrap();
    ...
    let total_claimable_balance =
        monitor
            ...
            .iter()
            .fold(0, |sum, balance| match balance {
                channelmonitor::Balance::ClaimableAwaitingConfirmations {
                    amount_satoshis, ..
                } => sum + amount_satoshis,
...
pub fn do_test_multiple_package_conflicts(p2a_anchor: bool) {
    ...
    user_cfg
        .channel_handshake_config
        .negotiate_anchor_zero_fee_commitments = p2a_anchor;
    ...
    let node_max_height = nodes
        .iter()
        .map(|node| node.best_block_info().1)
        .max()
        .unwrap();
    for node in &nodes {
    ...
    assert_eq!(
        conflict_tx.input[0].previous_output.txid,
        node2_commit_tx.compute_txid()
    );
    assert_eq!(
        conflict_tx.input[1].previous_output.txid,
        node2_commit_tx.compute_txid()
    );
    assert_eq!(
        conflict_tx.input[2].previous_output.txid,
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, updates.update_fulfill_htlcs.remove(0));
    nodes[0]
        .node
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &commit_signed);
...
pub fn test_htlc_on_chain_success() {
    ...
    let node_max_height = nodes
        .iter()
        .map(|node| node.blocks.lock().unwrap().len())
        .max()
        .unwrap() as u32;
    ...
    let node_txn = nodes[2]
        .tx_broadcaster
        .txn_broadcasted
        .lock()
        .unwrap()
        .clone(); // ChannelMonitor: 2 (2 * HTLC-Success tx)
    ...
    let txn = vec![
        commitment_tx[0].clone(),
        node_txn[0].clone(),
        node_txn[1].clone(),
    ];
    connect_block(
        &nodes[1],
        &create_dummy_block(nodes[1].best_block_hash(), 42, txn),
    );
    ...
    assert_eq!(
        commitment_spend.input[0].witness.last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    );
    assert_eq!(
        commitment_spend.input[1].witness.last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    );
    assert_eq!(
        commitment_spend.lock_time.to_consensus_u32(),
    ...
    connect_block(
        &nodes[0],
        &create_dummy_block(nodes[0].best_block_hash(), 42, txn),
    );
...
fn do_test_htlc_on_chain_timeout(connect_style: ConnectStyle) {
    ...
    *nodes[2].connect_style.borrow_mut() = connect_style;
    ...
    let timeout_tx = {
        ...
        assert_eq!(txn.len(), 1);
        txn.iter()
            .for_each(|tx| check_spends!(tx, commitment_tx[0]));
        assert_eq!(
...
fn do_test_commitment_revoked_fail_backward_exhaustive(
    ...
    let revoked_local_txn = get_local_commitment_txn!(nodes[2], chan_2.2);
    assert_eq!(
        revoked_local_txn[0].output.len(),
        if no_to_remote { 1 } else { 2 }
    );
    // Revoke the old state
    claim_payment(&nodes[0], &[&nodes[1], &nodes[2]], payment_preimage);
    ...
    expect_and_process_pending_htlcs_and_htlc_handling_failed(
        &nodes[2],
        &[HTLCHandlingFailureType::Receive {
            payment_hash: first_payment_hash,
        }],
    );
    check_added_monitors(&nodes[2], 1);
    let updates = get_htlc_update_msgs!(nodes[2], node_b_id);
    ...
    expect_and_process_pending_htlcs_and_htlc_handling_failed(
        &nodes[2],
        &[HTLCHandlingFailureType::Receive {
            payment_hash: third_payment_hash,
        }],
    );
    check_added_monitors(&nodes[2], 1);
    let updates = get_htlc_update_msgs!(nodes[2], node_b_id);
    ...
    nodes[1]
        .node
        .send_payment_with_route(route, fourth_payment_hash, onion, id)
        .unwrap();
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    assert!(nodes[1].node.get_and_clear_pending_events().is_empty());
    ...
    match nodes_0_event {
        MessageSendEvent::UpdateHTLCs {
            ...
            nodes[0]
                .node
                .handle_update_fail_htlc(node_b_id, &update_fail_htlcs[1]);
            nodes[0]
                .node
                .handle_update_fail_htlc(node_b_id, &update_fail_htlcs[2]);
...
pub fn fail_backward_pending_htlc_upon_channel_failure() {
    ...
    // Alice -> Bob: Route a payment but without Bob sending revoke_and_ack.
    {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route, payment_hash, onion, id)
            .unwrap();
        check_added_monitors(&nodes[0], 1);
        ...
        get_route_and_payment_hash!(nodes[0], nodes[1], 50_000);
    {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route, failed_payment_hash, onion, id)
            .unwrap();
    ...
    // Alice <- Bob: Send a malformed update_add_htlc so Alice fails the channel.
    {
        ...
        nodes[0]
            .node
            .handle_update_add_htlc(node_b_id, &update_add_htlc);
    }
    let events = nodes[0].node.get_and_clear_pending_events();
    assert_eq!(events.len(), 3);
    ...
    match events[2] {
        Event::ChannelClosed {
            reason: ClosureReason::ProcessingError { ref err },
            ..
        } => {
...
pub fn test_force_close_fail_back() {
    ...
    let mut payment_event = {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route, our_payment_hash, onion, id)
            .unwrap();
        check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &payment_event.msgs[0]);
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &payment_event.msgs[0]);
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &payment_event.commitment_msg);
    check_added_monitors(&nodes[2], 1);
    ...
    assert_eq!(
        node_txn.len(),
        if nodes[2].connect_style.borrow().updates_best_block_first() {
            2
        } else {
            1
        }
    ...
    assert_eq!(
        htlc_tx.input[0].previous_output.txid,
...
pub fn test_dup_events_on_peer_disconnect() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, claim_msgs.update_fulfill_htlcs.remove(0));
...
pub fn test_peer_disconnected_before_funding_broadcasted() {
    ...
    let expected_temporary_channel_id = nodes[0]
        .node
        .create_channel(node_b_id, 1_000_000, 500_000_000, 42, None, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .handle_accept_channel(node_b_id, &accept_channel);
...
fn do_test_drop_messages_peer_disconnect(messages_delivered: u8, simulate_broken_lnd: bool) {
    ...
    let payment_event = {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route, payment_hash_1, onion, id)
            .unwrap();
        check_added_monitors(&nodes[0], 1);
    ...
    if messages_delivered < 3 {
        if simulate_broken_lnd {
            ...
            nodes[1]
                .node
                .handle_channel_ready(node_a_id, &as_channel_ready.as_ref().unwrap().0);
        }
        ...
        Event::PaymentClaimable {
            ...
            assert_eq!(
                *receiving_channel_ids,
                vec![(channel_id, Some(user_channel_id))]
            );
            match &purpose {
    ...
    if messages_delivered >= 1 {
        nodes[0]
            .node
            .handle_update_fulfill_htlc(node_b_id, update_fulfill_htlc);
...
pub fn test_channel_ready_without_best_block_updated() {
    ...
    *nodes[0].connect_style.borrow_mut() = ConnectStyle::BestBlockFirstSkippingBlocks;
    ...
    nodes[0]
        .node
        .transactions_confirmed(&conf_block_header, &conf_txn[..], conf_height);
    ...
    nodes[1]
        .node
        .handle_channel_ready(node_a_id, &as_channel_ready);
}
...
pub fn test_channel_monitor_skipping_block_when_channel_manager_is_lagging() {
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .block_connected(&block_1, height_1);
...
pub fn test_drop_messages_peer_disconnect_dual_htlc() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, payment_hash_2, onion, id)
        .unwrap();
    check_added_monitors(&nodes[0], 1);
    ...
    match events_2.remove(0) {
        MessageSendEvent::UpdateHTLCs {
            ...
            nodes[0]
                .node
                .handle_update_fulfill_htlc(node_b_id, update_fulfill_htlcs.remove(0));
    ...
    nodes[0]
        .node
        .peer_connected(node_b_id, &init_msg, true)
        .unwrap();
    let reestablish_1 = get_chan_reestablish_msgs!(nodes[0], nodes[1]);
    assert_eq!(reestablish_1.len(), 1);
    nodes[1]
        .node
        .peer_connected(node_a_id, &init_msg, false)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, as_resp.1.as_ref().unwrap());
    ...
    assert!(bs_second_commitment_signed
        .update_fail_malformed_htlcs
        .is_empty());
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_second_revoke_and_ack);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors(&nodes[0], 1);
...
fn do_test_htlc_timeout(send_partial_mpp: bool) {
    ...
    commitment_signed_dance!(
        nodes[0],
        nodes[1],
        htlc_timeout_updates.commitment_signed,
        false
    );
...
fn do_test_holding_cell_htlc_add_timeouts(forwarded_htlc: bool) {
    ...
    nodes[1]
        .node
        .send_payment_with_route(route, first_payment_hash, onion, id)
        .unwrap();
    ...
    sending_node
        .node
        .send_payment_with_route(route, second_payment_hash, onion, id)
        .unwrap();
...
pub fn test_static_spendable_outputs_preimage_tx() {
    ...
    assert_eq!(
        commitment_tx[0].input[0].previous_output.txid,
        chan_1.3.compute_txid()
    );
    ...
    let node_txn = nodes[1]
        ...
        .lock()
        .unwrap()
        .clone(); // ChannelMonitor: preimage tx
    assert_eq!(node_txn.len(), 1);
    check_spends!(node_txn[0], commitment_tx[0]);
    assert_eq!(
...
pub fn test_static_spendable_outputs_justice_tx_revoked_htlc_success_tx() {
    ...
    assert_eq!(
        revoked_local_txn[0].output[unspent_local_txn_output]
            .script_pubkey
            .len(),
        2 + 20
    ); // P2WPKH
    ...
    assert_ne!(
        spend_txn[0].input[0].previous_output,
        revoked_htlc_txn[0].input[0].previous_output
    );
...
pub fn test_onchain_to_onchain_claim() {
    ...
    let c_txn = nodes[2]
        ...
        .lock()
        .unwrap()
        .clone(); // ChannelMonitor: 1 (HTLC-Success tx)
    ...
    assert_eq!(
        b_txn[0].input[0].witness.clone().last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    );
    assert!(b_txn[0].output[0].script_pubkey.is_p2wpkh()); // direct payment
    assert_eq!(
        b_txn[0].lock_time.to_consensus_u32(),
        nodes[1].best_block_info().1
    ); // Success tx
...
pub fn test_duplicate_payment_hash_one_failure_one_success() {
    ...
    connect_blocks(
        &nodes[0],
        node_max_height * 2 - nodes[0].best_block_info().1,
    );
    connect_blocks(
        &nodes[1],
        node_max_height * 2 - nodes[1].best_block_info().1,
    );
    connect_blocks(
        &nodes[2],
        node_max_height * 2 - nodes[2].best_block_info().1,
    );
    connect_blocks(
        &nodes[3],
        node_max_height * 2 - nodes[3].best_block_info().1,
    );
    connect_blocks(
        &nodes[4],
        node_max_height * 2 - nodes[4].best_block_info().1,
    );
    ...
    let payment_secret = nodes[4]
        .node
        .create_inbound_payment_for_hash(dup_payment_hash, None, 7200, None)
        .unwrap();
    ...
    connect_blocks(
        &nodes[1],
        TEST_FINAL_CLTV + config.channel_config.cltv_expiry_delta as u32,
    );
    ...
    let htlc_timeout_tx = {
        ...
        assert_eq!(
            tx.input[1].witness.last().unwrap().len(),
            ACCEPTED_HTLC_SCRIPT_WEIGHT + 1
        );

        // Split the HTLC claim transaction into two, one for each HTLC.
        if commitment_txn[0].output[tx.input[1].previous_output.vout as usize]
            .value
            .to_sat()
            < 850
        {
        ...
        if commitment_txn[0].output[tx.input[0].previous_output.vout as usize]
            .value
            .to_sat()
            < 850
        {
    ...
    nodes[2]
        .node
        .handle_update_fulfill_htlc(node_e_id, updates.update_fulfill_htlcs.remove(0));
    ...
    let htlc_success_txn: Vec<_> = nodes[2]
        .tx_broadcaster
        .txn_broadcasted
        .lock()
        .unwrap()
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &htlc_updates.update_fail_htlcs[0]);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    commitment_signed_dance!(
        nodes[0],
        nodes[1],
        &htlc_updates.commitment_signed,
        false,
        true
    );
...
fn do_test_fail_backwards_unrevoked_remote_announce(deliver_last_raa: bool, announce_latest: bool) {
    ...
    assert_eq!(
        get_local_commitment_txn!(nodes[3], chan_2_3.2)[0]
            .output
            .len(),
        2
    );
    ...
    let payment_secret = nodes[5]
        .node
        .create_inbound_payment_for_hash(hash_1, None, 7200, None)
        .unwrap();
    ...
    let payment_secret = nodes[5]
        .node
        .create_inbound_payment_for_hash(hash_2, None, 7200, None)
        .unwrap();
    ...
    let payment_secret = nodes[5]
        .node
        .create_inbound_payment_for_hash(hash_3, None, 7200, None)
        .unwrap();
    send_along_route_with_secret(
        &nodes[1],
        route.clone(),
    ...
    let payment_secret = nodes[5]
        .node
        .create_inbound_payment_for_hash(hash_4, None, 7200, None)
        .unwrap();
    ...
    let payment_secret = nodes[5]
        .node
        .create_inbound_payment_for_hash(hash_5, None, 7200, None)
        .unwrap();
    send_along_route_with_secret(
        &nodes[1],
        route,
    ...
    let payment_secret = nodes[5]
        .node
        .create_inbound_payment_for_hash(hash_6, None, 7200, None)
        .unwrap();
    ...
    nodes[3]
        .node
        .handle_update_fail_htlc(node_e_id, &four_removes.update_fail_htlcs[0]);
    nodes[3]
        .node
        .handle_update_fail_htlc(node_e_id, &four_removes.update_fail_htlcs[1]);
    nodes[3]
        .node
        .handle_update_fail_htlc(node_e_id, &four_removes.update_fail_htlcs[2]);
    nodes[3]
        .node
        .handle_update_fail_htlc(node_e_id, &four_removes.update_fail_htlcs[3]);
    ...
    nodes[3]
        .node
        .handle_update_fail_htlc(node_f_id, &two_removes.update_fail_htlcs[0]);
    nodes[3]
        .node
        .handle_update_fail_htlc(node_f_id, &two_removes.update_fail_htlcs[1]);
    ...
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &six_removes.update_fail_htlcs[0]);
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &six_removes.update_fail_htlcs[1]);
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &six_removes.update_fail_htlcs[2]);
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &six_removes.update_fail_htlcs[3]);
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &six_removes.update_fail_htlcs[4]);
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &six_removes.update_fail_htlcs[5]);
    if deliver_last_raa {
    ...
    for msg in cs_msgs {
        match msg {
            MessageSendEvent::UpdateHTLCs {
                ...
                target
                    .node
                    .handle_update_fail_htlc(node_c_id, &updates.update_fail_htlcs[1]);
                target
                    .node
                    .handle_update_fail_htlc(node_c_id, &updates.update_fail_htlcs[2]);
                if announce_latest {
                    target
                        .node
                        .handle_update_fail_htlc(node_c_id, &updates.update_fail_htlcs[3]);
...
pub fn test_key_derivation_params() {
    ...
    let chan_1 = create_announced_chan_between_nodes(&nodes, 0, 1);
    assert_ne!(
        chan_0.3.output[0].script_pubkey,
        chan_1.3.output[0].script_pubkey
    );
...
fn do_htlc_claim_local_commitment_only(use_dust: bool) {
    ...
    let (payment_preimage, payment_hash, ..) = route_payment(
        &nodes[0],
        &[&nodes[1]],
        if use_dust { 50000 } else { 3_000_000 },
    );
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_updates.update_fulfill_htlcs.remove(0));
    expect_payment_sent(&nodes[0], payment_preimage, None, false, false);

    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_updates.commitment_signed);
    check_added_monitors(&nodes[0], 1);
    let as_updates = get_revoke_commit_msgs!(nodes[0], node_b_id);
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_updates.0);
    check_added_monitors(&nodes[1], 1);
...
fn do_htlc_claim_previous_remote_commitment_only(use_dust: bool, check_revoke_no_close: bool) {
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_updates.1);
    check_added_monitors(&nodes[1], 1);
    let bs_revoke_and_ack = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, node_a_id);
    ...
    if !check_revoke_no_close {
        ...
        check_added_monitors(&nodes[0], 1);
        let reason = ClosureReason::HTLCsTimedOut {
            payment_hash: Some(our_payment_hash),
        };
        check_closed_event!(nodes[0], 1, reason, [node_b_id], 100000);
    } else {
...
pub fn test_fail_holding_cell_htlc_upon_free() {
    ...
    nodes[1]
        .node
        .handle_update_fee(node_a_id, update_msg.unwrap());
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, commitment_signed);
    let (as_revoke_and_ack, _) = get_revoke_commit_msgs!(nodes[1], node_a_id);
    check_added_monitors(&nodes[1], 1);
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &as_revoke_and_ack);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[0].logger.assert_log(
        ...
        format!(
            "Freeing holding cell with 1 HTLC updates in channel {}",
            chan.2
        ),
        1,
    );
...
pub fn test_free_and_fail_holding_cell_htlcs() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_1, payment_hash_1, onion, id_1)
        .unwrap();
    chan_stat = get_channel_value_stat!(nodes[0], nodes[1], chan.2);
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_2.clone(), payment_hash_2, onion, id_2)
        .unwrap();
    chan_stat = get_channel_value_stat!(nodes[0], nodes[1], chan.2);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &revoke_and_ack);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &commitment_signed);
    check_added_monitors(&nodes[0], 2);
    ...
    nodes[0].logger.assert_log(
        "lightning::ln::channel",
        format!(
            "Freeing holding cell with 2 HTLC updates in channel {}",
            chan.2
        ),
        1,
    );
    ...
    match &events[0] {
        &Event::PaymentPathFailed {
            ...
            assert_eq!(
                *short_channel_id,
                Some(route_2.paths[0].hops[0].short_channel_id)
            );
        }
        _ => panic!("Unexpected event"),
    }
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, update_msgs.update_fulfill_htlcs.remove(0));
    commitment_signed_dance!(
        nodes[0],
        nodes[1],
...
pub fn test_fail_holding_cell_htlc_upon_free_multihop() {
    ...
    nodes[2]
        .node
        .handle_update_fee(node_b_id, update_msg.unwrap());
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &commitment_signed);
    check_added_monitors(&nodes[1], 2);
    ...
    let (fail_msg, commitment_signed) = match &fail_event[0] {
        &MessageSendEvent::UpdateHTLCs { ref updates, .. } => {
            ...
            assert_eq!(updates.update_fail_htlcs.len(), 1);
            (
                updates.update_fail_htlcs[0].clone(),
                updates.commitment_signed.clone(),
            )
        }
        _ => panic!("Unexpected event"),
...
pub fn test_update_fulfill_htlc_bolt2_after_malformed_htlc_message_must_forward_update_fail_htlc() {
    ...
    nodes[1]
        .node
        .handle_update_fail_malformed_htlc(node_c_id, &update_msg.0);
...
pub fn test_channel_failed_after_message_with_badonion_node_perm_bits_set() {
    ...
    match events_3[0] {
        MessageSendEvent::UpdateHTLCs { ref updates, .. } => {
            ...
            nodes[1]
                .node
                .handle_update_fail_malformed_htlc(node_c_id, &update_msg);
    ...
    match events_4[0] {
        MessageSendEvent::UpdateHTLCs { ref updates, .. } => {
            nodes[0]
                .node
                .handle_update_fail_htlc(node_b_id, &updates.update_fail_htlcs[0]);
...
fn do_test_failure_delay_dust_htlc_local_commitment(announce_latest: bool) {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &remove.update_fail_htlcs[0]);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &remove.commitment_signed);
    check_added_monitors(&nodes[0], 1);
...
fn do_test_sweep_outbound_htlc_failure_update(revoked: bool, local: bool) {
    ...
    if local {
        ...
        connect_blocks(
            &nodes[0],
            TEST_FINAL_CLTV + LATENCY_GRACE_PERIOD_BLOCKS - ANTI_REORG_DELAY,
        );
        check_added_monitors(&nodes[0], 0);
        assert_eq!(nodes[0].node.get_and_clear_pending_events().len(), 0);
...
pub fn test_check_htlc_underpaying() {
    ...
    let events = nodes[1].node.get_and_clear_pending_events();
    let fail = HTLCHandlingFailureType::Receive {
        payment_hash: our_payment_hash,
    };
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &update_fail_htlc);
    commitment_signed_dance!(nodes[0], nodes[1], commitment_signed, false, true);
...
pub fn test_announce_disable_channels() {
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &reestablish_2[1]);
    handle_chan_reestablish_msgs!(nodes[0], nodes[1]);
    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &reestablish_1[1]);
    handle_chan_reestablish_msgs!(nodes[1], nodes[0]);
    // Reestablish chan_3
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &reestablish_2[2]);
    handle_chan_reestablish_msgs!(nodes[0], nodes[1]);
    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &reestablish_1[2]);
    handle_chan_reestablish_msgs!(nodes[1], nodes[0]);
...
pub fn test_bump_penalty_txn_on_revoked_commitment() {
    ...
    assert_eq!(
        revoked_txn[0].input[0].previous_output.txid,
        chan.3.compute_txid()
    );
    ...
    connect_block(
        &nodes[1],
        &create_dummy_block(header_114, 42, vec![revoked_txn[0].clone()]),
    );
    check_closed_broadcast(&nodes[1], 1, true);
    check_added_monitors(&nodes[1], 1);
            ...
            let mut $fee_rates = new_hash_map();
            {
                ...
                for tx in node_txn.iter() {
                    ...
                    let total_input: u64 = tx
                        ...
                        .iter()
                        .map(|i| {
                            revoked_txn[0].output[i.previous_output.vout as usize]
                                .value
                                .to_sat()
                        })
                        .sum();
...
pub fn test_bump_penalty_txn_on_revoked_htlcs() {
    ...
    let revoked_htlc_txn = {
        ...
        assert_eq!(
            txn[1].input[0].witness.last().unwrap().len(),
            OFFERED_HTLC_SCRIPT_WEIGHT
        );
    ...
    expect_htlc_failure_conditions(
        events[0..1].to_vec(),
        &[HTLCHandlingFailureType::Receive {
            payment_hash: failed_payment_hash,
        }],
    );
...
pub fn test_bump_penalty_txn_on_remote_commitment() {
    ...
    let remote_txn = {
        ...
        assert_eq!(
            remote_txn[0].input[0].previous_output.txid,
            chan.3.compute_txid()
        );
    ...
    let feerate_preimage;
    {
        ...
        assert_eq!(
            node_txn[0].input[0].previous_output,
            preimage_bump.input[0].previous_output
        );
    ...
    connect_blocks(
        &nodes[1],
        crate::chain::package::LOW_FREQUENCY_BUMP_INTERVAL,
    );
    {
        let mut node_txn = nodes[1].tx_broadcaster.txn_broadcasted.lock().unwrap();
        assert_eq!(node_txn.len(), 1);
...
pub fn test_counterparty_raa_skip_no_crash() {
    ...
    let next_per_commitment_point;
    {
        ...
        keys.as_ecdsa()
            ...
            .last_holder_commitment -= 1;
        per_commitment_secret = keys
            .as_ref()
            .release_commitment_secret(INITIAL_COMMITMENT_NUMBER)
            .unwrap();
        ...
        keys.as_ecdsa()
            ...
            .last_holder_commitment -= 1;
        keys.as_ref()
            .release_commitment_secret(INITIAL_COMMITMENT_NUMBER - 1)
            .unwrap();
        ...
        let sec = keys
            .as_ref()
            .release_commitment_secret(INITIAL_COMMITMENT_NUMBER - 2)
            .unwrap();
    ...
    let reason = ClosureReason::ProcessingError {
        err: "Received an unexpected revoke_and_ack".to_string(),
    };
    check_closed_event!(nodes[1], 1, reason, [node_a_id], 100000);
}
...
pub fn test_bump_txn_sanitize_tracking_maps() {
    ...
    connect_block(
        &nodes[0],
        &create_dummy_block(nodes[0].best_block_hash(), 42, penalty_txn),
    );
    connect_blocks(&nodes[0], ANTI_REORG_DELAY - 1);
    {
        let monitor = nodes[0]
...
pub fn test_channel_conf_timeout() {
    ...
    assert_eq!(
        nodes[1].chain_monitor.chain_monitor.list_monitors().len(),
        1
    );
    assert!(!nodes[1]
        .chain_monitor
...
pub fn test_channel_update_has_correct_htlc_maximum_msat() {
    ...
    assert_eq!(
        node_0_chan_update.contents.htlc_maximum_msat,
    ...
    assert_eq!(
        node_1_chan_update.contents.htlc_maximum_msat,
    ...
    // `channel_value`.
    assert_eq!(
        node_2_chan_update.contents.htlc_maximum_msat,
    ...
    // `channel_value`.
    assert_eq!(
        node_3_chan_update.contents.htlc_maximum_msat,
        channel_value_90_percent_msat
    );
}
...
pub fn test_onion_value_mpp_set_calculation() {
    ...
    let chan_1_id = create_announced_chan_between_nodes(&nodes, 0, 1)
        .0
        .contents
        .short_channel_id;
    let chan_2_id = create_announced_chan_between_nodes(&nodes, 0, 2)
        .0
        .contents
        .short_channel_id;
    let chan_3_id = create_announced_chan_between_nodes(&nodes, 1, 3)
        .0
        .contents
        .short_channel_id;
    let chan_4_id = create_announced_chan_between_nodes(&nodes, 2, 3)
        .0
        .contents
        .short_channel_id;
...
fn do_test_overshoot_mpp(msat_amounts: &[u64], total_msat: u64) {
    ...
    let expected_paths: Vec<&[&Node]> = expected_paths
        .iter()
...
pub fn test_preimage_storage() {
...

    {
        let (payment_hash, payment_secret) = nodes[1]
            .node
            .create_inbound_payment(Some(100_000), 7200, None)
            .unwrap();
...
pub fn test_bad_secret_hash() {
    ...
    let (our_payment_hash, our_payment_secret) = nodes[1]
        .node
        .create_inbound_payment(Some(100_000), 2, None)
        .unwrap();
    let (route, _, _, _) = get_route_and_payment_hash!(nodes[0], nodes[1], 100_000);
    ...
    nodes[0]
        .node
        .send_payment_with_route(route.clone(), random_hash, onion, id)
        .unwrap();
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, random_hash, onion, id)
        .unwrap();
    handle_unknown_invalid_payment_data!(random_hash);
    expect_payment_failed!(
        nodes[0],
...
pub fn test_concurrent_monitor_claim() {
    ...
    alice_broadcaster
        ...
        .lock()
        .unwrap()
        .resize((HTLC_TIMEOUT_BROADCAST) as usize, next_block);
    watchtower_alice
        .chain_monitor
        .block_connected(&block, HTLC_TIMEOUT_BROADCAST);
    ...
    watchtower_bob
        .chain_monitor
        .block_connected(&block, HTLC_TIMEOUT_BROADCAST - 1);
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &updates.update_add_htlcs[0]);
    {
        let mut per_peer_lock;
        let mut peer_state_lock;
        let chan_ref =
        ...
        if let Some(channel) = chan_ref.as_funded_mut() {
            ...
            if let Ok(Some(update)) = channel.commitment_signed(
                ...
                assert_eq!(
                    watchtower_alice
                        .chain_monitor
                        .update_channel(chan_1.2, &update),
                    ChannelMonitorUpdateStatus::InProgress
                );
                assert_eq!(
                    watchtower_bob
    ...
    check_closed_broadcast(&nodes[0], 1, true);
    let reason = ClosureReason::HTLCsTimedOut {
        payment_hash: Some(payment_hash_timeout),
    };
...
pub fn test_pre_lockin_no_chan_closed_update() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 100000, 10001, 42, None, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .handle_accept_channel(node_b_id, &accept_chan_msg);
    ...
    let reason = ClosureReason::CounterpartyForceClosed {
        peer_msg: UntrustedString("Hi".to_string()),
    };
...
fn do_test_onchain_htlc_settlement_after_close(
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(node_c_id, carol_fulfill);
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &carol_updates.commitment_signed);
    ...
    nodes[2]
        .node
        .handle_revoke_and_ack(node_b_id, &bob_revocation);
    check_added_monitors(&nodes[2], 1);
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bob_updates.commitment_signed);
    check_added_monitors(&nodes[2], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_c_id, &carol_revocation);
    check_added_monitors(&nodes[1], 1);
    ...
    if broadcast_alice {
        ...
        check_spends!(bob_txn[0], txn_to_broadcast[0]);
        assert_eq!(
            bob_txn[0].input[0].witness.last().unwrap().len(),
            OFFERED_HTLC_SCRIPT_WEIGHT
        );
    } else {
        assert_eq!(
        ...
        assert_eq!(
            htlc_tx.input[0].witness.last().unwrap().len(),
            ACCEPTED_HTLC_SCRIPT_WEIGHT + 1
        );
    }
}
...
pub fn test_peer_funding_sidechannel() {
    ...
    nodes[0]
        .node
        .handle_funding_created(node_b_id, &funding_created_msg);
    ...
    let res = nodes[0]
        .node
        .funding_transaction_generated(temp_chan_id_ab, node_b_id, tx);
...
pub fn test_duplicate_conflicting_funding_from_second_peer() {
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .watch_channel(real_chan_id, dummy_monitor)
        .unwrap();

    nodes[0]
        .node
        .funding_transaction_generated(temp_chan_id, node_b_id, tx.clone())
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_funding_created(node_a_id, &funding_created_msg);
    ...
    nodes[0]
        .node
        .handle_funding_signed(node_b_id, &funding_signed_msg);
    ...
    let reason = ClosureReason::ProcessingError {
        err: "Channel ID was a duplicate".to_owned(),
    };
...
pub fn test_error_chans_closed() {
    ...
    let reason = ClosureReason::CounterpartyForceClosed {
        peer_msg: UntrustedString("ERR".to_string()),
    };
    check_closed_event!(nodes[0], 1, reason, [node_b_id], 100000);
...
fn do_test_dup_htlc_second_rejected(test_for_second_fail_panic: bool) {
    ...
    if test_for_second_fail_panic {
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(node_b_id, &fail_updates_1.update_fail_htlcs[0]);
        nodes[0]
            .node
            .handle_update_fail_htlc(node_b_id, &fail_updates_1.update_fail_htlcs[1]);
...
pub fn test_inconsistent_mpp_params() {
    ...
    let session_privs = {
        ...
        nodes[0]
            .node
            .test_add_new_pending_payment(hash, onion, id, &dup_route)
            .unwrap()
    };
    let onion = RecipientOnionFields::secret_only(payment_secret);
    ...
    nodes[0]
        .node
        .test_send_payment_along_path(
            path_a, &hash, onion, real_amt, cur_height, id, &None, priv_a,
        )
        .unwrap();
    check_added_monitors(&nodes[0], 1);
...

    {
        ...
        nodes[3]
            .node
            .handle_update_add_htlc(node_c_id, &payment_event.msgs[0]);
    ...
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &fail_updates_1.update_fail_htlcs[0]);
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_c_id, &fail_updates_2.update_fail_htlcs[0]);
    ...
    nodes[0]
        .node
        .test_send_payment_along_path(
            path_b, &hash, onion, real_amt, cur_height, id, &None, priv_c,
        )
        .unwrap();
    check_added_monitors(&nodes[0], 1);
...
pub fn test_double_partial_claim() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, hash, onion, id)
        .unwrap();
    check_added_monitors(&nodes[0], 2);
...
fn do_test_max_dust_htlc_exposure(
    dust_outbound_balance: bool,
    exposure_breach_event: ExposureEvent,
    on_holder_tx: bool,
    multiplier_dust_limit: bool,
    apply_excess_fee: bool,
) {
    ...
    let per_htlc_cost_sat = if on_holder_tx {
    ...
    nodes[0]
        .node
        .funding_transaction_generated(chan_id, node_b_id, tx.clone())
        .unwrap();
    ...
    if exposure_breach_event == ExposureEvent::AtHTLCForward {
        ...
        for _ in 0..AT_FEE_OUTBOUND_HTLCS {
            ...
            nodes[0]
                .node
                .send_payment_with_route(route.clone(), hash, onion, id)
                .unwrap();
        }
        {
            let mut feerate_lock = chanmon_cfgs[0].fee_estimator.sat_per_kw.lock().unwrap();
...
pub fn test_nondust_htlc_excess_fees_are_dust() {
    ...
    assert_eq!(nodes[1].node.list_channels().len(), 1);
    assert_eq!(
        nodes[0].node.list_channels()[0].pending_inbound_htlcs.len(),
        0
    );
    assert_eq!(
        nodes[1].node.list_channels()[0]
            .pending_outbound_htlcs
            .len(),
        0
    );
    ...
    nodes[1]
        .node
        .handle_update_fail_htlc(node_a_id, &updates.update_fail_htlcs[0]);
    ...
    let res = nodes[0]
        .node
        .send_payment_with_route(route_0_1, payment_hash_0_1, onion, id);
    unwrap_send_err!(nodes[0], res, true, APIError::ChannelUnavailable { .. }, {});
    ...
    nodes[2]
        .node
        .send_payment_with_route(route, payment_hash, onion, PaymentId([0; 32]))
        .unwrap();
    check_added_monitors(&nodes[2], 1);
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_c_id, &send.msgs[0]);
    ...
    nodes[2]
        .node
        .handle_update_fail_htlc(node_a_id, &fail.update_fail_htlcs[0]);
...
fn do_test_nondust_htlc_fees_dust_exposure_delta(features: ChannelTypeFeatures) {
    ...
    let node_chanmgrs = create_node_chanmgrs(
        2,
        &node_cfgs,
        &[Some(default_config), Some(fixed_limit_config)],
    );
    let mut nodes = create_network(2, &node_cfgs, &node_chanmgrs);
    ...
    assert_eq!(
        nodes[1].node.list_channels()[0].pending_inbound_htlcs.len(),
        DUST_HTLC_COUNT
    );
    ...
    nodes[1]
        .node
        .update_partial_channel_config(&node_a_id, &[chan_id], &config)
        .unwrap();
    ...
    nodes[1]
        .node
        .update_partial_channel_config(&node_a_id, &[chan_id], &update)
        .unwrap();
    ...
    let res = nodes[1]
        .node
        .send_payment_with_route(route_1_0, payment_hash_1_0, onion, id);
...
fn do_payment_with_custom_min_final_cltv_expiry(valid_delta: bool, use_user_hash: bool) {
    ...
    let final_cltv_expiry_delta = if valid_delta {
        min_cltv_expiry_delta + 2
    } else {
        min_cltv_expiry_delta - 2
    };
    ...
    let (hash, payment_preimage, payment_secret) = if use_user_hash {
        ...
        (
            ...
            nodes[1]
                .node
                .get_payment_preimage(hash, payment_secret)
                .unwrap(),
            payment_secret,
        )
    };
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, hash, onion, PaymentId(hash.0))
        .unwrap();
    check_added_monitors(&nodes[0], 1);
    let mut events = nodes[0].node.get_and_clear_pending_msg_events();
    assert_eq!(events.len(), 1);
    ...
    if valid_delta {
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(node_b_id, &fail_updates.update_fail_htlcs[0]);
        commitment_signed_dance!(
            nodes[0],
            nodes[1],
            fail_updates.commitment_signed,
            false,
            true
        );
...
pub fn test_disconnects_peer_awaiting_response_ticks() {
    ...
    *nodes[0].fee_estimator.sat_per_kw.lock().unwrap() *= 2;
    ...
    nodes[1]
        .node
        .handle_update_fee(node_a_id, alice_fee_update.update_fee.as_ref().unwrap());
    nodes[1]
        .node
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bob_revoke_and_ack);
    check_added_monitors(&&nodes[0], 1);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bob_commitment_signed);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[0]
        .node
        .peer_connected(node_b_id, &bob_init, true)
        .unwrap();
    ...
    nodes[1]
        .node
        .peer_connected(node_a_id, &alice_init, true)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &alice_channel_reestablish);
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &bob_channel_reestablish);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &alice_revoke_and_ack);
    check_added_monitors(&nodes[1], 1);
...
pub fn test_remove_expired_outbound_unfunded_channels() {
    ...
    let temp_channel_id = nodes[0]
        .node
        .create_channel(node_b_id, 100_000, 0, 42, None, None)
        .unwrap();
    let open_channel_message =
        get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, node_b_id);
    nodes[1]
        .node
        .handle_open_channel(node_a_id, &open_channel_message);
    let accept_channel_message =
        get_event_msg!(nodes[1], MessageSendEvent::SendAcceptChannel, node_a_id);
    nodes[0]
        .node
        .handle_accept_channel(node_b_id, &accept_channel_message);
    ...
    let check_outbound_channel_existence = |should_exist: bool| {
        ...
        let chan_lock = per_peer_state.get(&node_b_id).unwrap().lock().unwrap();
        assert_eq!(
            chan_lock.channel_by_id.contains_key(&temp_channel_id),
            should_exist
        );
    };
...
fn do_test_manual_broadcast_skips_commitment_until_funding(
    ...
    assert_eq!(
        commitment_input.previous_output.vout,
        u32::from(funding_outpoint.index)
    );
    ...
    assert!(monitor_events
        .iter()
        .all(|event| !matches!(event, Event::BumpTransaction(_))));
}
...
fn do_test_multi_post_event_actions(do_reload: bool) {
    ...
    for dest in &[1, 2] {
        ...
        commitment_signed_dance!(
            nodes[0],
            nodes[*dest],
            htlc_fulfill.commitment_signed,
            false
        );
        check_added_monitors(&nodes[0], 0);
    }
    ...
    nodes[1]
        .node
        .send_payment_with_route(route, payment_hash_3, onion, payment_id)
        .unwrap();
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &send_event.msgs[0]);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &send_event.commitment_msg);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
...
pub fn test_dust_exposure_holding_cell_assertion() {
    ...
    let bc_chan = bs_chans
        .iter()
        .find(|chan| chan.counterparty.node_id == node_c_id)
        .unwrap();
    assert_eq!(
    ...
    nodes[1]
        .node
        .send_payment_with_route(route_bc, payment_hash_bc, onion_bc, id)
        .unwrap();
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_ac, payment_hash_cell, onion_ac, id)
        .unwrap();
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &send_ab.msgs[0]);
    ...
    nodes[2]
        .node
        .send_payment_with_route(route_cb, payment_hash_cb, onion_cb, id)
        .unwrap();
    check_added_monitors(&nodes[2], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_c_id, &send_event_cb.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &send_event_cb.commitment_msg);
    check_added_monitors(&nodes[1], 1);

    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &send_bc.msgs[0]);
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &send_bc.commitment_msg);
    check_added_monitors(&nodes[2], 1);
    ...
    let next_hop = HTLCHandlingFailureType::Forward {
        node_id: Some(node_c_id),
        channel_id: bc_chan_id,
    };
    ...
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_cs);
    check_added_monitors(&nodes[2], 1);
    let cs_raa = get_event_msg!(nodes[2], MessageSendEvent::SendRevokeAndACK, node_b_id);

    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &cs_cs.commitment_signed);
    check_added_monitors(&nodes[1], 1);
