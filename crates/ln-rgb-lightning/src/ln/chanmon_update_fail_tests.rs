...
fn test_monitor_and_persister_update_fail() {
    ...
    let tx_broadcaster = TestBroadcaster {
        ...
        blocks: Arc::new(Mutex::new(vec![
            (genesis_block(Network::Testnet), 200);
            200
        ])),
    };
    let chain_mon = {
    ...
    chain_mon.chain_monitor.block_connected(
...
fn do_test_simple_monitor_temporary_update_fail(disconnect: bool) {
    ...
    chanmon_cfgs[0]
        .persister
        .set_update_ret(ChannelMonitorUpdateStatus::Completed);
        ...
        Event::PaymentClaimable {
            ...
            assert_eq!(
                *receiving_channel_ids,
                &[(channel_id, Some(user_channel_id))]
            );
            match &purpose {
    ...
    nodes[0]
        .node
        .force_close_broadcasting_latest_txn(&channel_id, &node_b_id, message)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
...
fn do_test_monitor_temporary_update_fail(disconnect_count: usize) {
    ...
    let (bs_initial_fulfill, bs_initial_commitment_signed) = match events_2[0] {
        MessageSendEvent::UpdateHTLCs {
            ...
            if (disconnect_count & 16) == 0 {
                ...
                nodes[0]
                    .node
                    .handle_update_fulfill_htlc(node_b_id, fulfill_msg);
    ...
    let (payment_event, initial_revoke_and_ack) = if disconnect_count & !disconnect_flags > 0 {
        ...
        if (disconnect_count & 16) == 0 {
            ...
            assert_eq!(
                bs_resp.2.as_ref().unwrap().update_fulfill_htlcs,
                [bs_initial_fulfill]
            );
            assert_eq!(
                bs_resp.2.as_ref().unwrap().commitment_signed,
                bs_initial_commitment_signed
            );
    ...
    macro_rules! handle_initial_raa {
        () => {
            ...
            assert!(bs_second_commitment_update
                .update_fail_malformed_htlcs
                .is_empty());
...
fn test_monitor_update_fail_cs() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &send_event.msgs[0]);
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &send_event.commitment_msg);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 1);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
...
fn test_monitor_update_raa_while_paused() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, our_payment_hash_1, onion, id)
        .unwrap();
    ...
    nodes[1]
        .node
        .send_payment_with_route(route, our_payment_hash_2, onion_2, id_2)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &send_event_1.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &send_event_1.commitment_msg);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &send_event_2.msgs[0]);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &send_event_2.commitment_msg);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[0], 1);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_update_raa.0);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_cs.commitment_signed);
    check_added_monitors!(nodes[0], 1);
    let as_second_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, node_b_id);

    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_second_raa);
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_second_raa);
    check_added_monitors!(nodes[1], 1);
    expect_and_process_pending_htlcs(&nodes[1], false);
...
fn do_test_monitor_update_fail_raa(test_ignore_second_cs: bool) {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, payment_hash_2, onion_2, id_2)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_c_id, &bs_revoke_and_ack);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    assert!(nodes[1].node.get_and_clear_pending_events().is_empty());
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, payment_hash_3, onion_3, id_3)
        .unwrap();
    check_added_monitors!(nodes[0], 1);

    chanmon_cfgs[1]
        .persister
        .set_update_ret(ChannelMonitorUpdateStatus::Completed); // We succeed in updating the monitor for the first channel
    ...
    let (payment_preimage_4, payment_hash_4) = if test_ignore_second_cs {
        ...
        nodes[2]
            .node
            .send_payment_with_route(route, payment_hash_4, onion_4, id_4)
            .unwrap();
        check_added_monitors!(nodes[2], 1);
        ...
        nodes[1]
            .node
            .handle_update_add_htlc(node_c_id, &send_event.msgs[0]);
        nodes[1]
            .node
            .handle_commitment_signed_batch_test(node_c_id, &send_event.commitment_msg);
        check_added_monitors!(nodes[1], 1);
        assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    ...
    nodes[1]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(chan_2.2, latest_update);
    check_added_monitors!(nodes[1], 0);
    expect_and_process_pending_htlcs_and_htlc_handling_failed(
        &nodes[1],
    ...
    let messages_a = match nodes_0_event {
        MessageSendEvent::UpdateHTLCs {
            ...
            assert!(updates.update_fee.is_none());
            (
                updates.update_fail_htlcs.remove(0),
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &messages_a.0);
    commitment_signed_dance!(nodes[0], nodes[1], messages_a.1, false);
    expect_payment_failed!(nodes[0], payment_hash_1, true);

    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &send_event_b.msgs[0]);
    let as_cs;
    if test_ignore_second_cs {
        nodes[2]
            .node
            .handle_commitment_signed_batch_test(node_b_id, &send_event_b.commitment_msg);
        check_added_monitors!(nodes[2], 1);
        let bs_revoke_and_ack =
            get_event_msg!(nodes[2], MessageSendEvent::SendRevokeAndACK, node_b_id);
        nodes[2]
            .node
            .handle_revoke_and_ack(node_b_id, &raa.unwrap());
        check_added_monitors!(nodes[2], 1);
        ...
        nodes[1]
            .node
            .handle_commitment_signed_batch_test(node_c_id, &bs_cs.commitment_signed);
        check_added_monitors!(nodes[1], 1);
    } else {
        nodes[2]
            .node
    ...
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &as_cs.commitment_signed);
    check_added_monitors!(nodes[2], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_c_id, &bs_second_raa);
    check_added_monitors!(nodes[1], 1);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());

    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &bs_second_cs.commitment_signed);
    check_added_monitors!(nodes[1], 1);
    let as_second_raa = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, node_c_id);

    nodes[2]
        .node
        .handle_revoke_and_ack(node_b_id, &as_second_raa);
    check_added_monitors!(nodes[2], 1);
...
fn test_monitor_update_fail_reestablish() {
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(node_c_id, updates.update_fulfill_htlcs.remove(0));
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &bs_reestablish);

    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &as_reestablish);
    ...
    assert_eq!(
        get_chan_reestablish_msgs!(nodes[0], nodes[1])
            .pop()
            .unwrap(),
        as_reestablish
    );
    assert_eq!(
        get_chan_reestablish_msgs!(nodes[1], nodes[0])
            .pop()
            .unwrap(),
        bs_reestablish
    );
    ...
    nodes[1]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(chan_1.2, latest_update);
    check_added_monitors!(nodes[1], 0);
...
fn raa_no_response_awaiting_raa_state() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route.clone(), payment_hash_1, onion_1, id_1)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    let onion_2 = RecipientOnionFields::secret_only(payment_secret_2);
    let id_2 = PaymentId(payment_hash_2.0);
    nodes[0]
        .node
        .send_payment_with_route(route.clone(), payment_hash_2, onion_2, id_2)
        .unwrap();
    check_added_monitors!(nodes[0], 0);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_responses.0);
    check_added_monitors!(nodes[0], 1);
    let mut events = nodes[0].node.get_and_clear_pending_msg_events();
    assert_eq!(events.len(), 1);
    let payment_event = SendEvent::from_event(events.pop().unwrap());
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_update.commitment_signed);
    check_added_monitors!(nodes[0], 1);
    let as_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, node_b_id);
...
fn claim_while_disconnected_monitor_update_fail() {
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &bs_reconnect);
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &as_reconnect);
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_updates.commitment_signed);
    check_added_monitors!(nodes[1], 1);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_commitment.commitment_signed);
    check_added_monitors!(nodes[0], 1);
    let as_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, node_b_id);

    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_commitment.commitment_signed);
    check_added_monitors!(nodes[1], 1);
    let bs_raa = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, node_a_id);
    nodes[1].node.handle_revoke_and_ack(node_a_id, &as_raa);
    check_added_monitors!(nodes[1], 1);
...
fn first_message_on_recv_ordering() {
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, payment_hash_1, onion_1, id_1)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
...
fn test_monitor_update_fail_claim() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_fulfill.update_fulfill_htlcs.remove(0));
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &bs_forward_update.update_add_htlcs[0]);
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &bs_forward_update.update_add_htlcs[1]);
    commitment_signed_dance!(
        nodes[0],
        nodes[1],
        bs_forward_update.commitment_signed,
        false
    );
    expect_and_process_pending_htlcs(&nodes[0], false);
    ...
    match events[0] {
        Event::PaymentClaimable {
            ...
            assert_eq!(
                *receiving_channel_ids.last().unwrap(),
                (channel_id, Some(42))
            );
            match &purpose {
                PaymentPurpose::Bolt11InvoicePayment {
...
fn test_monitor_update_on_pending_forwards() {
    ...
    nodes[1]
        .node
        .handle_update_fail_htlc(node_c_id, &cs_fail_update.update_fail_htlcs[0]);
    commitment_signed_dance!(
        nodes[1],
        nodes[2],
        cs_fail_update.commitment_signed,
        true,
        true
    );
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
...
fn do_during_funding_monitor_fail(
    confirm_a_first: bool,
    restore_b_before_conf: bool,
    restore_b_before_lock: bool,
) {
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        assert!(node
            ...
            .unwrap());
        node.gossip_sync
            .handle_channel_update(Some(counterparty_node_id), &as_update)
            .unwrap();
        node.gossip_sync
            .handle_channel_update(Some(counterparty_node_id), &bs_update)
            .unwrap();
    }
...
fn test_path_paused_mpp() {
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(chan_2_id, latest_update);
...
fn test_pending_update_fee_ack_on_reconnect() {
    ...
    nodes[1]
        .node
        .handle_update_fee(node_a_id, as_update_fee_msgs.update_fee.as_ref().unwrap());
    nodes[1]
        .node
    ...
    nodes[0]
        ...
        .peer_connected(node_b_id, &init_msg, true)
        .unwrap();
    let as_connect_msg = get_chan_reestablish_msgs!(nodes[0], nodes[1])
        .pop()
        .unwrap();
    nodes[1]
        .node
        .peer_connected(node_a_id, &init_msg, false)
        .unwrap();
    let bs_connect_msg = get_chan_reestablish_msgs!(nodes[1], nodes[0])
        .pop()
        .unwrap();

    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &as_connect_msg);
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &bs_connect_msg);
    get_event_msg!(nodes[0], MessageSendEvent::SendChannelUpdate, node_b_id);

    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &bs_initial_send_msgs.update_add_htlcs[0]);
    nodes[0]
        .node
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_first_raa);
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_third_raa);
    check_added_monitors!(nodes[0], 1);
...
fn test_fail_htlc_on_broadcast_after_claim() {
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(node_c_id, cs_updates.update_fulfill_htlcs.remove(0));
    let mut bs_updates = get_htlc_update_msgs!(nodes[1], node_a_id);
    check_added_monitors!(nodes[1], 1);
    expect_payment_forwarded!(nodes[1], nodes[0], nodes[2], Some(1000), false, false);
...
fn do_update_fee_resend_test(deliver_update: bool, parallel_updates: bool) {
    ...
    if deliver_update {
        nodes[1]
            .node
            .handle_update_fee(node_a_id, update_msgs.update_fee.as_ref().unwrap());
    }
    ...
    if parallel_updates {
        ...
        nodes[1]
            .node
            .handle_update_fee(node_a_id, as_second_update.update_fee.as_ref().unwrap());
        nodes[1]
            .node
        ...
        nodes[1]
            .node
            .handle_revoke_and_ack(node_a_id, &as_first_raa);
...
fn do_channel_holding_cell_serialize(disconnect: bool, reload_a: bool) {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &send.msgs[0]);
    nodes[1]
        .node
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(chan_id, mon_id);
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &cs);
    check_added_monitors!(nodes[0], 1);
    ...
    let commitment_msg = match events.pop().unwrap() {
        MessageSendEvent::UpdateHTLCs {
            ...
            nodes[1]
                .node
                .handle_update_add_htlc(node_a_id, &updates.update_add_htlcs[0]);
            updates.commitment_signed
        }
        _ => panic!("Unexpected event type!"),
    };
...
fn do_test_reconnect_dup_htlc_claims(htlc_status: HTLCStatusAtDupClaim, second_fails: bool) {
    ...
    if htlc_status == HTLCStatusAtDupClaim::HoldingCell {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route, second_payment_hash, onion_2, id_2)
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    ...
    // unavailable.
        ...
        assert_eq!(
            fulfill_msg.channel_id,
            cs_updates.update_fulfill_htlcs[0].channel_id
        );
        assert_eq!(
            fulfill_msg.htlc_id,
            cs_updates.update_fulfill_htlcs[0].htlc_id
        );
        assert_eq!(
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(node_c_id, fulfill_msg);
    expect_payment_forwarded!(nodes[1], nodes[0], nodes[2], Some(1000), false, false);
    check_added_monitors!(nodes[1], 1);
    ...
    if htlc_status == HTLCStatusAtDupClaim::HoldingCell {
        nodes[1]
            .node
            .handle_revoke_and_ack(node_a_id, &as_raa.unwrap());
        check_added_monitors!(nodes[1], 1);
...
fn test_temporary_error_during_shutdown() {
    ...
    nodes[0]
        .node
        .close_channel(&channel_id, &node_b_id)
        .unwrap();
...
fn double_temp_error() {
    ...
    nodes[1]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(channel_id, latest_update_1);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 0);
    nodes[1]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(channel_id, latest_update_2);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, update_fulfill_1);
    check_added_monitors!(nodes[0], 0);
    expect_payment_sent(&nodes[0], payment_preimage_1, None, false, false);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &commitment_signed_b1);
    check_added_monitors!(nodes[0], 1);
    nodes[0].node.process_pending_htlc_forwards();
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &commitment_signed_a1);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, update_fulfill_2);
    check_added_monitors!(nodes[0], 0);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
...
fn do_test_outbound_reload_without_init_mon(use_0conf: bool) {
    ...
    let node_chanmgrs = create_node_chanmgrs(
        2,
        &node_cfgs,
        &[Some(chan_config.clone()), Some(chan_config)],
    );
    let node_a_reload;
    ...
    match events[0] {
        Event::OpenChannelRequest {
            ...
            if use_0conf {
                ...
                nodes[1]
                    .node
                    .accept_inbound_channel(&chan_id, &node_a_id, 0, None)
                    .unwrap();
            }
        }
        _ => panic!("Unexpected event"),
...
fn do_test_blocked_chan_preimage_release(completion_mode: BlockedUpdateComplMode) {
    ...
    assert!(get_monitor!(nodes[1], chan_id_2)
        .get_stored_preimages()
        .contains_key(&payment_hash_2));
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
...
fn do_test_inverted_mon_completion_order(
    with_latest_manager: bool,
    complete_bc_commitment_dance: bool,
) {
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &cs_updates.commitment_signed);
    check_added_monitors(&nodes[1], 1);
...
fn do_test_durable_preimages_on_closed_channel(
    close_chans_before_reload: bool,
    close_only_a: bool,
    hold_post_reload_mon_update: bool,
) {
    ...
    nodes[0]
        .node
        .force_close_broadcasting_latest_txn(&chan_id_ab, &node_b_id, err_msg)
        .unwrap();
    check_closed_event(&nodes[0], 1, reason, false, &[node_b_id], 100000);
    check_added_monitors(&nodes[0], 1);
    let as_closing_tx = nodes[0]
        .tx_broadcaster
        .txn_broadcasted
        .lock()
        .unwrap()
...
fn do_test_reload_mon_update_completion_actions(close_during_reload: bool) {
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_c_id, &cs_final_raa);
    check_added_monitors(&nodes[1], 0);
    ...
    if close_during_reload {
        ...
        nodes[0]
            .node
            .force_close_broadcasting_latest_txn(&chan_id_ab, &node_b_id, msg)
            .unwrap();
        check_added_monitors!(nodes[0], 1);
        check_closed_broadcast!(nodes[0], true);
...
fn do_test_glacial_peer_cant_hang(hold_chan_a: bool) {
    ...
    if !hold_chan_a {
        ...
        let a_update = msg_events
            .drain(..)
            .filter_map(|ev| a_filtermap(ev))
            .collect::<Vec<_>>();
...
fn do_test_partial_claim_mon_update_compl_actions(reload_a: bool, reload_b: bool) {
    ...
    let chan_1_scid = create_announced_chan_between_nodes(&nodes, 0, 1)
        .0
        .contents
        .short_channel_id;
    let chan_2_scid = create_announced_chan_between_nodes(&nodes, 0, 2)
        .0
        .contents
        .short_channel_id;
    ...
    nodes[3]
        .chain_monitor
        .complete_sole_pending_chan_update(&chan_3_id);
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(node_d_id, updates.update_fulfill_htlcs.remove(0));
    check_added_monitors(&nodes[1], 1);
    ...
    if reload_a {
        ...
        let mons = &[
            &chan_3_monitor_serialized[..],
            &chan_4_monitor_serialized[..],
        ];
        reload_node!(
            nodes[3],
            &node_ser,
    ...
    assert!(get_monitor!(nodes[3], chan_3_id)
        .get_stored_preimages()
    ...
    nodes[3]
        .chain_monitor
        .complete_sole_pending_chan_update(&chan_4_id);
    ...
    match remove_first_msg_event_to_node(&node_c_id, &mut ds_msgs) {
        MessageSendEvent::UpdateHTLCs { mut updates, .. } => {
            ...
            nodes[2]
                .node
                .handle_update_fulfill_htlc(node_d_id, update_fulfill);
            check_added_monitors(&nodes[2], 1);
    ...
    nodes[3]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &cs_cs);
    check_added_monitors(&nodes[3], 1);
    ...
    assert!(get_monitor!(nodes[3], chan_3_id)
        .get_stored_preimages()
        .contains_key(&payment_hash));
    assert!(get_monitor!(nodes[3], chan_4_id)
        .get_stored_preimages()
        .contains_key(&payment_hash));
...
fn test_claim_to_closed_channel_blocks_forwarded_preimage_removal() {
    ...
    nodes[0]
        ...
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    let a_reason = ClosureReason::HolderForceClosed {
        broadcasted_latest_txn: Some(true),
        message,
    };
    ...
    nodes[1]
        .chain_monitor
        .complete_sole_pending_chan_update(&chan_a.2);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 1);
    assert!(!get_monitor!(nodes[1], chan_b.2)
        .get_all_current_outbound_htlcs()
...
fn test_single_channel_multiple_mpp() {
    ...
    *nodes[8].chain_monitor.write_blocker.lock().unwrap() = Some(blocker);
    ...
    nodes[7]
        .node
        .handle_update_fulfill_htlc(node_i_id, first_update_fulfill);
    check_added_monitors(&nodes[7], 1);
    expect_payment_forwarded!(nodes[7], nodes[1], nodes[8], Some(1000), false, false);
    nodes[7]
        .node
        .handle_commitment_signed_batch_test(node_i_id, &first_updates.commitment_signed);
    check_added_monitors(&nodes[7], 1);
    ...
    *nodes[8].chain_monitor.write_blocker.lock().unwrap() = None;
    nodes[8]
        .node
        .handle_commitment_signed_batch_test(node_h_id, &cs);
    ...
    nodes[7]
        .node
        .handle_update_fulfill_htlc(node_i_id, updates.update_fulfill_htlcs.remove(0));
    ...
    if let Some(update) = updates.update_fulfill_htlcs.get(0) {
        nodes[7]
            .node
            .handle_update_fulfill_htlc(node_i_id, update.clone());
    ...
    nodes[7]
        .node
        .handle_commitment_signed_batch_test(node_i_id, &updates.commitment_signed);
...
fn native_async_persist() {
    ...
    assert!(
        updates.len() >= 4,
        "The test below needs at least four updates"
    );
    ...
    assert_eq!(persist_futures.pending_futures(), 1);
    assert_eq!(
        async_chain_monitor.release_pending_monitor_events().len(),
        0
    );
...
fn test_mpp_claim_to_holding_cell() {
    ...
    nodes[2]
        .node
        .send_payment(paymnt_hash_2, onion, id, route_params, Retry::Attempts(0))
        .unwrap();
    check_added_monitors(&nodes[2], 1);
    ...
    nodes[3]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &payment_event.commitment_msg);
    check_added_monitors(&nodes[3], 1);
    ...
    nodes[2]
        .node
        .handle_commitment_signed_batch_test(node_d_id, &cs);
    check_added_monitors(&nodes[2], 1);
    ...
    nodes[3]
        .chain_monitor
        .chain_monitor
        .channel_monitor_updated(chan_3_id, latest_id)
        .unwrap();
    ...
    nodes[3]
        .chain_monitor
        .chain_monitor
        .channel_monitor_updated(chan_4_id, latest_id)
        .unwrap();
    ...
    let b_claim_msgs = (
        b_claim.update_fulfill_htlcs.pop().unwrap(),
        b_claim.commitment_signed,
    );
    let c_claim_msgs = (
        c_claim.update_fulfill_htlcs.pop().unwrap(),
        c_claim.commitment_signed,
    );
