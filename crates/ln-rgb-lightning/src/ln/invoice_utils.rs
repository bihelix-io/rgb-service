    ...
    L::Target: Logger,
{
    ...
    log_trace!(
        logger,
        "Considering {} channels for invoice route hints",
        channels.len()
    );
    for channel in channels.into_iter().filter(|chan| chan.is_channel_ready) {
            ...
            || channel.counterparty.forwarding_info.is_none()
        {
            log_trace!(
                logger,
                "Ignoring channel {} for invoice route hints",
                &channel.channel_id
            );
            continue;
        }
        ...
        if channel.is_usable && !online_channel_exists {
            log_trace!(
                logger,
                "Channel with connected peer exists for invoice route hints"
            );
...
mod test {
    ...
    fn create_and_pay_for_bolt11_invoice() {
        ...
        assert_eq!(
            ...
            chan.inbound_htlc_minimum_msat
        );
        assert_eq!(
            invoice.route_hints()[0].0[0].htlc_maximum_msat,
            chan.inbound_htlc_maximum_msat
        );
    ...
    fn do_create_invoice_min_final_cltv_delta(with_custom_delta: bool) {
        ...
        let min_final_cltv_expiry_delta = if with_custom_delta {
    ...
    fn test_create_invoice_from_channelmanager_and_duration_since_epoch_with_payment_hash() {
        ...
        assert_eq!(
            ...
            &sha256::Hash::from_slice(&payment_hash.0[..]).unwrap()
        );
    }
    ...
    fn test_hints_has_only_public_confd_channels() {
        ...
        nodes[1]
            .node
            .handle_channel_ready(node_a_id, &node_a_pub_channel_ready);
    ...
    fn test_hints_has_only_lowest_inbound_capacity_channel_above_minimum() {
        ...
        let mut scid_aliases = HashSet::new();
        scid_aliases.insert(
            chan_1_0_low_inbound_above_amt
                .0
    ...
    fn test_hints_has_only_online_channels() {
        ...
        nodes[0]
            .node
            .peer_disconnected(nodes[2].node.get_our_node_id());
        match_invoice_routes(Some(1_000_000_000), &nodes[0], scid_aliases.clone());
    ...
    fn test_forwarding_info_not_assigned_channel_excluded_from_hints() {
        ...
        nodes[2]
            .node
            .handle_accept_channel(node_a_id, &accept_channel);
        ...
        nodes[0]
            .node
            .handle_channel_ready(node_c_id, &cs_channel_ready);
    ...
    fn test_no_hints_if_a_mix_between_public_and_private_channel_exists() {
        ...
        nodes[2]
            .node
            .handle_channel_update(nodes[0].node.get_our_node_id(), &chan_2_0.1);
        nodes[0]
            .node
            .handle_channel_update(nodes[2].node.get_our_node_id(), &chan_2_0.0);
    ...
    fn test_only_public_channels_includes_no_channels_in_hints() {
        ...
        nodes[0]
            .node
            .handle_channel_update(nodes[1].node.get_our_node_id(), &chan_1_0.0);
        nodes[1]
            .node
            .handle_channel_update(nodes[0].node.get_our_node_id(), &chan_1_0.1);
    ...
    fn do_test_multi_node_receive(user_generated_pmt_hash: bool) {
        ...
        nodes[0]
            .node
            .handle_channel_update(nodes[1].node.get_our_node_id(), &chan_0_1.1);
        nodes[1]
            .node
            .handle_channel_update(nodes[0].node.get_our_node_id(), &chan_0_1.0);
        let chan_0_2 = create_announced_chan_between_nodes_with_value(&nodes, 0, 2, 100000, 10001);
        nodes[0]
            .node
            .handle_channel_update(nodes[2].node.get_our_node_id(), &chan_0_2.1);
        nodes[2]
            .node
            .handle_channel_update(nodes[0].node.get_our_node_id(), &chan_0_2.0);
        ...
        let payment_hash = if user_generated_pmt_hash {
            Some(PaymentHash(
                Sha256::hash(&user_payment_preimage.0[..]).to_byte_array(),
            ))
        } else {
            None
        };
        ...
        nodes[0]
            .node
            .send_payment(payment_hash, onion, id, params, Retry::Attempts(0))
            .unwrap();
        check_added_monitors(&nodes[0], 1);
        ...
        let payment_preimage_opt = if user_generated_pmt_hash {
            None
        } else {
    ...
    fn test_multi_node_hints_has_htlc_min_max_values() {
        ...
        let (payment_hash, _payment_secret) = nodes[1]
            .node
            .create_inbound_payment(Some(payment_amt), 3600, None)
            .unwrap();
        let route_hints = vec![
            nodes[1].node.get_phantom_route_hints(),
            nodes[2].node.get_phantom_route_hints(),
    ...
    fn create_phantom_invoice_with_custom_payment_hash_and_custom_min_final_cltv_delta() {
        ...
        let user_payment_preimage = PaymentPreimage([1; 32]);
        let payment_hash = Some(PaymentHash(
            Sha256::hash(&user_payment_preimage.0[..]).to_byte_array(),
        ));
        let non_default_invoice_expiry_secs = 4200;
    ...
    fn test_multi_node_forwarding_info_not_assigned_channel_excluded_from_hints() {
        ...
        nodes[1]
            .node
            .handle_channel_ready(node_d_id, &ds_channel_ready);
        get_event_msg!(nodes[1], MessageSendEvent::SendChannelUpdate, node_d_id);
        nodes[3]
            .node
            .handle_channel_ready(nodes[1].node.get_our_node_id(), &bs_channel_ready);
    ...
    fn test_multi_node_hints_has_only_lowest_inbound_channel_above_minimum() {
        ...
        let mut scid_aliases = HashSet::new();
        scid_aliases.insert(
            chan_0_1_above_amt_low_inbound
                .0
                .short_channel_id_alias
                .unwrap(),
        );
