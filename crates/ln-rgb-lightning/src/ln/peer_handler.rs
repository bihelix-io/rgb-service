...
impl ChannelMessageHandler for ErroringMessageHandler {
    ...
    fn handle_peer_storage_retrieval(
        &self,
        _their_node_id: PublicKey,
        _msg: msgs::PeerStorageRetrieval,
    ) {
    }
...
impl Peer {
    ...
    fn buffer_full_drop_gossip_broadcast(&self) -> bool {
        let total_outbound_buffered: usize = self
    ...
    SM::Target: SendOnlyMessageHandler,
{
    ...
    fn update_gossip_backlogged(&self) {
        ...
        let prev_state = self
            .gossip_processing_backlogged
            .swap(new_state, Ordering::Relaxed);
        if prev_state && !new_state {
            self.gossip_processing_backlog_lifted
            ...
            if peer.should_buffer_gossip_backfill() {
                match peer.sync_status {
                    ...
                    InitSyncTracker::ChannelsSyncing(c) if c < 0xffff_ffff_ffff_ffff => {
                        if let Some((announce, update_a_option, update_b_option)) = self
                            .message_handler
                            .route_handler
                            .get_next_channel_announcement(c)
                        {
    ...
    fn enqueue_message<M: wire::Type>(&self, peer: &mut Peer, message: &M) {
        ...
        peer.msgs_sent_since_pong += 1;
        peer.pending_outbound_buffer
            .push_back(peer.channel_encryptor.encrypt_message(message));
    }
        ...
        if let wire::Message::CommitmentSigned(msg) = message {
            if let Some(message_batch) = &mut peer_lock.message_batch {
                ...
                if messages.len() == message_batch.batch_size {
                    ...
                    return Ok(Some(LogicalMessage::CommitmentSignedBatch(
                        channel_id, batch,
                    )));
                } else {
                    return Ok(None);
                ...
                return Ok(Some(LogicalMessage::FromWire(
    ...
    > {
        ...
        match message {
            ...
            wire::Message::OpenChannelV2(_msg) => {
                self.message_handler
                    .chan_handler
                    .handle_open_channel_v2(their_node_id, &_msg);
            }
            ...
            wire::Message::PeerStorage(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_peer_storage(their_node_id, msg);
            }
            wire::Message::PeerStorageRetrieval(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_peer_storage_retrieval(their_node_id, msg);
            }
            ...
            wire::Message::SpliceInit(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_splice_init(their_node_id, &msg);
            }
            wire::Message::SpliceAck(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_splice_ack(their_node_id, &msg);
            }
            wire::Message::SpliceLocked(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_splice_locked(their_node_id, &msg);
            }
            ...
            wire::Message::ClosingComplete(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_closing_complete(their_node_id, msg);
            }
            #[cfg(simple_close)]
            wire::Message::ClosingSig(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_closing_sig(their_node_id, msg);
            }
    ...
    fn forward_broadcast_msg(
        ...
        match msg {
            ...
            _ => {
                debug_assert!(
                    false,
                    "We shouldn't attempt to forward anything but gossip messages"
                )
            }
        }
    }
    ...
    pub fn process_events(&self) {
        ...
        loop {
            self.update_gossip_backlogged();
            let flush_read_disabled = self
                .gossip_processing_backlog_lifted
...

            {
                ...
                let mut handle_event = |event, from_chan_handler| {
                    match event {
                        MessageSendEvent::SendPeerStorage {
                            ref node_id,
                            ref msg,
                        } => {
                            log_debug!(
                        ...
                        MessageSendEvent::SendPeerStorageRetrieval {
                            ref node_id,
                            ref msg,
                        } => {
                            log_debug!(
    ...
    pub fn broadcast_node_announcement(
        ...
        rgb: [u8; 3],
        alias: [u8; 32],
        mut addresses: Vec<SocketAddress>,
    ) {
            ...
            .sign_gossip_message(msgs::UnsignedGossipMessage::NodeAnnouncement(&announcement))
        {
            ...
            Err(_) => {
                log_error!(
                    self.logger,
                    "Failed to generate signature for node_announcement"
                );
                return;
            }
        };
...
mod tests {
    ...
    fn try_establish_connection<'a>(
        ...
        let addr_a = SocketAddress::TcpIpV4 {
            addr: [127, 0, 0, 1],
            port: 1000,
        };
        let addr_b = SocketAddress::TcpIpV4 {
            addr: [127, 0, 0, 1],
            port: 1001,
        };
    ...
    fn establish_connection<'a>(
        ...
        assert_eq!(
            peer_a.peer_by_node_id(&id_b).unwrap().counterparty_node_id,
            id_b
        );
        assert_eq!(
            peer_a.peer_by_node_id(&id_b).unwrap().socket_address,
            Some(addr_b)
        );
        assert_eq!(
            peer_a.peer_by_node_id(&id_b).unwrap().init_features,
            features_b
        );
        assert_eq!(
            peer_b.peer_by_node_id(&id_a).unwrap().counterparty_node_id,
            id_a
        );
        assert_eq!(
            peer_b.peer_by_node_id(&id_a).unwrap().socket_address,
            Some(addr_a)
        );
        assert_eq!(
            peer_b.peer_by_node_id(&id_a).unwrap().init_features,
            features_a
        );
    ...
    fn fuzz_threaded_connections() {
        ...
        let peers = Arc::new(create_network(2, unsafe {
            &*(&*cfgs as *const _) as &'static _
        }));
    ...
    fn test_feature_incompatible_peers() {
        ...
        let peer_pairs = [
            (&peers[0], &incompatible_peers[0]),
            (&incompatible_peers[1], &peers[1]),
        ];
    ...
    fn test_non_init_first_msg() {
        ...
        let addr_dup = SocketAddress::TcpIpV4 {
            addr: [127, 0, 0, 1],
            port: 1003,
        };
        let id_a = cfgs[0].node_signer.get_node_id(Recipient::Node).unwrap();
        peers[0]
            .new_inbound_connection(fd_dup.clone(), Some(addr_dup.clone()))
            .unwrap();
        ...
        let (act_three, _) = dup_encryptor
            .process_act_two(&a_data[..], &&cfgs[1].node_signer)
            .unwrap();
        peers[0].read_event(&mut fd_dup, &act_three).unwrap();

        let not_init_msg = msgs::Ping {
            ponglen: 4,
            byteslen: 0,
        };
    ...
    fn do_test_peer_connected_error_disconnects(handler: usize) {
        ...
        assert!(chan_handler
            ...
            .unwrap()
            .is_empty());
        assert!(route_handler
            .conn_tracker
            .connected_peers
            .lock()
            .unwrap()
            .is_empty());
        assert!(custom_message_handler
            .conn_tracker
            .connected_peers
            .lock()
            .unwrap()
            .is_empty());
        assert!(send_only_msg_handler
            .0
            .connected_peers
            .lock()
            .unwrap()
    ...
    fn test_do_attempt_write_data() {
        ...
        cfgs[0]
            .routing_handler
            .announcement_available_for_sync
            .store(true, Ordering::Release);
        cfgs[1]
            .routing_handler
            .announcement_available_for_sync
    ...
    fn test_handshake_timeout() {
        ...
        let mut fd_b = FileDescriptor::new(1);
        let initial_data = peers[1]
            .new_outbound_connection(a_id, fd_b.clone(), None)
            .unwrap();
    ...
    fn test_inbound_conn_handshake_complete_awaiting_pong() {
        ...
        let act_three_with_init_b = fd_b.outbound_data.lock().unwrap().split_off(0);
        {
            ...
            let handshake_complete = peer_a_lock
                .get(&fd_a)
                .unwrap()
                .lock()
                .unwrap()
        ...
        peer_a
            .read_event(&mut fd_a, &act_three_with_init_b)
            .unwrap();
        peer_a.process_events();
...

        {
            ...
            let handshake_complete = peer_b_lock
                .get(&fd_b)
                .unwrap()
                .lock()
                .unwrap()
    ...
    fn test_filter_addresses() {
        ...
        // For (10/8)
        let ip_address = SocketAddress::TcpIpV4 {
            addr: [10, 0, 0, 0],
            port: 1000,
        };
        ...
        assert_eq!(filter_addresses(Some(ip_address.clone())), None);
        let ip_address = SocketAddress::TcpIpV4 {
            addr: [169, 254, 221, 101],
            port: 1000,
        };
        assert_eq!(filter_addresses(Some(ip_address.clone())), None);
        let ip_address = SocketAddress::TcpIpV4 {
            addr: [169, 254, 255, 255],
            port: 1000,
        };
        assert_eq!(filter_addresses(Some(ip_address.clone())), None);
        ...
        assert_eq!(
            filter_addresses(Some(ip_address.clone())),
    ...
    fn test_process_events_multithreaded() {
        ...
        // Until we have std::thread::scoped we have to unsafe { turn off the borrow checker }.
        let peer = Arc::new(
            create_network(1, unsafe { &*(&*cfg as *const _) as &'static _ })
                .pop()
                .unwrap(),
        );
