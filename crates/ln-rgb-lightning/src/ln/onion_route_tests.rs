...
fn run_onion_failure_test<F1, F2>(
    ...
    expected_channel_update: Option<NetworkUpdate>,
...
fn run_onion_failure_test_with_fail_intercept<F1, F2, F3>(
    ...
    payment_secret: &PaymentSecret,
    mut callback_msg: F1,
    mut callback_fail: F2,
    mut callback_node: F3,
    expected_retryable: bool,
    ...
    F3: FnMut(),
{
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(nodes[0].node.get_our_node_id(), &update_add_0);
    ...
    let update_1_0 = match test_case {
        ...
        1 | 2 | 3 | 200 | 201 => {
            ...
            nodes[2]
                .node
                .handle_update_add_htlc(nodes[1].node.get_our_node_id(), &update_add_1);
            ...
            nodes[1]
                .node
                .handle_update_fail_htlc(nodes[2].node.get_our_node_id(), &fail_msg);
    ...
    if update_1_0.update_fail_htlcs.len() > 0 {
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(nodes[1].node.get_our_node_id(), &fail_msg);
    } else {
...
fn test_onion_failure() {
    ...
    run_onion_failure_test(
        ...
        |msg| {
            ...
            let (mut onion_payloads, _htlc_msat, _htlc_cltv) = build_onion_payloads(
                path,
                40000,
                &recipient_fields,
                cur_height,
    ...
    run_onion_failure_test(
        ...
        Some(short_channel_id),
        Some(HTLCHandlingFailureType::InvalidForward {
...
fn do_test_onion_failure_stale_channel_update(announce_for_forwarding: bool) {
    ...
    let channel_to_update = if announce_for_forwarding {
        ...
        (
            ...
            channel.0.short_channel_id_alias.unwrap(),
        )
    };
    ...
    let expect_onion_failure = |name: &str, error_reason: LocalHTLCFailureReason| {
        ...
        let network_update = NetworkUpdate::ChannelFailure {
            short_channel_id,
            is_permanent: false,
        };
        run_onion_failure_test(
...
fn test_always_create_tlv_format_onion_payloads() {
    ...
    *node_cfgs[1].override_init_features.borrow_mut() = Some(no_variable_length_onion_features);
    ...
    let recipient_fields = RecipientOnionFields::spontaneous_empty();
    let path = &route.paths[0];
    let (onion_payloads, _htlc_msat, _htlc_cltv) = build_onion_payloads(
        path,
        40000,
        &recipient_fields,
        cur_height,
...
fn test_trampoline_onion_payload_assembly_values() {
    ...
    if let OutboundTrampolinePayload::Forward {
        amt_to_forward,
        outgoing_cltv_value,
        ..
    ...
    if let OutboundOnionPayload::Forward {
        amt_to_forward,
        outgoing_cltv_value,
        ..
    } = outer_bob_payload
...
fn test_trampoline_onion_payload_construction_vectors() {
    ...
    let trampoline_payloads = vec![
        trampoline_payload_carol,
        trampoline_payload_dave,
        trampoline_payload_eve,
    ];
...
fn do_test_fail_htlc_backwards_with_reason(failure_code: FailureCode) {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(nodes[1].node.get_our_node_id(), &update_fail_htlc);
...
fn test_phantom_onion_hmac_failure() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(nodes[0].node.get_our_node_id(), &update_add);
    commitment_signed_dance!(nodes[1], nodes[0], &update_0.commitment_signed, false, true);
...
fn test_phantom_failure_modified_cltv() {
    ...
    expect_htlc_handling_failed_destinations!(
        ...
        &[HTLCHandlingFailureType::InvalidForward {
            requested_forward_scid: phantom_scid
        }]
    );
    check_added_monitors(&nodes[1], 1);
