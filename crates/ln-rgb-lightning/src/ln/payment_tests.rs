...
fn mpp_retry() {
    ...
    nodes[0]
        .router
        .expect_find_route(route_params.clone(), Ok(route.clone()));
    let onion = RecipientOnionFields::secret_only(pay_secret);
    let retry = Retry::Attempts(1);
    nodes[0]
        .node
        .send_payment(hash, onion, id, route_params.clone(), retry)
        .unwrap();
    ...
    let fail = HTLCHandlingFailureType::Forward {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_c_id, &htlc_updates.update_fail_htlcs[0]);
...
fn do_mpp_receive_timeout(send_partial_mpp: bool) {
    ...
    if send_partial_mpp {
        ...
        nodes[1]
            .node
            .handle_update_fail_htlc(node_d_id, &htlc_fail_updates.update_fail_htlcs[0]);
        check_added_monitors!(nodes[3], 1);

        commitment_signed_dance!(
            nodes[1],
            nodes[3],
            htlc_fail_updates.commitment_signed,
            false
        );
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(node_b_id, &htlc_fail_updates.update_fail_htlcs[0]);
...
fn do_test_keysend_payments(public_node: bool) {
...

    {
        ...
        nodes[0]
            .node
            .send_spontaneous_payment(preimage, onion, id, route_params, retry)
            .unwrap();
    }
...
fn test_fulfill_hold_times() {
    ...
    let (_, path_events) = claim_payment_along_route(ClaimAlongRouteArgs::new(
        &nodes[0],
        route,
        preimage.unwrap(),
    ));
...
fn test_reject_mpp_keysend_htlc_mismatching_secret() {
    ...
    nodes[0]
        .router
        .expect_find_route(route.route_params.clone().unwrap(), Ok(route.clone()));
    let params = route.route_params.clone().unwrap();
    let onion = RecipientOnionFields::spontaneous_empty();
    let retry = Retry::Attempts(0);
    nodes[0]
        .node
        .send_spontaneous_payment(preimage, onion, payment_id_0, params, retry)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &update_add_0);
    ...
    nodes[3]
        .node
        .handle_update_add_htlc(node_b_id, &update_add_1);
    ...
    nodes[0]
        .node
        .send_spontaneous_payment(preimage, onion, payment_id_1, params, retry)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_a_id, &update_add_2);
    ...
    nodes[3]
        .node
        .handle_update_add_htlc(node_c_id, &update_add_3);
    ...
    nodes[2]
        .node
        .handle_update_fail_htlc(node_d_id, &update_fail_0.update_fail_htlcs[0]);
    commitment_signed_dance!(nodes[2], nodes[3], update_fail_0.commitment_signed, false);

    let fail_type = HTLCHandlingFailureType::Forward {
        node_id: Some(node_d_id),
        channel_id: chan_4_chan_id,
    };
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_c_id, &update_fail_1.update_fail_htlcs[0]);
...
fn no_pending_leak_on_initial_send_failure() {
    ...
    let res = nodes[0]
        .node
        .send_payment_with_route(route, payment_hash, onion, payment_id);
...
fn do_retry_with_no_persist(confirm_before_reload: bool) {
    ...
    nodes[0]
        .node
        .send_payment(payment_hash, onion, id, route_params, Retry::Attempts(1))
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    ...
    if !confirm_before_reload {
        let as_broadcasted_txn = nodes[0]
            .tx_broadcaster
            .txn_broadcasted
            .lock()
            .unwrap()
        ...
        assert_eq!(
            as_broadcasted_txn[0].compute_txid(),
            as_commitment_tx.compute_txid()
        );
    } else {
        assert!(nodes[0]
            .tx_broadcaster
    ...
    if confirm_before_reload {
        ...
        nodes[0]
            .node
            .best_block_updated(&best_block.0.header, best_block.1);
    }
    ...
    nodes[0]
        .node
        .send_payment_with_route(new_route.clone(), payment_hash, onion, id)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    ...
    expect_payment_sent!(
        nodes[0],
        payment_preimage,
        Some(new_route.paths[0].hops[0].fee_msat)
    );
}
...
fn do_test_completed_payment_not_retryable_on_reload(use_dust: bool) {
    ...
    match as_err[1] {
        MessageSendEvent::HandleError {
            ...
            let reason = ClosureReason::CounterpartyForceClosed {
                peer_msg: UntrustedString(msg),
            };
            check_closed_event!(nodes[1], 1, reason, [node_a_id], 100000);
            check_added_monitors!(nodes[1], 1);
            bs_commitment_tx = nodes[1]
                .tx_broadcaster
                .txn_broadcasted
                .lock()
                .unwrap()
    ...
    nodes[1]
        .node
        .handle_update_fail_htlc(node_c_id, &htlc_fulfill_updates.update_fail_htlcs[0]);
    commitment_signed_dance!(
        nodes[1],
        nodes[2],
    ...
    match nodes[0]
        .node
        .send_payment_with_route(new_route.clone(), hash, onion, payment_id)
    {
    ...
    match nodes[0]
        .node
        .send_payment_with_route(new_route, hash, onion, payment_id)
    {
        Err(RetryableSendFailure::DuplicatePayment) => {}
        _ => panic!("Unexpected error"),
    }
...
fn do_test_dup_htlc_onchain_doesnt_fail_on_reload(
    persist_manager_post_event: bool,
    persist_monitor_after_events: bool,
    confirm_commitment_tx: bool,
    payment_timeout: bool,
) {
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .block_connected(&claim_block, height);
    assert!(nodes[0].node.get_and_clear_pending_events().is_empty());
}
...
fn get_ldk_payment_preimage() {
    ...
    let (payment_hash, payment_secret) = nodes[1]
        .node
        .create_inbound_payment(Some(amt_msat), expiry_secs, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .send_payment_with_route(route.unwrap(), payment_hash, onion, id)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
...
fn failed_probe_yields_event() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &probe_event.msgs[0]);
    ...
    match events.drain(..).next().unwrap() {
        crate::events::Event::ProbeFailed {
            payment_id: ev_pid,
            payment_hash: ev_ph,
            ..
        } => {
...
fn preflight_probes_yield_event() {
    ...
    let expected_route: &[(&[&Node], PaymentHash)] = &[
        (&[&nodes[1], &nodes[3]], res[0].0),
        (&[&nodes[2], &nodes[3]], res[1].0),
    ];
...
fn claimed_send_payment_idempotent() {
    ...
    macro_rules! check_send_rejected {
        () => {
            ...
            let send_result =
                nodes[0]
                    .node
                    .send_payment_with_route(route.clone(), hash_b, onion, payment_id);
            match send_result {
                Err(RetryableSendFailure::DuplicatePayment) => {}
    ...
    nodes[0]
        .node
        .send_payment_with_route(route, hash_b, onion, payment_id)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    pass_along_route(
        &nodes[0],
...
fn abandoned_send_payment_idempotent() {
    ...
    let fail_type = HTLCHandlingFailureType::Receive {
...
fn test_holding_cell_inflight_htlcs() {
    ...
    // holding cell as nodes[0] is AwaitingRAA.
    {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route.clone(), payment_hash_1, onion, id)
            .unwrap();
        check_added_monitors!(nodes[0], 1);
...
fn do_test_intercepted_payment(test: InterceptTest) {
    ...
    let configs = [
        None,
        Some(intercept_forwards_config),
        Some(zero_conf_chan_config),
    ];
    ...
    let (hash, payment_secret) = nodes[2]
        .node
        .create_inbound_payment(Some(amt_msat), 60 * 60, None)
        .unwrap();
    let onion = RecipientOnionFields::secret_only(payment_secret);
    let id = PaymentId(hash.0);
    nodes[0]
    ...
    let (intercept_id, outbound_amt) = match events[0] {
        crate::events::Event::HTLCIntercepted {
            ...
            assert_eq!(
                inbound_amount_msat,
                route.get_total_amount() + route.get_total_fees()
            );
    ...
    if test == InterceptTest::Fail {
        ...
        let fail = HTLCHandlingFailureType::InvalidForward {
        ...
        let temp_id = nodes[1]
            .node
            .create_channel(node_c_id, 100_000, 0, 42, None, None)
            .unwrap();
        let unusable_chan_err =
            nodes[1]
                .node
                .forward_intercepted_htlc(intercept_id, &temp_id, node_c_id, outbound_amt);
        let err = format!(
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(node_b_id, &htlc_fail.update_fail_htlcs[0]);
...
fn do_accept_underpaying_htlcs_config(num_mpp_parts: usize) {
    ...
    let payment_params = PaymentParameters::from_node_id(node_c_id, TEST_FINAL_CLTV)
        ...
        .unwrap();
    let route_params = RouteParameters::from_payment_params_and_value(payment_params, amt_msat);
    let (payment_hash, payment_secret) = nodes[2]
        .node
        .create_inbound_payment(Some(amt_msat), 60 * 60, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .send_payment(payment_hash, onion, id, route_params, Retry::Attempts(0))
        .unwrap();
    ...
    for (idx, ev) in events.into_iter().enumerate() {
        ...
        nodes[2]
            .node
            .handle_update_add_htlc(node_b_id, &pay_event.msgs[0]);
    ...
    match events[0] {
        crate::events::Event::PaymentClaimable {
            ...
            assert_eq!(pmt_hash, payment_hash);
            assert_eq!(
                amt_msat - skimmed_fee_msat * num_mpp_parts as u64,
                amount_msat
            );
            assert_eq!(
                skimmed_fee_msat * num_mpp_parts as u64,
                counterparty_skimmed_fee_msat
            );
...
fn do_automatic_retries(test: AutoRetry) {
    ...
    if test == AutoRetry::Success {
        ...
        nodes[0]
            .node
            .send_payment(hash, onion, id, route_params, retry)
            .unwrap();
        ...
        nodes[0]
            .node
            .send_payment(hash, onion, id, route_params, Retry::Attempts(1))
            .unwrap();
        pass_failed_attempt_with_retry_along_path!(channel_id_2, true);
        ...
        nodes[0]
            .node
            .send_payment(hash, onion, id, route_params, Retry::Attempts(2))
            .unwrap();
        pass_failed_attempt_with_retry_along_path!(channel_id_2, true);
...
fn auto_retry_partial_failure() {
    ...
    nodes[0]
        .router
        .expect_find_route(route_params.clone(), Ok(send_route));
    ...
    nodes[0]
        .router
        .expect_find_route(retry_1_params.clone(), Ok(retry_1_route));
    ...
    nodes[0]
        .router
        .expect_find_route(retry_2_params, Ok(retry_2_route));
    ...
    nodes[0]
        .node
        .send_payment(payment_hash, onion, id, route_params, Retry::Attempts(3))
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &as_2nd_htlcs.msgs[0]);
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &as_2nd_htlcs.msgs[1]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_2nd_htlcs.commitment_msg);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_claim.update_fulfill_htlcs.remove(0));
    expect_payment_sent(&nodes[0], payment_preimage, None, false, false);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_claim.commitment_signed);
    check_added_monitors!(nodes[0], 1);
    let (as_third_raa, as_third_cs) = get_revoke_commit_msgs!(nodes[0], node_b_id);

    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_third_raa);
    check_added_monitors!(nodes[1], 4);
    let mut bs_2nd_claim = get_htlc_update_msgs!(nodes[1], node_a_id);

    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_third_cs);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_second_fulfill_a);
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, bs_second_fulfill_b);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_2nd_claim.commitment_signed);
    check_added_monitors!(nodes[0], 1);
    let (as_fourth_raa, as_fourth_cs) = get_revoke_commit_msgs!(nodes[0], node_b_id);

    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_fourth_raa);
    check_added_monitors!(nodes[1], 1);

    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_fourth_cs);
    check_added_monitors!(nodes[1], 1);
...
fn retry_multi_path_single_failed_payment() {
    ...
    pay_params
        .previously_failed_channels
        .push(chans[1].short_channel_id.unwrap());
    ...
    nodes[0]
        .router
        .expect_find_route(retry_params, Ok(route.clone()));
...
fn immediate_retry_on_failure() {
    ...
    pay_params
        .previously_failed_channels
        .push(chans[0].short_channel_id.unwrap());
...
fn no_extra_retries_on_back_to_back_fail() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &first_htlc.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &first_htlc.commitment_msg);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &second_htlc.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &second_htlc.commitment_msg);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &bs_fail_update.update_fail_htlcs[0]);
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &bs_fail_update.update_fail_htlcs[1]);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &retry_htlc_updates.msgs[0]);
    commitment_signed_dance!(
        nodes[1],
        nodes[0],
...
fn test_simple_partial_retry() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &second_htlc_updates.msgs[0]);
    commitment_signed_dance!(
        nodes[1],
        nodes[0],
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &bs_second_forward.update_add_htlcs[0]);
    commitment_signed_dance!(
        nodes[2],
        nodes[1],
        &bs_second_forward.commitment_signed,
        false
    );
...
fn test_threaded_payment_retries() {
    ...
    nodes[0]
        .node
        .send_payment(payment_hash, onion, id, route_params.clone(), retry)
        .unwrap();
    check_added_monitors!(nodes[0], 2);
    ...
    loop {
        ...
        nodes[0]
            .router
            .expect_find_route(new_route_params, Ok(route.clone()));

        let bs_fail_updates = get_htlc_update_msgs!(nodes[1], node_a_id);
        nodes[0]
            .node
            .handle_update_fail_htlc(node_b_id, &bs_fail_updates.update_fail_htlcs[0]);
...
fn do_no_missing_sent_on_reload(persist_manager_with_payment: bool, at_midpoint: bool) {
    ...
    if at_midpoint {
        ...
        nodes[0]
            .node
            .handle_update_fulfill_htlc(node_b_id, fulfill.update_fulfill_htlcs.remove(0));
...
fn do_claim_from_closed_chan(fail_payment: bool) {
    ...
    let mut route = nodes[0]
        .router
        .find_route(&node_a_id, &route_params, None, inflight)
        .unwrap();
    ...
    send_msgs.sort_by(|a, _| {
        let a_node_id = if let MessageSendEvent::UpdateHTLCs { node_id, .. } = a {
            node_id
        } else {
            panic!()
        };
...
fn do_test_custom_tlvs(spontaneous: bool, even_tlvs: bool, known_tlvs: bool) {
    ...
    let custom_tlvs = vec![
        (
            if even_tlvs { 5482373482 } else { 5482373483 },
            vec![1, 2, 3, 4],
        ),
        (5482373487, vec![0x42u8; 16]),
    ];
    let onion = RecipientOnionFields {
        payment_secret: if spontaneous {
            None
        } else {
            Some(payment_secret)
        },
    ...
    if spontaneous {
        ...
        nodes[0]
            .node
            .send_spontaneous_payment(Some(preimage), onion, id, params, retry)
            .unwrap();
    } else {
        nodes[0]
            .node
    ...
    match events[0] {
        Event::PaymentClaimable {
            ...
            assert_eq!(
                onion_fields.clone().unwrap().custom_tlvs().clone(),
                custom_tlvs
            );
        }
        _ => panic!("Unexpected event"),
...
fn test_retry_custom_tlvs() {
    ...
    nodes[0]
        .node
        .send_payment(hash, onion, id, route_params.clone(), Retry::Attempts(1))
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &update_add_htlcs[0]);
...
fn do_test_custom_tlvs_consistency(
    first_tlvs: Vec<(u64, Vec<u8>)>,
    second_tlvs: Vec<(u64, Vec<u8>)>,
    ...
    nodes[0]
        ...
        .test_send_payment_along_path(
            path_a, &hash, onion, amt_msat, cur_height, id, &None, priv_a,
        )
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[0]
        .node
        .test_send_payment_along_path(
            path_b, &hash, onion, amt_msat, cur_height, id, &None, priv_b,
        )
        .unwrap();
    check_added_monitors!(nodes[0], 1);
...
fn do_test_payment_metadata_consistency(do_reload: bool, do_modify: bool) {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &b_recv_ev.msgs[0]);
    ...
    nodes[3]
        .node
        .handle_update_add_htlc(node_b_id, &b_forward_ev.msgs[0]);
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_a_id, &c_recv_ev.msgs[0]);
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_c_id, &cs_fail.update_fail_htlcs[0]);
    ...
    if do_modify {
        nodes[0]
            .node
            .test_set_payment_metadata(payment_id, Some(Vec::new()));
    }
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_a_id, &as_resend.msgs[0]);
    ...
    nodes[3]
        .node
        .handle_update_add_htlc(node_c_id, &cs_forward.msgs[0]);
    ...
    if do_modify {
        ...
        nodes[2]
            .node
            .handle_update_fail_htlc(node_d_id, &ds_fail.update_fail_htlcs[0]);
        commitment_signed_dance!(nodes[2], nodes[3], ds_fail.commitment_signed, false, true);
        let events = nodes[2].node.get_and_clear_pending_events();
        let fail_type = HTLCHandlingFailureType::Forward {
            node_id: Some(node_d_id),
            channel_id: cd_chan_id,
        };
...
fn test_non_strict_forwarding() {
    ...
    for i in 0..4 {
        ...
        nodes[0]
            .node
            .send_payment_with_route(route.clone(), payment_hash, onion, id)
            .unwrap();
        check_added_monitors!(nodes[0], 1);
        let mut msg_events = nodes[0].node.get_and_clear_pending_msg_events();
        assert_eq!(msg_events.len(), 1);
...
fn remove_pending_outbound_probe_on_buggy_path() {
    ...
    assert_eq!(
        nodes[0]
            .node
            .send_probe(route.paths.pop().unwrap())
            .unwrap_err(),
...
fn max_out_mpp_path() {
    ...
    nodes[0]
        .node
        .pay_for_bolt11_invoice(&invoice, id, None, route_params_cfg, retry)
        .unwrap();
