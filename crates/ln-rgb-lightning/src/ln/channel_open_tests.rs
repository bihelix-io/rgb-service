...
fn test_outbound_chans_unlimited() {
    ...
    nodes[0]
        .node
        .create_channel(node_b, 100_000, 0, 42, None, None)
        .unwrap();
    ...
    nodes[1]
        .node
        .create_channel(node_a, 100_000, 0, 42, None, None)
        .unwrap();
...
fn test_0conf_limiting() {
    ...
    for _ in 0..MAX_UNFUNDED_CHANNEL_PEERS - 1 {
        ...
        nodes[1]
            .node
            .peer_connected(random_pk, init_msg, true)
            .unwrap();

        nodes[1]
            .node
            .handle_open_channel(random_pk, &open_channel_msg);
    ...
    nodes[1]
        .node
        .peer_connected(last_random_pk, init_msg, true)
        .unwrap();
    nodes[1]
        .node
        .handle_open_channel(last_random_pk, &open_channel_msg);
    let events = nodes[1].node.get_and_clear_pending_events();
    match events[0] {
        Event::OpenChannelRequest {
            temporary_channel_id,
...
fn test_inbound_anchors_manual_acceptance_overridden() {
    ...
    assert_eq!(
        accept_message.common_fields.max_htlc_value_in_flight_msat,
        5_000_000
    );
...
fn do_test_manual_inbound_accept_with_override(
    ...
    let node_chanmgrs = create_node_chanmgrs(
        ...
        &node_cfgs,
        &[
            Some(start_cfg.clone()),
            Some(start_cfg.clone()),
            Some(mannual_accept_cfg.clone()),
        ],
    );
...
fn do_test_channel_type_downgrade(
    initiator_cfg: UserConfig,
    acceptor_cfg: UserConfig,
    start_type: ChannelTypeFeatures,
    ...
    nodes[0]
        .node
        .create_channel(node_b, 100_000, 0, 0, None, None)
        .unwrap();
    let mut open_channel_msg = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, node_b);
    assert_eq!(
        open_channel_msg
    ...
    for downgrade_type in downgrade_types {
        ...
        let channel_type = open_channel_msg
            .common_fields
            .channel_type
            .as_ref()
            .unwrap();
...
fn test_channel_resumption_fail_post_funding() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 1_000_000, 0, 42, None, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .funding_transaction_generated(temp_chan_id, node_b_id, tx)
        .unwrap();
...
pub fn test_insane_channel_opens() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, channel_value_sat, push_msat, 42, None, None)
        .unwrap();
    ...
    let insane_open_helper =
        |expected_error_str: &str, message_mutator: fn(msgs::OpenChannel) -> msgs::OpenChannel| {
            ...
            nodes[1]
                .node
                .handle_open_channel(node_a_id, &open_channel_mutated);
            let msg_events = nodes[1].node.get_and_clear_pending_msg_events();
    ...
    insane_open_helper(
        "max_accepted_htlcs was 484. It must not be larger than 483",
        |mut msg| {
...
fn test_insane_zero_fee_channel_open() {
    ...
    insane_open_helper(
        "Zero Fee Channels must never attempt to use a fee".into(),
        |mut msg| {
...
pub fn bolt2_open_channel_sending_node_checks_part1() {
    ...
    nodes[1]
        .node
        .handle_open_channel(node_a_id, &node0_to_1_send_open_channel);
...
pub fn bolt2_open_channel_sending_node_checks_part2() {
    ...
    assert_eq!(
        node0_to_1_send_open_channel.common_fields.chain_hash,
        chain_hash
    );
}
...
pub fn bolt2_open_channel_sane_dust_limit() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, value_sats, push_msat, 42, None, None)
        .unwrap();
    let mut node0_to_1_send_open_channel =
        get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, node_b_id);
    node0_to_1_send_open_channel
        .common_fields
        .dust_limit_satoshis = 547;
...
pub fn test_user_configurable_csv_delay() {
    ...
    nodes[1]
        .node
        .create_channel(node_a_id, 1000000, 1000000, 42, None, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 1000000, 1000000, 42, None, None)
        .unwrap();
    let open_channel = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, node_b_id);
    nodes[1].node.handle_open_channel(node_a_id, &open_channel);
...
pub fn test_manually_accept_inbound_channel_request() {
    ...
    nodes[0]
        .node
        .handle_channel_ready(node_b_id, &as_channel_ready);
...
pub fn test_can_not_accept_unknown_inbound_channel() {
    ...
    let api_res = nodes[0]
        .node
        .accept_inbound_channel(&unknown_channel_id, &node_b_id, 0, None);
    match api_res {
        Err(APIError::APIMisuseError { err }) => {
            assert_eq!(err, "No such channel awaiting to be accepted.");
        }
...
pub fn test_duplicate_temporary_channel_id_from_different_peers() {
    ...
    nodes[1]
        .node
        .create_channel(node_a_id, 100000, 10001, 42, None, None)
        .unwrap();
    ...
    nodes[2]
        .node
        .create_channel(node_a_id, 100000, 10001, 43, None, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .handle_open_channel(node_b_id, &open_chan_msg_chan_1_0);
    {
    ...
    nodes[0]
        .node
        .handle_open_channel(node_c_id, &open_chan_msg_chan_2_0);
    {
        let events = nodes[0].node.get_and_clear_pending_msg_events();
        assert_eq!(events.len(), 1);
        match &events[0] {
...
pub fn test_duplicate_funding_err_in_funding() {
    ...
    let real_chan_funding_txo = chain::transaction::OutPoint {
        txid: funding_tx.compute_txid(),
        index: 0,
    };
    assert_eq!(
        ChannelId::v1_from_funding_outpoint(real_chan_funding_txo),
        real_channel_id
    );
    ...
    nodes[2]
        .node
        .funding_transaction_generated(node_c_temp_chan_id, node_b_id, fund_tx)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_funding_created(node_c_id, &funding_created_msg);
...
pub fn test_duplicate_chan_id() {
    ...
    nodes[1].node.handle_open_channel(node_a_id, &open_chan_msg);
    {
        ...
        match events[0] {
            MessageSendEvent::HandleError {
                ...
                assert_eq!(
                    msg.channel_id,
                    open_chan_msg.common_fields.temporary_channel_id
                );
                assert_eq!(node_id, node_a_id);
            }
            _ => panic!("Unexpected event"),
    ...
    nodes[1]
        .node
        .handle_open_channel(node_a_id, &open_chan_2_msg);
    nodes[0].node.handle_accept_channel(
        node_b_id,
        &get_event_msg!(nodes[1], MessageSendEvent::SendAcceptChannel, node_a_id),
    );
    ...
    assert_eq!(
        nodes[0].tx_broadcaster.txn_broadcasted.lock().unwrap()[0],
        tx
    );
...
pub fn test_invalid_funding_tx() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 100_000, 10_000, 42, None, None)
        .unwrap();
...
pub fn test_coinbase_funding_tx() {
    ...
    nodes[0]
        .node
        .funding_transaction_generated(channel_id, node_b_id, tx.clone())
        .unwrap();
    check_added_monitors(&nodes[0], 0);
    let funding_created = get_event_msg!(nodes[0], MessageSendEvent::SendFundingCreated, node_b_id);
...
pub fn test_non_final_funding_tx() {
        ...
        .funding_transaction_generated(temp_channel_id, node_b_id, tx.clone())
    {
        Err(APIError::APIMisuseError { err }) => {
            assert_eq!(
                format!("Funding transaction absolute timelock is non-final"),
                err
            );
        }
        _ => panic!(),
...
pub fn test_batch_channel_open() {
    ...
    nodes[1]
        .node
        .handle_funding_created(node_a_id, &funding_created_msgs[0]);
    check_added_monitors(&nodes[1], 1);
    expect_channel_pending_event(&nodes[1], &node_a_id);
    ...
    nodes[2]
        .node
        .handle_funding_created(node_a_id, &funding_created_msgs[1]);
    ...
    nodes[0]
        .node
        .handle_funding_signed(node_c_id, &funding_signed_msg);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[0]
        .chain_monitor
        .complete_sole_pending_chan_update(&ChannelId::v1_from_funding_outpoint(OutPoint {
            txid: tx.compute_txid(),
            index: 1,
        }));
    let events = nodes[0].node.get_and_clear_pending_events();
...
pub fn test_close_in_funding_batch() {
    ...
    let funding_txo_1 = OutPoint {
        txid: tx.compute_txid(),
        index: 0,
    };
    let funding_txo_2 = OutPoint {
        txid: tx.compute_txid(),
        index: 1,
    };
    ...
    nodes[0]
        .node
        .force_close_broadcasting_latest_txn(&channel_id_1, &node_b_id, err)
        .unwrap();
...
pub fn test_funding_and_commitment_tx_confirm_same_block() {
    ...
    let node_chanmgrs = create_node_chanmgrs(
        ...
        &node_cfgs,
        &[
            Some(min_depth_1_block_cfg.clone()),
            Some(min_depth_1_block_cfg),
        ],
    );
...
pub fn test_accept_inbound_channel_errors_queued() {
    ...
    match events[0] {
        Event::OpenChannelRequest {
            ...
            match nodes[1]
                .node
                .accept_inbound_channel(&temporary_channel_id, &node_a_id, 23, None)
            {
...
pub fn test_manual_funding_abandon() {
    ...
    let funding_signed = get_event_msg!(nodes[1], MessageSendEvent::SendFundingSigned, node_a_id);
    let err = msgs::ErrorMessage {
        channel_id: funding_signed.channel_id,
        data: "".to_string(),
    };
    ...
    assert!(close_events
        .iter()
        .any(|ev| matches!(ev, Event::ChannelClosed { .. })));
...
fn test_fund_pending_channel() {
    ...
    let res = nodes[0]
        .node
        .batch_funding_transaction_generated(&pending_chan, tx);
