...
use crate::rgb::{ContractId, RgbClaimPurpose, RgbClaimableOutpoint, RgbFundingRef};
...
const RGB_COMMITMENT_OP_RETURN_WEIGHT: u64 = (8 + 1 + 2 + 32) * 4;
...
#[rustfmt::skip]
pub(crate) fn commit_tx_fee_sat_with_rgb(feerate_per_kw: u32, num_htlcs: usize, channel_type_features: &ChannelTypeFeatures, with_rgb_commitment: bool) -> u64 {
	let rgb_weight = if with_rgb_commitment { RGB_COMMITMENT_OP_RETURN_WEIGHT } else { 0 };
	feerate_per_kw as u64 *
		(commitment_tx_base_weight(channel_type_features) + rgb_weight +
...
pub fn build_closing_transaction(to_holder_value_sat: Amount, to_counterparty_value_sat: Amount, to_holder_script: ScriptBuf, to_counterparty_script: ScriptBuf, funding_outpoint: OutPoint) -> Transaction {
	build_closing_transaction_with_rgb(to_holder_value_sat, to_counterparty_value_sat, None, to_holder_script, to_counterparty_script, funding_outpoint, None)
}

fn serialize_funding_rgb_transfer(funding_rgb_transfer: Option<&RgbFundingRef>) -> Option<Vec<u8>> {
    funding_rgb_transfer.map(|transfer| {
        let mut buf = Vec::new();
        transfer
            .write(&mut buf)
            .expect("RgbFundingRef serialization failed");
        buf
    })
}

fn deserialize_funding_rgb_transfer(serialized: Option<&Vec<u8>>) -> Option<RgbFundingRef> {
    serialized.map(|serialized| {
        let mut cursor = bitcoin::io::Cursor::new(serialized.clone());
        Readable::read(&mut cursor).expect("RgbFundingRef deserialization failed")
    })
}

/// Build a closing transaction carrying an RGB commitment.
#[rustfmt::skip]
pub fn build_closing_transaction_with_rgb(to_holder_value_sat: Amount, to_counterparty_value_sat: Amount, to_each_value_rgb: Option<(ContractId, u64, u64)>, to_holder_script: ScriptBuf, to_counterparty_script: ScriptBuf, funding_outpoint: OutPoint, funding_rgb_transfer: Option<RgbFundingRef>) -> Transaction {
	let txins = {
	...
	let mut to_counterparty_value_vout: Option<u32> = None;
	let mut to_holder_value_vout: Option<u32> = None;
	let mut opret_vout: Option<u32> = None;

	let mut txouts: Vec<(TxOut, &mut Option<u32>)> = Vec::new();

	if to_each_value_rgb.is_some() {
		txouts.push((TxOut {
	...
	if to_counterparty_value_sat > Amount::ZERO {
		...
		}, &mut to_counterparty_value_vout));
	}
	...
	if let Some((contract_id, to_holder_value_rgb, to_counterparty_value_rgb)) = to_each_value_rgb {
		let funding_ref = funding_rgb_transfer.expect("RGB closing transaction requires daemon funding reference");
		let change_vout = to_holder_value_vout.or(to_counterparty_value_vout)
			.expect("RGB closing transaction requires a non-zero BTC output for RGB change anchoring");
		closing_tx = crate::rgb::get_rgb_ln_tx_composer()
			.compose_closing(crate::rgb::RgbLnClosingComposeRequest {
				funding_ref,
				unsigned_tx: closing_tx,
				funding_outpoint,
				contract_id,
				to_holder_rgb: to_holder_value_rgb,
				to_holder_vout: to_holder_value_vout,
				to_counterparty_rgb: to_counterparty_value_rgb,
				to_counterparty_vout: to_counterparty_value_vout,
				change_vout,
			})
			.expect("RGB daemon LN closing compose failed");
	}
...
pub struct HTLCOutputInCommitment {
    ...
    pub amount_msat: u64,
    /// Optional RGB asset amount carried by this HTLC.
...
impl HTLCOutputInCommitment {
    ...
    pub(crate) fn is_data_equal(&self, other: &HTLCOutputInCommitment) -> bool {
        self.offered == other.offered
            ...
            && self.amount_rgb == other.amount_rgb
...
pub fn build_htlc_transaction(
    ...
    let txins = vec![build_htlc_input(
        commitment_txid,
        htlc,
...
impl ChannelTransactionParameters {
    ...
    pub fn counterparty_pubkeys(&self) -> Option<&ChannelPublicKeys> {
        self.counterparty_parameters
            .as_ref()
            .map(|params| &params.pubkeys)
    }
...
pub struct ClosingTransaction {
    ...
    to_each_value_rgb: Option<(ContractId, u64, u64)>,
    to_holder_script: ScriptBuf,
    to_counterparty_script: ScriptBuf,
    funding_rgb_transfer: Option<Vec<u8>>,
    built: Transaction,
}
...
impl ClosingTransaction {
    ...
    /// Construct a closing transaction carrying RGB balances.
	#[rustfmt::skip]
    pub fn new_with_rgb(
		to_holder_value_sat: u64,
		to_counterparty_value_sat: u64,
		...
		to_counterparty_script: ScriptBuf,
		funding_outpoint: OutPoint,
		funding_rgb_transfer: Option<RgbFundingRef>,
	) -> Self {
		...
		let serialized_funding_rgb_transfer = serialize_funding_rgb_transfer(funding_rgb_transfer.as_ref());
		let built = build_closing_transaction_with_rgb(
		...
		ClosingTransaction {
			...
			to_holder_script,
			to_counterparty_script,
			funding_rgb_transfer: serialized_funding_rgb_transfer,
			built
		}
	}
    ...
    pub fn verify(&self, funding_outpoint: OutPoint) -> Result<TrustedClosingTransaction<'_>, ()> {
		let funding_rgb_transfer = deserialize_funding_rgb_transfer(self.funding_rgb_transfer.as_ref());
		let built = build_closing_transaction_with_rgb(
			self.to_holder_value_sat, self.to_counterparty_value_sat,
			self.to_each_value_rgb,
...
pub struct CommitmentTransaction {
    ...
    keys: TxCreationKeys,
    funding_rgb_transfer: Option<Vec<u8>>,
    rgb_claimable_outpoints: Vec<RgbClaimableOutpoint>,
...
impl PartialEq for CommitmentTransaction {
    ...
    fn eq(&self, o: &Self) -> bool {
		let eq = self.commitment_number == o.commitment_number &&
			...
			self.to_each_value_rgb == o.to_each_value_rgb &&
			self.feerate_per_kw == o.feerate_per_kw &&
			self.nondust_htlcs == o.nondust_htlcs &&
			self.channel_type_features == o.channel_type_features &&
			self.keys == o.keys &&
			self.funding_rgb_transfer == o.funding_rgb_transfer &&
			self.rgb_claimable_outpoints == o.rgb_claimable_outpoints;
		if eq {
...
impl Writeable for CommitmentTransaction {
    ...
    fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		write_tlv_fields!(writer, {
			...
			(5, self.to_each_value_rgb, option),
			...
			(17, self.funding_rgb_transfer, option),
			(18, self.rgb_claimable_outpoints, optional_vec),
		});
		Ok(())
	}
}
...
impl Readable for CommitmentTransaction {
    ...
    fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		_init_and_read_len_prefixed_tlv_fields!(reader, {
			...
			(17, funding_rgb_transfer, option),
			(18, rgb_claimable_outpoints, optional_vec),
		});
		...
		Ok(Self {
			...
			keys: keys.0.unwrap(),
			funding_rgb_transfer,
			rgb_claimable_outpoints: rgb_claimable_outpoints.unwrap_or_default(),
...
impl CommitmentTransaction {
    ...
    /// Constructs a new `CommitmentTransaction` with optional RGB direct balances and HTLC amounts.
	#[rustfmt::skip]
    pub fn new_with_rgb(commitment_number: u64, per_commitment_point: &PublicKey, to_broadcaster_value_sat: u64, to_countersignatory_value_sat: u64, to_each_value_rgb: Option<(ContractId, u64, u64)>, feerate_per_kw: u32, mut nondust_htlcs: Vec<HTLCOutputInCommitment>, channel_parameters: &DirectedChannelTransactionParameters, secp_ctx: &Secp256k1<secp256k1::All>, funding_rgb_transfer: Option<RgbFundingRef>) -> CommitmentTransaction {
		...
		let with_opret = to_each_value_rgb.is_some();
		let serialized_funding_rgb_transfer = serialize_funding_rgb_transfer(funding_rgb_transfer.as_ref());
		...
		let outputs = Self::build_outputs_and_htlcs(&keys, to_broadcaster_value_sat, to_countersignatory_value_sat, &mut nondust_htlcs, channel_parameters, with_opret);
		let mut rgb_claim_templates = Vec::new();
		if let Some((contract_id, to_broadcaster_value_rgb, to_countersignatory_value_rgb)) = to_each_value_rgb {
			if to_broadcaster_value_rgb > 0 {
				let vout = Self::find_broadcaster_output_index(&keys, channel_parameters, &outputs)
					.expect("RGB broadcaster balance requires a broadcaster commitment output");
				rgb_claim_templates.push((vout, contract_id, to_broadcaster_value_rgb, RgbClaimPurpose::ToBroadcaster));
			}
			if to_countersignatory_value_rgb > 0 {
				let vout = Self::find_countersignatory_output_index(channel_parameters, &outputs)
					.expect("RGB countersignatory balance requires a countersignatory commitment output");
				rgb_claim_templates.push((vout, contract_id, to_countersignatory_value_rgb, RgbClaimPurpose::ToCountersignatory));
			}
			for htlc in &nondust_htlcs {
				if let Some((cid, amount_rgb)) = htlc.amount_rgb {
					assert_eq!(cid, contract_id);
					if amount_rgb > 0 {
						let vout = htlc.transaction_output_index.expect("RGB HTLC output missing");
						rgb_claim_templates.push((vout, cid, amount_rgb, RgbClaimPurpose::Htlc));
					}
				}
			}
		}
		...
		let transaction = Self::make_transaction(
			obscured_commitment_transaction_number, txins, outputs, channel_parameters,
			to_each_value_rgb, &keys, &nondust_htlcs, funding_rgb_transfer
		);
		let txid = transaction.compute_txid();
		let rgb_claimable_outpoints = rgb_claim_templates.into_iter()
			.map(|(vout, contract_id, amount_rgb, purpose)| RgbClaimableOutpoint {
    ...
    fn rebuild_transaction(&self, keys: &TxCreationKeys, channel_parameters: &DirectedChannelTransactionParameters) -> Result<BuiltCommitmentTransaction, ()> {
		...
		let mut rgb_htlcs = self.nondust_htlcs.clone();
		if self.to_each_value_rgb.is_some() {
			outputs.insert(0, TxOut {
				script_pubkey: ScriptBuf::new_op_return(&[0u8; 32]),
				value: Amount::ZERO,
			});
			for htlc in &mut rgb_htlcs {
				if let Some(vout) = htlc.transaction_output_index.as_mut() {
					*vout += 1;
				}
			}
		}
		...
		let transaction = Self::make_transaction(
			obscured_commitment_transaction_number, txins, outputs, channel_parameters,
			self.to_each_value_rgb, keys, &rgb_htlcs, funding_rgb_transfer
		);
		let txid = transaction.compute_txid();
    ...
    fn make_transaction(
		...
		if let Some((contract_id, to_broadcaster_value_rgb, to_countersignatory_value_rgb)) = to_each_value_rgb {
			let funding_ref = funding_rgb_transfer.expect("RGB commitment transaction requires daemon funding reference");
			let broadcaster_vout = Self::find_broadcaster_output_index(keys, channel_parameters, &commitment_tx.output);
			let countersignatory_vout = Self::find_countersignatory_output_index(channel_parameters, &commitment_tx.output);
			let mut htlcs = Vec::new();
			for htlc in nondust_htlcs {
				if let Some((cid, amount_rgb)) = htlc.amount_rgb {
					assert_eq!(cid, contract_id);
					let vout = htlc.transaction_output_index.expect("RGB HTLC output missing");
					htlcs.push(crate::rgb::RgbLnHtlcOutput { vout, amount_rgb });
				}
			}
			let change_vout = broadcaster_vout.or(countersignatory_vout)
				.or_else(|| htlcs.first().map(|htlc| htlc.vout))
				.expect("RGB commitment transaction requires a non-zero BTC output for RGB change anchoring");
			let funding_outpoint = commitment_tx.input[0].previous_output;
			commitment_tx = crate::rgb::get_rgb_ln_tx_composer()
				.compose_commitment(crate::rgb::RgbLnCommitmentComposeRequest {
					funding_ref,
					unsigned_tx: commitment_tx,
					funding_outpoint,
					contract_id,
					to_broadcaster_rgb: to_broadcaster_value_rgb,
					to_broadcaster_vout: broadcaster_vout,
					to_countersignatory_rgb: to_countersignatory_value_rgb,
					to_countersignatory_vout: countersignatory_vout,
					htlcs,
					change_vout,
				})
				.expect("RGB daemon LN commitment compose failed");
		}

		commitment_tx
	}

    fn find_broadcaster_output_index(
        keys: &TxCreationKeys,
        channel_parameters: &DirectedChannelTransactionParameters,
        outputs: &[TxOut],
    ) -> Option<u32> {
    ...
    fn find_countersignatory_output_index(
        channel_parameters: &DirectedChannelTransactionParameters,
        outputs: &[TxOut],
    ) -> Option<u32> {
        let payment_point = &channel_parameters.countersignatory_pubkeys().payment_point;
        let script_pubkey = if channel_parameters
            .channel_type_features()
            .supports_anchors_zero_fee_htlc_tx()
        {
            get_to_countersigner_keyed_anchor_redeemscript(payment_point).to_p2wsh()
        } else {
            ScriptBuf::new_p2wpkh(&Hash160::hash(&payment_point.serialize()).into())
        };
        outputs
            .iter()
            .position(|output| output.script_pubkey == script_pubkey)
    ...
    fn build_outputs_and_htlcs(
		...
		if with_opret {
			let idx = match outputs.binary_search_by(|output| output.value.cmp(&Amount::ZERO).then(output.script_pubkey.cmp(&ScriptBuf::new_op_return(&[0u8; 32])))) {
				Ok(i) => i,
				Err(i) => i,
			};
			outputs.insert(idx, TxOut {
				script_pubkey: ScriptBuf::new_op_return(&[0u8; 32]),
				value: Amount::ZERO,
			});
			nondust_htlcs
    ...
    pub(crate) fn to_each_value_rgb(&self) -> Option<(ContractId, u64, u64)> {
        self.to_each_value_rgb
    }

    pub(crate) fn rgb_claimable_outpoints(&self) -> &[RgbClaimableOutpoint] {
        &self.rgb_claimable_outpoints
    }
...
mod tests {
    ...
    fn test_per_commitment_storage() {
...

        {
            ...
            monitor
                .provide_secret(281474976710655, secrets.last().unwrap().clone())
                .unwrap();
            test_secrets!();
            ...
            secrets.last_mut().unwrap()[0..32].clone_from_slice(&<Vec<u8>>::from_hex(hex).unwrap());
            monitor
                .provide_secret(281474976710653, secrets.last().unwrap().clone())
                .unwrap();
            test_secrets!();
            ...
            secrets.last_mut().unwrap()[0..32].clone_from_slice(&<Vec<u8>>::from_hex(hex).unwrap());
            monitor
                .provide_secret(281474976710651, secrets.last().unwrap().clone())
                .unwrap();
            test_secrets!();
            ...
            secrets.last_mut().unwrap()[0..32].clone_from_slice(&<Vec<u8>>::from_hex(hex).unwrap());
            monitor
                .provide_secret(281474976710649, secrets.last().unwrap().clone())
                .unwrap();
            test_secrets!();
