...
fn test_priv_forwarding_rejection() {
    ...
    nodes[0]
        .node
        .peer_connected(node_b_id, &bs_init_msg, true)
        .unwrap();
    ...
    nodes[1]
        .node
        .peer_connected(node_a_id, &as_init_msg, false)
        .unwrap();
    ...
    nodes[1]
        .node
        .peer_connected(node_c_id, &cs_init_msg, true)
        .unwrap();
    nodes[2]
        .node
        .peer_connected(node_b_id, &bs_init_msg, false)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(node_c_id, &cs_reestablish);
...
fn do_test_1_conf_open(connect_style: ConnectStyle) {
    ...
    *nodes[0].connect_style.borrow_mut() = connect_style;
    ...
    let as_channel_ready = if let MessageSendEvent::SendChannelReady {
        ref node_id,
        ref msg,
    } = as_msg_events[0]
    {
    ...
    if let MessageSendEvent::SendChannelUpdate {
        ref node_id,
        msg: _,
    } = as_msg_events[1]
    {
    ...
    let (bs_announcement, bs_update) = if let MessageSendEvent::BroadcastChannelAnnouncement {
        ref msg,
        ref update_msg,
    ...
    let (announcement, as_update) = if let MessageSendEvent::BroadcastChannelAnnouncement {
        ref msg,
        ref update_msg,
...
fn test_routed_scid_alias() {
    ...
    assert_eq!(
        ...
        last_hop[0].inbound_scid_alias.unwrap()
    );
    ...
    assert_eq!(
        updated_channel_info[0].inbound_scid_alias.unwrap(),
        0xdeadbeef
    );
...
fn test_scid_privacy_on_pub_channel() {
    ...
    assert!(!open_channel
        ...
        .as_ref()
        .unwrap()
        .supports_scid_privacy()); // we ignore `negotiate_scid_privacy` on pub channels
    open_channel
        .common_fields
        .channel_type
        .as_mut()
...
fn test_scid_privacy_negotiation() {
    ...
    nodes[1]
        .node
        .handle_open_channel(node_a_id, &second_open_channel);
...
fn test_inbound_scid_privacy() {
    ...
    nodes[1]
        .node
        .handle_accept_channel(node_c_id, &accept_channel);
    ...
    assert_eq!(
        route_2.paths[0].hops[1].short_channel_id,
        last_hop[0].short_channel_id.unwrap()
    );
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_2, payment_hash_2, onion, id)
        .unwrap();
    check_added_monitors!(nodes[0], 1);
...
fn test_scid_alias_returned() {
    ...
    assert_eq!(
        route.paths[0].hops[1].short_channel_id,
        nodes[2].node.list_usable_channels()[0]
            .inbound_scid_alias
            .unwrap()
    );
...
fn test_0conf_channel_with_async_monitor() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 100000, 10001, 42, None, Some(chan_config))
        .unwrap();
    ...
    nodes[1]
        .chain_monitor
        .complete_sole_pending_chan_update(&channel_id);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &as_send.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_send.commitment_msg);
    check_added_monitors!(nodes[1], 1);
    ...
    chanmon_cfgs[1]
        ...
        .set_update_ret(ChannelMonitorUpdateStatus::Completed);
    let (_, latest_update) = {
        let latest_monitor_update_id = nodes[1]
            .chain_monitor
    ...
    nodes[2]
        .node
        .handle_update_add_htlc(node_b_id, &bs_send.msgs[0]);
...
fn test_0conf_close_no_early_chan_update() {
    ...
    nodes[0]
        .node
        .force_close_all_channels_broadcasting_latest_txn(message.clone());
    check_added_monitors!(nodes[0], 1);
...
fn test_public_0conf_channel() {
    ...
    nodes[1]
        .node
        .handle_announcement_signatures(node_a_id, &as_announcement_sigs);
    nodes[0]
        .node
        .handle_announcement_signatures(node_b_id, &bs_announcement_sigs);
...
fn test_0conf_channel_reorg() {
    ...
    let real_scid = nodes[0].node.list_usable_channels()[0]
        .short_channel_id
        .unwrap();
    assert_eq!(
        nodes[1].node.list_usable_channels()[0]
            .short_channel_id
            .unwrap(),
        real_scid
    );
...
fn test_connect_before_funding() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 100_000, 10_001, 42, None, None)
        .unwrap();
    let open_channel = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, node_b_id);
