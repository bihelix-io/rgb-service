...
fn do_test_get_initial_channel_type<F1, F2>(
    ...
    start_features: InitFeatures,
    start_type: ChannelTypeFeatures,
    mut local_cfg_mod: F1,
    mut remote_features_mod: F2,
    ...
    F2: FnOnce(&mut InitFeatures),
{
    ...
    assert_eq!(
        get_initial_channel_type(&config, &start_features),
        start_type
    );
    ...
    assert_eq!(
        get_initial_channel_type(&start_cfg, &their_features),
        start_type
    );
...
fn do_test_supports_channel_type(config: UserConfig, expected_channel_type: ChannelTypeFeatures) {
    ...
    let open_channel_msg = channel_a
        .get_open_channel(ChainHash::using_genesis_block(network), &&logger)
        .unwrap();
...
fn test_rejects_if_channel_type_not_set() {
    ...
    let mut open_channel_msg = channel_a
        .get_open_channel(ChainHash::using_genesis_block(network), &&logger)
        .unwrap();
