...
pub(super) fn get_payment_preimage(
    ...
    match Method::from_bits((metadata_bytes[0] & 0b1110_0000) >> METHOD_TYPE_OFFSET) {
        ...
        Err(other) => Err(APIError::APIMisuseError {
            err: format!("Unknown payment type: {}", other),
        }),
    }
}
...
fn derive_ldk_payment_preimage(
    ...
    if !fixed_time_eq(
        &payment_hash.0,
        &Sha256::hash(&decoded_payment_preimage).to_byte_array(),
    ) {
