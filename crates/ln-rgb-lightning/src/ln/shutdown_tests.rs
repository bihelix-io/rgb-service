...
fn test_lnd_bug_6039() {
    ...
    let err_msg = msgs::ErrorMessage {
        channel_id: chan.2,
        data: "link failed to shutdown".to_string(),
    };
...
fn shutdown_on_unfunded_channel() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 1_000_000, 100_000, 0, None, None)
        .unwrap();
...
fn expect_channel_shutdown_state_with_force_closure() {
    ...
    let reason_b = ClosureReason::HolderForceClosed {
...
fn updates_shutdown_wait() {
    ...
    let res = nodes[0]
        .node
        .send_payment_with_route(route_1, payment_hash, onion, id);
    unwrap_send_err!(nodes[0], res, true, APIError::ChannelUnavailable { .. }, {});

    let onion = RecipientOnionFields::secret_only(payment_secret);
    let res = nodes[1]
        .node
        .send_payment_with_route(route_2, payment_hash, onion, id);
...
fn do_htlc_fail_async_shutdown(blinded_recipient: bool) {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &updates_2.update_fail_htlcs[0]);
    commitment_signed_dance!(nodes[0], nodes[1], updates_2.commitment_signed, false, true);
...
fn do_test_shutdown_rebroadcast(recv_count: u8) {
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &node_0_reestablish);
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &node_1_reestablish);
    let node_0_2nd_shutdown = if recv_count > 0 {
        ...
        nodes[0]
            .node
            .handle_shutdown(node_b_id, &node_1_2nd_shutdown);
        node_0_2nd_shutdown
    } else {
    ...
    nodes[1]
        .node
        .handle_shutdown(node_a_id, &node_0_2nd_shutdown);
    ...
    nodes[1]
        .node
        .peer_connected(node_a_id, &init_msg, true)
        .unwrap();
    let node_1_2nd_reestablish = get_chan_reestablish_msgs!(nodes[1], nodes[0])
        .pop()
        .unwrap();
    nodes[0]
        .node
        .peer_connected(node_b_id, &init_msg, false)
        .unwrap();
    if recv_count == 0 {
        // If all closing_signeds weren't delivered we can just resume where we left off...
        let node_0_2nd_reestablish = get_chan_reestablish_msgs!(nodes[0], nodes[1])
            .pop()
            .unwrap();

        nodes[0]
            .node
            .handle_channel_reestablish(node_b_id, &node_1_2nd_reestablish);
        ...
        nodes[1]
            .node
            .handle_channel_reestablish(node_a_id, &node_0_2nd_reestablish);
        ...
        nodes[1]
            .node
            .handle_shutdown(node_a_id, &node_0_3rd_shutdown);
        assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());

        nodes[0]
            .node
            .handle_shutdown(node_b_id, &node_1_3rd_shutdown);

        nodes[1]
            .node
            .handle_closing_signed(node_a_id, &node_0_2nd_closing_signed);
        let node_1_closing_signed =
            get_event_msg!(nodes[1], MessageSendEvent::SendClosingSigned, node_a_id);
        nodes[0]
            .node
...
fn test_upfront_shutdown_script() {
    ...
    node_0_shutdown.scriptpubkey = Builder::new()
    ...
    nodes[2]
        .node
        .handle_shutdown(node_a_id, &node_0_orig_shutdown);
...
fn test_unsupported_anysegwit_shutdown_script() {
    ...
    *node_cfgs[1].override_init_features.borrow_mut() = Some(features);
    ...
    let supported_shutdown_script = chanmon_cfgs[1]
        .keys_manager
    ...
    chanmon_cfgs[1]
        .keys_manager
        .expect(OnGetShutdownScriptpubkey {
            returns: unsupported_shutdown_script.clone(),
        })
        .expect(OnGetShutdownScriptpubkey {
            returns: supported_shutdown_script,
        });
    ...
    match nodes[1].node.close_channel(&chan.2, &node_a_id) {
        Err(APIError::IncompatibleShutdownScript { script }) => {
            assert_eq!(
                script.into_inner(),
                unsupported_shutdown_script.clone().into_inner()
            );
        }
...
fn do_test_closing_signed_reinit_timeout(timeout_step: TimeoutStep) {
    ...
    if timeout_step != TimeoutStep::AfterShutdown {
        ...
        if timeout_step == TimeoutStep::NoTimeout {
            nodes[1]
                .node
                .handle_closing_signed(node_a_id, &node_0_2nd_closing_signed.1.unwrap());
            let reason_b = ClosureReason::CounterpartyInitiatedCooperativeClosure;
            check_closed_event!(nodes[1], 1, reason_b, [node_a_id], 100000);
        }
        let reason_a = ClosureReason::LocallyInitiatedCooperativeClosure;
...
fn do_simple_legacy_shutdown_test(high_initiator_fee: bool) {
    ...
    nodes[0]
        .node
        .handle_closing_signed(node_b_id, &node_1_closing_signed.unwrap());
...
fn simple_target_feerate_shutdown() {
    ...
    assert_eq!(
        node_0_closing_signed.fee_satoshis,
        node_1_closing_signed.fee_satoshis
    );
...
fn do_outbound_update_no_early_closing_signed(use_htlc: bool) {
    ...
    if use_htlc {
        ...
        nodes[1]
            .node
            .handle_update_fee(node_a_id, &updates.update_fee.unwrap());
    }
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &updates.commitment_signed);
    ...
    assert_eq!(nodes[0].node.get_and_clear_pending_msg_events(), Vec::new());
    let (latest_update, _) = {
        let latest_monitor_update_id = nodes[0]
            .chain_monitor
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(chan_id, latest_update);
    ...
    nodes[0]
        .node
        .handle_closing_signed(node_b_id, &bs_closing_signed);
    let (_, as_2nd_closing_signed) = get_closing_signed_broadcast!(nodes[0].node, node_b_id);
    nodes[1]
        .node
        .handle_closing_signed(node_a_id, &as_2nd_closing_signed.unwrap());
    let (_, node_1_none) = get_closing_signed_broadcast!(nodes[1].node, node_a_id);
    assert!(node_1_none.is_none());
...
fn batch_funding_failure() {
    ...
    nodes[0]
        .node
        .batch_funding_transaction_generated(&chans, tx)
        .unwrap_err();
...
fn test_force_closure_on_low_stale_fee() {
    ...
    connect_blocks(
        &nodes[1],
        super::channelmanager::FEERATE_TRACKING_BLOCKS as u32 * 2,
    );
    assert!(nodes[1].node.get_and_clear_pending_events().is_empty());
    ...
    check_closed_events(
        &nodes[1],
        &[ExpectedCloseEvent::from_id_reason(chan_id, false, reason)],
    );
}
...
fn test_pending_htlcs_arent_lost_on_mon_delay() {
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(node_c_id, claim.update_fulfill_htlcs.pop().unwrap());

    // Now, while sitting on the `PaymentSent` event, move the B <-> C channel forward until B is
    // just waiting on C's last RAA.
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_c_id, &claim.commitment_signed);
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[0]
        .node
        .send_payment_with_route(route_b, payment_hash_b, onion, id)
        .unwrap();
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &as_send.update_add_htlcs[0]);
    ...
    nodes[1]
        .node
        .force_close_broadcasting_latest_txn(&chan_id_bc, &node_c_id, message)
        .unwrap();
    check_closed_broadcast(&nodes[1], 1, true);
    check_added_monitors(&nodes[1], 1);
    ...
    assert!(events.iter().any(|ev| {
        if let Event::HTLCHandlingFailed {
            failure_type,
            failure_reason,
            ..
        } = ev
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(node_b_id, &failures.update_fail_htlcs[0]);
