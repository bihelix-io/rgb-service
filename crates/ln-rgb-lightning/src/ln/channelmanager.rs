...
impl PendingHTLCRouting {
    ...
    fn blinded_failure(&self) -> Option<BlindedFailure> {
        match self {
            Self::Forward {
                blinded: Some(BlindedForward { failure, .. }),
                ..
            } => Some(*failure),
...
pub struct PendingHTLCInfo {
    ...
    /// RGB asset amount received in the incoming HTLC, if this is an RGB-aware payment.
    pub amount_rgb: Option<(ContractId, u64)>,
...
struct ClaimableHTLC {
    ...
    value: u64,
    /// The RGB amount of this MPP part
    value_rgb: u64,
    ...
    /// The RGB amount that the sender intended to be sent in this MPP part.
    sender_intended_rgb: u64,
    ...
    /// The total RGB amount received for a payment (sum of all MPP parts if the payment is a MPP).
    /// Gets set to the amount reported when pushing [`Event::PaymentClaimable`].
    total_rgb_received: Option<u64>,
    /// The sender intended sum total of all MPP parts specified in the onion
    total_msat: u64,
    /// The sender intended RGB sum total of all MPP parts.
    total_rgb: Option<(ContractId, u64)>,
...
impl Ord for ClaimableHTLC {
    fn cmp(&self, other: &ClaimableHTLC) -> cmp::Ordering {
        ...
        if res.is_eq() {
            debug_assert!(
                self == other,
                "ClaimableHTLCs from the same source should be identical"
            );
        }
        res
    }
...
pub(crate) enum SentHTLCId {
    ...
    OutboundRoute {
        session_priv: [u8; SECRET_KEY_SIZE],
    },
}
impl SentHTLCId {
    pub(crate) fn from_source(source: &HTLCSource) -> Self {
        match source {
            ...
            HTLCSource::OutboundRoute { session_priv, .. } => Self::OutboundRoute {
...
impl HTLCSource {
    ...
    pub(crate) fn possibly_matches_output(
        &self,
        htlc: &super::chan_utils::HTLCOutputInCommitment,
    ) -> bool {
...
impl MsgHandleErrInternal {
    ...
    fn from_chan_no_close(err: ChannelError, channel_id: ChannelId) -> Self {
        ...
        let err = match err {
            ...
            ChannelError::Abort(reason) => LightningError {
...
struct ClaimingPayment {
    amount_msat: u64,
    amount_rgb: Option<(ContractId, u64)>,
    payment_purpose: events::PaymentPurpose,
    receiver_node_id: PublicKey,
    htlcs: Vec<events::ClaimedHTLC>,
    sender_intended_value: Option<u64>,
    sender_intended_value_rgb: Option<(ContractId, u64)>,
...
impl_writeable_tlv_based!(ClaimingPayment, {
    ...
    (26233_1, amount_rgb, option),
    (26233_3, sender_intended_value_rgb, option),
});
...
impl ClaimablePayment {
    fn inbound_payment_id(&self, secret: &[u8; 32]) -> PaymentId {
        PaymentId::for_inbound_from_htlcs(
            ...
            self.htlcs
                .iter()
                .map(|htlc| (htlc.prev_hop.channel_id, htlc.prev_hop.htlc_id)),
        )
    }
			...
			Some(payment) => {
				...
				let claiming_payment = self.pending_claiming_payments
					...
					.or_insert_with(|| {
						...
						let sender_intended_value_rgb = payment.htlcs.first().and_then(|htlc| htlc.total_rgb);
						let amount_rgb = payment.htlcs.first().and_then(|htlc| {
							htlc.total_rgb.map(|(contract_id, _)| {
								(contract_id, payment.htlcs.iter().map(|source| source.value_rgb).sum())
							})
						});
    ...
    L::Target: Logger,
{
    ...
    /// RGB channel contexts keyed by temporary or final channel id.
    rgb_channel_contexts: Mutex<HashMap<ChannelId, crate::rgb::RgbContext>>,
    /// RGB funding daemon references attached to not-yet-funded RGB channels.
    rgb_funding_refs: Mutex<HashMap<ChannelId, crate::rgb::RgbFundingTransfer>>,
    /// RGB payment metadata keyed by payment id.
    rgb_payment_metadata: Mutex<HashMap<PaymentId, crate::rgb::RgbPaymentMetadata>>,
    /// RGB amounts keyed by payment hash after an inbound payment has been claimed.
    claimed_rgb_payments: Mutex<HashMap<PaymentHash, (ContractId, u64)>>,
...
macro_rules! emit_channel_pending_event {
    ($locked_events: expr, $channel: expr) => {
        if $channel.context.should_emit_channel_pending_event() {
            ...
            let funding_redeem_script = Some(
                $channel
                    .funding
    ...
    L::Target: Logger,
{
		...
		L: Clone,
	{
		...
		ChannelManager {
			...
			rgb_channel_contexts: Mutex::new(new_hash_map()),
			rgb_funding_refs: Mutex::new(new_hash_map()),
			rgb_payment_metadata: Mutex::new(new_hash_map()),
			claimed_rgb_payments: Mutex::new(new_hash_map()),
    ...
    /// Creates a new outbound channel carrying RGB funding intent.
    ///
    /// This is the `ln-rgb` fork entry point used by the wallet layer. It preserves the upstream
    /// channel-open behavior while storing the RGB channel context inside the fork, keyed by the
    /// temporary channel id returned by LDK.
	#[rustfmt::skip]
    pub fn create_rgb_channel(&self, their_network_key: PublicKey, channel_value_satoshis: u64, push_msat: u64, user_channel_id: u128, temporary_channel_id: Option<ChannelId>, override_config: Option<UserConfig>, rgb_context: crate::rgb::RgbChannelContext) -> Result<ChannelId, APIError> {
		let funding_rgb = Some((rgb_context.asset.contract_id, rgb_context.asset.amount));
		let stored_context = rgb_context.into_rgb_context(true);
		let channel_id = self.create_channel(their_network_key, channel_value_satoshis, push_msat, user_channel_id, temporary_channel_id, override_config)?;
		if let Some(peer_state_mutex) = self.per_peer_state.read().unwrap().get(&their_network_key) {
			let mut peer_state = peer_state_mutex.lock().unwrap();
			if let Some(channel) = peer_state.channel_by_id.get_mut(&channel_id) {
				channel.context_mut().rgb_context = Some(stored_context.clone());
			}
			for event in peer_state.pending_msg_events.iter_mut().rev() {
				match event {
					MessageSendEvent::SendOpenChannel { node_id, msg }
						if *node_id == their_network_key && msg.common_fields.temporary_channel_id == channel_id =>
					{
						msg.funding_rgb = funding_rgb;
						break;
					},
		...
		self.rgb_channel_contexts.lock().unwrap().insert(channel_id, stored_context);
		Ok(channel_id)
	}

    /// Returns the RGB channel context currently known for a channel id.
    pub fn rgb_channel_context(&self, channel_id: &ChannelId) -> Option<crate::rgb::RgbContext> {
        for peer_state_mutex in self.per_peer_state.read().unwrap().values() {
            let peer_state = peer_state_mutex.lock().unwrap();
            if let Some(channel) = peer_state.channel_by_id.get(channel_id) {
                if let Some(rgb_context) = channel.context().rgb_context.clone() {
                    return Some(rgb_context);
                }
            }
        }
        self.rgb_channel_contexts
            .lock()
            .unwrap()
            .get(channel_id)
            .cloned()
            ...
            hash_map::Entry::Occupied(mut chan_phase_entry) => {
                ...
                if let Some(chan) = chan_phase_entry.get_mut().as_funded_mut() {
                    ...
                    if let Some(msg) = msg_opt {
                        peer_state
                            .pending_msg_events
                            .push(MessageSendEvent::SendStfu {
                                node_id: *counterparty_node_id,
                                msg,
                            });
                    }
        ...
        match peer_state.channel_by_id.entry(*channel_id) {
            hash_map::Entry::Occupied(mut chan_phase_entry) => {
                ...
                if let Some(chan) = chan_phase_entry.get_mut().as_funded_mut() {
                    ...
                    peer_state
                        .pending_msg_events
                        .push(MessageSendEvent::SendTxAbort {
                            node_id: *counterparty_node_id,
                            msg: tx_abort,
                        });
        ...
        let err: Result<(), _> = loop {
            ...
            let (counterparty_node_id, id) = match first_chan {
                None => {
                    ...
                    log_error!(
                        logger,
                        "Failed to find first-hop for payment hash {payment_hash}"
                    );
                    return Err(APIError::ChannelUnavailable {
            ...
            if let hash_map::Entry::Occupied(mut chan_entry) = peer_state.channel_by_id.entry(id) {
                match chan_entry.get_mut().as_funded_mut() {
                    Some(chan) => {
                        ...
                        let htlc_rgb = {
                            let rgb_payment = self
                                .rgb_payment_metadata
                                .lock()
                                .unwrap()
                                .get(&payment_id)
                                .cloned();
                            if rgb_payment.is_some() && path.hops.len() != 1 {
                                return Err(APIError::APIMisuseError {
									err: "RGB Lightning payments currently require a direct one-hop path"
										.to_owned(),
								});
                            }
                            rgb_payment
                                .map(|metadata| (metadata.asset.contract_id, metadata.asset.amount))
                        };
    ...
    pub(crate) fn test_set_payment_metadata(
        &self,
        payment_id: PaymentId,
        new_payment_metadata: Option<Vec<u8>>,
    ) {
        self.pending_outbound_payments
            .test_set_payment_metadata(payment_id, new_payment_metadata);
    }
    ...
    fn abandon_payment_with_reason(&self, payment_id: PaymentId, reason: PaymentFailureReason) {
        let _persistence_guard = PersistenceNotifierGuard::notify_on_drop(self);
        self.pending_outbound_payments
            .abandon_payment(payment_id, reason, &self.pending_events);
    }
    ...
    /// Sends a spontaneous payment carrying RGB payment metadata.
    ///
    /// This delegates BTC HTLC construction to upstream LDK, then annotates the generated
    /// `update_add_htlc` messages with the RGB amount so RGB-aware peers can validate the asset
    /// transfer intent at the same wire boundary as the Lightning payment.
    pub fn send_rgb_spontaneous_payment(
        &self,
        payment_preimage: Option<PaymentPreimage>,
        recipient_onion: RecipientOnionFields,
        payment_id: PaymentId,
        ...
        let amount_rgb = Some((rgb_payment.asset.contract_id, rgb_payment.asset.amount));
        self.rgb_payment_metadata
            .lock()
            .unwrap()
            .insert(payment_id, rgb_payment.clone());
        let result = self.send_spontaneous_payment(
            payment_preimage,
            recipient_onion,
            payment_id,
            route_params,
        ...
        if let Ok(payment_hash) = result {
            for peer_state_mutex in self.per_peer_state.read().unwrap().values() {
                let mut peer_state = peer_state_mutex.lock().unwrap();
                for event in peer_state.pending_msg_events.iter_mut() {
                    if let MessageSendEvent::UpdateHTLCs { updates, .. } = event {
                        for update_add in updates.update_add_htlcs.iter_mut() {
                            if update_add.payment_hash == payment_hash {
                                update_add.amount_rgb = amount_rgb;
                            }
                        }
                    }
                }
    ...
    /// Returns RGB metadata for a payment id, if it has been registered.
    pub fn rgb_payment_metadata(
        &self,
        payment_id: &PaymentId,
    ) -> Option<crate::rgb::RgbPaymentMetadata> {
        self.rgb_payment_metadata
            .lock()
            .unwrap()
            .get(payment_id)
    ...
    pub fn inbound_rgb_payment_amount(
        ...
        if let Some(payment) = claimable_payments.claimable_payments.get(payment_hash) {
            return payment.htlcs.first().and_then(|htlc| {
                htlc.total_rgb.map(|(contract_id, _)| {
                    let amount = payment.htlcs.iter().map(|htlc| htlc.value_rgb).sum();
                    (contract_id, amount)
                })
            });
        }
        let pending_rgb = claimable_payments
            .pending_claiming_payments
            .get(payment_hash)
            .and_then(|payment| payment.amount_rgb);
        drop(claimable_payments);
        pending_rgb.or_else(|| {
            self.claimed_rgb_payments
                .lock()
                .unwrap()
                .get(payment_hash)
                .cloned()
    ...
    pub fn send_preflight_probes(
        ...
        let route = self
            ...
            .map_err(|e| {
                log_error!(
                    self.logger,
                    "Failed to find path for payment probe: {:?}",
                    e
                );
        ...
        for mut path in route.paths {
            ...
            if let Some(first_path_hop) = path.hops.first() {
                if let Some(first_hop) = first_hops.iter().find(|h| {
                    ...
                    let used_liquidity = used_liquidity_map
                        .entry(first_path_hop.short_channel_id)
                        .or_insert(0);
    ...
    /// Provides an RGB transfer payload for an unfunded outbound channel.
    ///
    /// The wallet layer validates and stages the RGB transfer before the BTC funding transaction is
    /// announced. This method stores the transfer in the fork and attaches it to the RGB channel
    /// context so later commitment/on-chain handling can recolor transactions from the same state.
    pub fn provide_funding_rgb_transfer_for_unfunded_channel(
        &self,
        temporary_channel_id: ChannelId,
        counterparty_node_id: PublicKey,
        rgb_transfer: crate::rgb::RgbFundingTransfer,
    ) -> Result<(), APIError> {
        let mut contexts = self.rgb_channel_contexts.lock().unwrap();
        let Some(context) = contexts.get_mut(&temporary_channel_id) else {
            return Err(APIError::APIMisuseError {
                err: format!(
                    "Channel with temporary channel id {temporary_channel_id} is not a RGB channel"
                ),
            });
        };
        context.provide_funding_ref(rgb_transfer.funding_ref.clone());
        if let Some(peer_state_mutex) = self
            .per_peer_state
            .read()
            .unwrap()
        ...
        {
            let mut peer_state = peer_state_mutex.lock().unwrap();
            if let Some(channel) = peer_state.channel_by_id.get_mut(&temporary_channel_id) {
                if let Some(rgb_context) = channel.context_mut().rgb_context.as_mut() {
                    rgb_context.provide_funding_ref(rgb_transfer.funding_ref.clone());
                }
            }
        }
        self.rgb_funding_refs
            .lock()
            .unwrap()
            .insert(temporary_channel_id, rgb_transfer);
        Ok(())
    }

    /// Provides an RGB transfer payload for an already-known channel.
    ///
    /// This is used during durable recovery when the wallet layer has the funding daemon reference on disk
    /// but the serialized channel state was restored before the transfer was attached. The channel
    /// may be addressed either by its final channel id or by the temporary channel id used during
    /// the open handshake.
    pub fn provide_funding_rgb_transfer_for_channel(
        &self,
        channel_id: ChannelId,
        counterparty_node_id: PublicKey,
        rgb_transfer: crate::rgb::RgbFundingTransfer,
    ) -> Result<(), APIError> {
        let mut attached_channel_id = None;
        if let Some(peer_state_mutex) = self
            .per_peer_state
            .read()
            .unwrap()
        ...
        {
            let mut peer_state = peer_state_mutex.lock().unwrap();
            for (candidate_channel_id, channel) in peer_state.channel_by_id.iter_mut() {
                if *candidate_channel_id != channel_id
                    && channel.context().temporary_channel_id() != Some(channel_id)
                {
                    continue;
                }
                let Some(rgb_context) = channel.context_mut().rgb_context.as_mut() else {
                    return Err(APIError::APIMisuseError {
                        err: format!("Channel {candidate_channel_id} is not a RGB channel"),
                    });
                };
                if rgb_context.funding_ref.is_none() {
                    rgb_context.provide_funding_ref(rgb_transfer.funding_ref.clone());
                }
                self.rgb_channel_contexts
                    .lock()
                    .unwrap()
                    .insert(*candidate_channel_id, rgb_context.clone());
                attached_channel_id = Some(*candidate_channel_id);
                break;
            }
        }

        let Some(attached_channel_id) = attached_channel_id else {
            return Err(APIError::APIMisuseError {
                err: format!(
                    "Channel with channel id or temporary channel id {channel_id} was not found"
                ),
            });
        };
        self.rgb_funding_refs
            .lock()
            .unwrap()
            .insert(attached_channel_id, rgb_transfer.clone());
        if attached_channel_id != channel_id {
            self.rgb_funding_refs
                .lock()
                .unwrap()
                .insert(channel_id, rgb_transfer);
        }
        Ok(())
    }

    /// Returns the RGB funding daemon reference registered for a channel id.
    pub fn rgb_funding_ref(
        &self,
        channel_id: &ChannelId,
    ) -> Option<crate::rgb::RgbFundingTransfer> {
        self.rgb_funding_refs
            .lock()
            .unwrap()
            .get(channel_id)
    ...
    /// Returns the RGB funding daemon reference registered for a channel id.
    ///
    /// Kept under the old name for callers that previously registered a funding transfer.
    /// The returned payload now carries only an opaque daemon reference, not RGB entity data.
    pub fn rgb_funding_transfer(
        &self,
        channel_id: &ChannelId,
    ) -> Option<crate::rgb::RgbFundingTransfer> {
        self.rgb_funding_ref(channel_id)
    }
    ...
    pub fn forward_intercepted_htlc(
        ...
        let outbound_scid_alias = {
            ...
            match peer_state.channel_by_id.get(next_hop_channel_id) {
                ...
                None => {
                    ...
                    log_error!(
                        logger,
                        "{error} when attempting to forward intercepted HTLC"
                    );
        ...
        let skimmed_fee_msat = payment
            .forward_info
            .outgoing_amt_msat
            .saturating_sub(amt_to_forward_msat);
        let pending_htlc_info = PendingHTLCInfo {
            skimmed_fee_msat: if skimmed_fee_msat == 0 {
                None
            } else {
                Some(skimmed_fee_msat)
            },
    ...
    fn process_pending_update_add_htlcs(&self) -> bool {
        ...
        mem::swap(
            ...
            &mut self.decode_update_add_htlcs.lock().unwrap(),
        );
        ...
        'outer_loop: for (incoming_scid_alias, update_add_htlcs) in decode_update_add_htlcs {
            ...
            for update_add_htlc in &update_add_htlcs {
                ...
                let outgoing_scid_opt =
                    next_packet_details_opt
                        .as_ref()
                        .and_then(|d| match d.outgoing_connector {
    ...
    pub fn process_pending_htlc_forwards(&self) {
        ...
        self.pending_htlc_forwards_processor
    ...
    fn forwarding_channel_not_found(
        &self,
        forward_infos: impl Iterator<Item = HTLCForwardInfo>,
        short_chan_id: u64,
        forwarding_counterparty: Option<PublicKey>,
    ...
    fn process_forward_htlcs(
        &self,
        short_chan_id: u64,
        pending_forwards: &mut Vec<HTLCForwardInfo>,
        failed_forwards: &mut Vec<FailedHTLCForward>,
        phantom_receives: &mut Vec<PerSourcePendingForward>,
    ) {
        let mut forwarding_counterparty = None;

        let chan_info_opt = self
            .short_to_chan_info
            .read()
            .unwrap()
            .get(&short_chan_id)
            ...
            match forward_info {
                HTLCForwardInfo::AddHTLC(payment) => {
                    ...
                    let claimable_htlc = ClaimableHTLC {
                        ...
                        value_rgb: amount_rgb.map(|(_, amount)| amount).unwrap_or(0),
                        sender_intended_value: outgoing_amt_msat,
                        sender_intended_rgb: amount_rgb.map(|(_, amount)| amount).unwrap_or(0),
                        timer_ticks: 0,
                        total_value_received: None,
                        total_rgb_received: None,
                        total_msat: if let Some(data) = &payment_data {
                            data.total_msat
                        } else {
                            outgoing_amt_msat
                        },
                        total_rgb: amount_rgb,
                        cltv_expiry,
                        onion_payload,
                    ...
                    macro_rules! check_total_value {
						($purpose: expr) => {{
							...
							for htlc in claimable_payment.htlcs.iter() {
								...
								if htlc.total_rgb != claimable_htlc.total_rgb {
									log_trace!(self.logger, "Failing HTLCs with payment_hash {} as the HTLCs had inconsistent RGB total values",
										&payment_hash);
									total_value = msgs::MAX_VALUE_MSAT;
								}
							...
							if total_value >= msgs::MAX_VALUE_MSAT {
								...
								if claimable_htlc.total_rgb.map(|(_, total)| total).unwrap_or(0) != total_rgb {
									fail_htlc!(claimable_htlc, payment_hash);
								}
								...
								let _amount_rgb = {
									let htlcs = &mut claimable_payment.htlcs;
									let contract_id = htlcs.first().and_then(|htlc| {
										htlc.total_rgb.map(|(contract_id, _)| contract_id)
									});
									let rgb_sum = htlcs.iter().map(|htlc| htlc.value_rgb).sum();
									htlcs.iter_mut().for_each(|htlc| htlc.total_rgb_received = Some(rgb_sum));
									contract_id.map(|contract_id| (contract_id, rgb_sum))
								};
    ...
    fn process_background_events(&self) -> NotifyOption {
        ...
        self.background_events_processed_since_startup
            .store(true, Ordering::Release);

        let mut background_events = Vec::new();
        mem::swap(
            &mut *self.pending_background_events.lock().unwrap(),
    ...
    pub fn timer_tick_occurred(&self) {
        PersistenceNotifierGuard::optionally_notify(self, || {
            ...
            self.claimable_payments
                .lock()
                .unwrap()
                .claimable_payments
                .retain(|payment_hash, payment| {
            ...
            for htlc_source in timed_out_mpp_htlcs.drain(..) {
                ...
                let reason = HTLCFailReason::from_failure_code(failure_reason);
                let receiver = HTLCHandlingFailureType::Receive {
                    payment_hash: htlc_source.1,
                };
    ...
    fn fail_holding_cell_htlcs(
        &self,
        mut htlcs_to_fail: Vec<(HTLCSource, PaymentHash)>,
        channel_id: ChannelId,
        counterparty_node_id: &PublicKey,
    ) {
            ...
            HTLCSource::OutboundRoute {
                ...
                if let Some(update) = from_monitor_update_completion {
                    ...
                    let have_action = {
                        ...
                        pending_events
                            .iter()
                            .any(|(_, act)| act.as_ref() == Some(&action))
                    };
                    if !have_action {
    ...
    fn claim_payment_internal(&self, payment_preimage: PaymentPreimage, custom_tlvs_known: bool) {
        ...
        if valid_mpp {
            ...
            let payment_info = Some(PaymentClaimDetails {
                mpp_parts,
                claiming_payment,
            });
            for htlc in sources {
    ...
    fn claim_funds_from_hop<
        ComplFunc: FnOnce(
            ...
            Option<MonitorUpdateCompletionAction>,
            Option<RAAMonitorUpdateBlockingAction>,
        ),
    >(
        &self,
        prev_hop: HTLCPreviousHopData,
        ...
        let counterparty_node_id = prev_hop.counterparty_node_id.or_else(|| {
            ...
            short_to_chan_info
                .get(&prev_hop.prev_outbound_scid_alias)
                .map(|(cp_id, _)| *cp_id)
        });
    ...
    >(
        ...
        debug_assert_ne!(
            self.claimable_payments.held_by_thread(),
            LockHeldState::HeldByThread
        );
        ...
        let update_id = if let Some(latest_update_id) = peer_state
...
This indicates a bug inside LDK. Please report this error at https://github.com/lightningdevkit/rust-lightning/issues/new";
    ...
    fn finalize_claims(&self, sources: Vec<(HTLCSource, Option<AttributionData>)>) {
        ...
        self.pending_outbound_payments
            .finalize_claims(hold_times, &self.pending_events);
    }
    ...
    fn claim_funds_internal(
        ...
        source: HTLCSource,
        payment_preimage: PaymentPreimage,
        forwarded_htlc_value_msat: Option<u64>,
        ...
        match source {
            HTLCSource::OutboundRoute {
                ...
                let have_action = if ev_completion_action.is_some() {
                    ...
                    pending_events
                        .iter()
                        .any(|(_, act)| *act == ev_completion_action)
                } else {
                    false
                };
                if !have_action {
			...
			match action {
				MonitorUpdateCompletionAction::PaymentClaimed { payment_hash, pending_mpp_claim } => {
					...
					if let Some(ClaimingPayment {
						...
						sender_intended_value_rgb: _sender_intended_total_rgb,
						onion_fields,
						payment_id,
						durable_preimage_channel,
					}) = payment {
						if let Some(amount_rgb) = amount_rgb {
							self.claimed_rgb_payments.lock().unwrap().insert(payment_hash, amount_rgb);
						}
    ...
    fn do_accept_inbound_channel(
			...
			Some(unaccepted_channel) => {
				...
				match unaccepted_channel.open_channel_msg {
					OpenChannelMessage::V1(open_channel_msg) => {
						let funding_rgb = open_channel_msg.funding_rgb;
						InboundV1Channel::new(
							...
							(*temporary_channel_id, Channel::from(channel), message_send_event, funding_rgb)
						})
					},
					OpenChannelMessage::V2(open_channel_msg) => {
						...
						PendingV2Channel::new_inbound(
							...
							(channel.context.channel_id(), Channel::from(channel), Some(message_send_event), funding_rgb)
						})
					},
				}
			},
		...
		let (channel_id, mut channel, message_send_event, funding_rgb) = match res {
			Ok(res) => res,
			Err(err) => {
				mem::drop(peer_state_lock);
				mem::drop(per_peer_state);
		...
		if let Some((contract_id, amount)) = funding_rgb {
			let rgb_context = crate::rgb::RgbContext::new_for_inbound_channel(contract_id, amount);
			channel.context_mut().rgb_context = Some(rgb_context.clone());
			self.rgb_channel_contexts
				.lock()
				.unwrap()
				.insert(channel.context().channel_id(), rgb_context);
		}
    ...
    fn internal_open_channel(&self, counterparty_node_id: &PublicKey, msg: OpenChannelMessageRef<'_>) -> Result<(), MsgHandleErrInternal> {
		...
		let funding_rgb = match msg {
			OpenChannelMessageRef::V1(msg) => msg.funding_rgb,
			OpenChannelMessageRef::V2(msg) => msg.funding_rgb,
		};
    ...
    fn internal_peer_storage_retrieval(
        ...
        let encrypted_ops = match EncryptedOurPeerStorage::new(msg.data) {
            ...
            Err(()) => {
                log_debug!(
                    logger,
                    "Received a peer backup which wasn't long enough to be valid"
                );
                return Err(err());
            }
        };
        ...
        log_trace!(
            logger,
            "Got valid {}-byte peer backup from {}",
            decrypted.len(),
            peer_node_id
        );
        let per_peer_state = self.per_peer_state.read().unwrap();
        ...
        let mon_list = <Vec<PeerStorageMonitorHolder> as Readable>::read(&mut cursor)
            .unwrap_or_else(|e| {
                ...
                log_debug!(
                    self.logger,
                    "Unable to unpack the retrieved peer storage {:?}",
                    e
                );
                Vec::new()
            });
    ...
    fn internal_tx_add_input(
        ...
        self.internal_tx_msg(
            &counterparty_node_id,
            msg.channel_id,
            |channel: &mut Channel<SP>| channel.tx_add_input(msg, &self.logger),
        )
    }
    ...
    fn internal_tx_add_output(
        ...
        self.internal_tx_msg(
            &counterparty_node_id,
            msg.channel_id,
            |channel: &mut Channel<SP>| channel.tx_add_output(msg, &self.logger),
        )
    }
    ...
    fn internal_tx_remove_input(
        ...
        self.internal_tx_msg(
            &counterparty_node_id,
            msg.channel_id,
            |channel: &mut Channel<SP>| channel.tx_remove_input(msg, &self.logger),
        )
    }
    ...
    fn internal_tx_remove_output(
        ...
        self.internal_tx_msg(
            &counterparty_node_id,
            msg.channel_id,
            |channel: &mut Channel<SP>| channel.tx_remove_output(msg, &self.logger),
        )
    }
    ...
    L::Target: Logger,
{
    ...
    pub fn set_paths_to_static_invoice_server(
        ...
        let peers = self.get_peers_for_blinded_path();
        self.flow
            .set_paths_to_static_invoice_server(paths_to_static_invoice_server, peers)?;
    ...
    pub fn pay_for_offer_from_human_readable_name(
        ...
        let (onion_message, context) =
            self.flow
                .hrn_resolver
                .resolve_name(payment_id, name, &*self.entropy_source)?;
    ...
    pub fn blinded_paths_for_async_recipient(
        ...
        let peers = self.get_peers_for_blinded_path();
        self.flow
            .blinded_paths_for_async_recipient(recipient_id, relative_expiry, peers)
    }
    ...
    L::Target: Logger,
{
    ...
    fn transaction_unconfirmed(&self, txid: &Txid) {
        ...
        self.do_chain_event(None, |channel| {
            let logger = WithChannelContext::from(&self.logger, &channel.context, None);
            channel
                .transaction_unconfirmed(txid, &&logger)
    ...
    fn message_received(&self) {
            ...
            .release_invoice_requests_awaiting_invoice()
        {
            ...
            let enqueue_invreq_res =
                self.flow
                    .enqueue_invoice_request(invoice_request, payment_id, nonce, peers);
    ...
    L::Target: Logger,
{
    fn handle_offer_paths_request(
        ...
        let peers = self.get_peers_for_blinded_path();
        let (message, reply_path_context) = match self
            .flow
            .handle_offer_paths_request(&message, context, peers)
        {
            Some(msg) => msg,
            None => return None,
        };
    ...
    fn handle_serve_static_invoice(
        ...
        let (recipient_id, invoice_slot) = match self
            .flow
            .verify_serve_static_invoice_message(&message, context)
        {
    ...
    fn handle_static_invoice_persisted(
        &self,
        _message: StaticInvoicePersisted,
        context: AsyncPaymentsContext,
    ) {
    ...
    fn handle_held_htlc_available(
        ...
        self.flow
            .verify_inbound_async_payment_context(context)
            .ok()?;
    ...
    fn handle_release_held_htlc(&self, _message: ReleaseHeldHtlc, context: AsyncPaymentsContext) {
        match context {
            ...
            AsyncPaymentsContext::ReleaseHeldHtlc {
                ...
                log_trace!(
                    logger,
                    "Releasing held htlc with intercept_id {}",
                    intercept_id
                );
...
pub fn provided_init_features(config: &UserConfig) -> InitFeatures {
    ...
    if config
        .channel_handshake_config
        .negotiate_anchor_zero_fee_commitments
    {
...
impl Writeable for ClaimableHTLC {
    fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
        ...
        write_tlv_fields!(writer, {
            ...
            (26233_1, self.value_rgb, required),
            (26233_3, self.sender_intended_rgb, required),
            (26233_5, self.total_rgb_received, option),
            (26233_7, self.total_rgb, option),
        });
        Ok(())
    }
}
...
impl Readable for ClaimableHTLC {
    ...
    fn read<R: Read>(reader: &mut R) -> Result<Self, DecodeError> {
		_init_and_read_len_prefixed_tlv_fields!(reader, {
			...
			(26233_1, value_rgb, (default_value, 0)),
			(26233_3, sender_intended_rgb, option),
			(26233_5, total_rgb_received, option),
			(26233_7, total_rgb, option),
		});
		...
		let value_rgb = value_rgb.0.unwrap();
		Ok(Self {
			...
			sender_intended_rgb: sender_intended_rgb.unwrap_or(value_rgb),
    ...
    L::Target: Logger,
{
    fn read<Reader: io::Read>(
        reader: &mut Reader,
        args: ChannelManagerReadArgs<'a, M, T, ES, NS, SP, F, R, MR, L>,
    ...
    L::Target: Logger,
{
    fn read<Reader: io::Read>(
        ...
        for _ in 0..pending_outbound_payments_count_compat {
            ...
            if pending_outbound_payments_compat
                .insert(PaymentId(session_priv), payment)
                .is_some()
            {
                return Err(DecodeError::InvalidValue);
            };
        ...
        if let Some(purposes) = claimable_htlc_purposes {
            ...
            if let Some(onion_fields) = claimable_htlc_onion_fields {
                ...
                for (purpose, (onion, (payment_hash, htlcs))) in purposes.into_iter().zip(
                    onion_fields
                        .into_iter()
        ...
        for (payment_hash, payment) in claimable_payments.iter() {
            for htlc in payment.htlcs.iter() {
                ...
                if short_to_chan_info
                    .get(&htlc.prev_hop.prev_outbound_scid_alias)
                    .is_some()
                {
        ...
        if let Some(network_pubkey) = received_network_pubkey {
            if network_pubkey != our_network_pubkey {
                log_error!(
                    args.logger,
                    "Key that was generated does not match the existing key."
                );
                return Err(DecodeError::InvalidValue);
            }
        }
            ...
            for (chan_id, chan) in peer_state.channel_by_id.iter_mut() {
                if let Some(funded_chan) = chan.as_funded_mut() {
                    ...
                    if funded_chan.context.outbound_scid_alias() == 0 {
                        ...
                        funded_chan
                            .context
                            .set_outbound_scid_alias(outbound_scid_alias);
                    ...
                    if let Some(payment) = payment {
                        ...
                        for claimable_htlc in &payment.htlcs {
                            ...
                            if let Some(previous_hop_monitor) = args
                                .channel_monitors
                                .get(&claimable_htlc.prev_hop.channel_id)
                            {
...
mod tests {
    ...
    #[test]
	#[rustfmt::skip]
	fn create_rgb_channel_writes_funding_rgb_to_open_channel() {
		let chanmon_cfgs = create_chanmon_cfgs(2);
		let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
		let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, None]);
		let nodes = create_network(2, &node_cfgs, &node_chanmgrs);

		let contract_id = [7; 32].into();
		let asset = crate::rgb::RgbAssetAmount::new(contract_id, 42);
		let channel_id = nodes[0].node.create_rgb_channel(
			nodes[1].node.get_our_node_id(), 100_000, 0, 42, None, None,
			crate::rgb::RgbChannelContext::new(asset),
		).unwrap();

		let open_channel_msg = get_event_msg!(
			nodes[0], MessageSendEvent::SendOpenChannel, nodes[1].node.get_our_node_id()
		);
		assert_eq!(open_channel_msg.funding_rgb, Some((contract_id, 42)));

		let outbound_rgb_context = nodes[0].node.rgb_channel_context(&channel_id).unwrap();
		assert_eq!(outbound_rgb_context.contract_id, contract_id);
		assert_eq!(outbound_rgb_context.funding_rgb, 42);
		assert_eq!(outbound_rgb_context.to_self, 42);

		nodes[1].node.handle_open_channel(nodes[0].node.get_our_node_id(), &open_channel_msg);
		let _accept_channel_msg = get_event_msg!(
			nodes[1], MessageSendEvent::SendAcceptChannel, nodes[0].node.get_our_node_id()
		);
		let inbound_rgb_context = nodes[1].node.rgb_channel_context(&open_channel_msg.common_fields.temporary_channel_id).unwrap();
		assert_eq!(inbound_rgb_context.contract_id, contract_id);
		assert_eq!(inbound_rgb_context.funding_rgb, 42);
		assert_eq!(inbound_rgb_context.to_self, 0);
	}

    #[test]
	#[rustfmt::skip]
	fn create_rgb_channel_manual_accept_preserves_funding_rgb() {
		do_create_rgb_channel_manual_accept_preserves_funding_rgb(100_000);
	}

    #[test]
	#[rustfmt::skip]
	fn create_edge_rgb_channel_manual_accept_returns_funding_event_or_error() {
		do_create_rgb_channel_manual_accept_preserves_funding_rgb(3_000);
	}

    #[rustfmt::skip]
	fn do_create_rgb_channel_manual_accept_preserves_funding_rgb(channel_value_satoshis: u64) {
		let chanmon_cfgs = create_chanmon_cfgs(2);
		let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
		let mut receiver_cfg = test_default_channel_config();
		receiver_cfg.manually_accept_inbound_channels = true;
		let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, Some(receiver_cfg)]);
		let nodes = create_network(2, &node_cfgs, &node_chanmgrs);
		...
		let asset = crate::rgb::RgbAssetAmount::new(contract_id, 42);
		let channel_id = nodes[0].node.create_rgb_channel(
			node_b_id, channel_value_satoshis, 0, 42, None, None,
			crate::rgb::RgbChannelContext::new(asset),
		).unwrap();
		...
		assert_eq!(events.len(), 1);
		match events[0] {
			Event::OpenChannelRequest { temporary_channel_id, counterparty_node_id, funding_satoshis, .. } => {
				assert_eq!(temporary_channel_id, open_channel_msg.common_fields.temporary_channel_id);
				assert_eq!(counterparty_node_id, node_a_id);
				assert_eq!(funding_satoshis, channel_value_satoshis);
		...
		let accept_channel_msg = get_event_msg!(
			nodes[1], MessageSendEvent::SendAcceptChannel, node_a_id
		);
		assert_eq!(accept_channel_msg.common_fields.minimum_depth, 0);
		...
		assert_eq!(channel_id, open_channel_msg.common_fields.temporary_channel_id);
		let events = nodes[0].node.get_and_clear_pending_events();
		assert_eq!(events.len(), 1);
		match events[0] {
			Event::FundingGenerationReady { temporary_channel_id, counterparty_node_id, channel_value_satoshis: event_channel_value_satoshis, user_channel_id, .. } => {
				assert_eq!(temporary_channel_id, channel_id);
				assert_eq!(counterparty_node_id, node_b_id);
				assert_eq!(event_channel_value_satoshis, channel_value_satoshis);
				assert_eq!(user_channel_id, 42);
			},
			_ => panic!("Unexpected event"),
		}
