...
pub fn complete_splice_handshake<'a, 'b, 'c, 'd>(
    ...
    acceptor
        .node
        .handle_splice_init(node_id_initiator, &splice_init);
    let splice_ack = get_event_msg!(acceptor, MessageSendEvent::SendSpliceAck, node_id_initiator);
    initiator
        .node
        .handle_splice_ack(node_id_acceptor, &splice_ack);
...
pub fn complete_interactive_funding_negotiation<'a, 'b, 'c, 'd>(
    ...
    loop {
        if !expected_initiator_inputs.is_empty() {
            let tx_add_input = get_event_msg!(
                initiator,
            ...
            acceptor
                .node
                .handle_tx_add_input(node_id_initiator, &tx_add_input);
        } else if !expected_initiator_scripts.is_empty() {
            let tx_add_output = get_event_msg!(
                initiator,
            ...
            acceptor
                .node
                .handle_tx_add_output(node_id_initiator, &tx_add_output);
        } else {
...
pub fn sign_interactive_funding_tx<'a, 'b, 'c, 'd>(
    ...
    acceptor
        .node
        .handle_commitment_signed(node_id_initiator, &initial_commit_sig_for_acceptor);
    ...
    if let MessageSendEvent::UpdateHTLCs { ref updates, .. } = &msg_events[0] {
        ...
        initiator
            .node
            .handle_commitment_signed(node_id_acceptor, commitment_signed);
    } else {
        panic!();
    }
    ...
    {
        let partially_signed_tx = initiator
            .wallet_source
            .sign_tx(unsigned_transaction)
            .unwrap();
        initiator
            .node
...
pub fn lock_splice<'a, 'b, 'c, 'd>(
    node_a: &'a Node<'b, 'c, 'd>,
    node_b: &'a Node<'b, 'c, 'd>,
    splice_locked_for_node_b: &msgs::SpliceLocked,
    is_0conf: bool,
) {
    ...
    node_b
        .node
        .handle_splice_locked(node_id_a, splice_locked_for_node_b);

    let mut msg_events = node_b.node.get_and_clear_pending_msg_events();
    assert_eq!(
        msg_events.len(),
        if is_0conf { 1 } else { 2 },
        "{msg_events:?}"
    );
...
fn do_test_splice_state_reset_on_disconnect(reload: bool) {
    ...
    nodes[1]
        .node
        .handle_tx_add_output(node_id_0, &tx_add_output);
    let tx_complete = get_event_msg!(nodes[1], MessageSendEvent::SendTxComplete, node_id_0);
    nodes[0].node.handle_tx_complete(node_id_1, &tx_complete);
...
fn do_test_splice_commitment_broadcast(splice_status: SpliceStatus, claim_htlcs: bool) {
    ...
    let htlc_claim_tx = if claim_htlcs {
        ...
        let tx_with_fee_bump_utxo = if splice_status == SpliceStatus::Unconfirmed {
            &coinbase_tx
        } else {
            &splice_tx
        };
    ...
    if claim_htlcs {
        assert_eq!(
            events
                .iter()
                .filter(|e| matches!(e, Event::PaymentSent { .. }))
                .count(),
            2
        );
        assert_eq!(
            events
                .iter()
                .filter(|e| matches!(e, Event::PaymentPathSuccessful { .. }))
                .count(),
            2
        );
    } else {
        assert_eq!(
            events
                .iter()
                .filter(|e| matches!(e, Event::PaymentFailed { .. }))
                .count(),
            2,
        );
        assert_eq!(
            events
                .iter()
                .filter(|e| matches!(e, Event::PaymentPathFailed { .. }))
                .count(),
            2
        );
    }
...
fn do_test_splice_reestablish(reload: bool, async_monitor_update: bool) {
    ...
    assert_eq!(
        initial_commit_sig_for_initiator.commitment_signed[0]
            .htlc_signatures
            .len(),
        1
    );
    ...
    } = event
    {
        ...
        nodes[0]
            .node
            .funding_transaction_signed(&channel_id, &node_id_1, tx)
            .unwrap();
    }
...
fn do_test_propose_splice_while_disconnected(reload: bool, use_0conf: bool) {
    ...
    nodes[1]
        .node
        .handle_splice_locked(node_id_0, &splice_locked);
...
fn disconnect_on_unexpected_interactive_tx_message() {
    ...
    initiator
        .node
        .handle_tx_complete(node_id_acceptor, &tx_complete);
...
fn fail_splice_on_interactive_tx_error() {
    ...
    let _tx_complete = get_event_msg!(
        acceptor,
    ...
    initiator
        .node
        .handle_tx_add_input(node_id_acceptor, &tx_add_input);
...
fn fail_splice_on_tx_abort() {
    ...
    acceptor
        .node
        .abandon_splice(&channel_id, &node_id_initiator)
        .unwrap();
    let tx_abort = get_event_msg!(acceptor, MessageSendEvent::SendTxAbort, node_id_initiator);
    initiator.node.handle_tx_abort(node_id_acceptor, &tx_abort);
...
fn fail_splice_on_channel_close() {
    ...
    let _ = complete_splice_handshake(initiator, acceptor, channel_id, contribution.clone());
    let _tx_add_input = get_event_msg!(
        initiator,
        MessageSendEvent::SendTxAddInput,
        node_id_acceptor
    );
...
fn do_test_splice_with_inflight_htlc_forward_and_resolution(expire_scid_pre_forward: bool) {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_id_0, &update_add_0_1.update_add_htlcs[0]);
    ...
    if expire_scid_pre_forward {
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(node_id_1, &update_fail_1_0.update_fail_htlcs[0]);
        ...
        nodes[2]
            .node
            .handle_update_add_htlc(node_id_1, &update_add_1_2.update_add_htlcs[0]);
        ...
        nodes[1]
            .node
            .handle_update_fail_htlc(node_id_2, &update_fail_1_2.update_fail_htlcs[0]);
        ...
        nodes[0]
            .node
            .handle_update_fail_htlc(node_id_1, &update_fail_0_1.update_fail_htlcs[0]);
