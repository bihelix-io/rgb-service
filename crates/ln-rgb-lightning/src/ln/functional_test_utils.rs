...
impl ConnectStyle {
    ...
    fn random_style() -> ConnectStyle {
        ...
        let rand_val = std::collections::hash_map::RandomState::new()
            .build_hasher()
            .finish();
...
pub fn create_dummy_block(prev_blockhash: BlockHash, time: u32, txdata: Vec<Transaction>) -> Block {
    Block {
        header: create_dummy_header(prev_blockhash, time),
        txdata,
    }
}
    ...
    if !skip_intermediaries {
        ...
        match *node.connect_style.borrow() {
            ...
            | ConnectStyle::BestBlockFirstReorgsOnlyTip => {
                node.chain_monitor
                    .chain_monitor
                    .best_block_updated(&block.header, height);
                call_claimable_balances(node);
                ...
                node.node
                    .transactions_confirmed(&block.header, &txdata, height);
            }
    ...
    for i in 0..count {
        ...
        match *node.connect_style.borrow() {
            ConnectStyle::FullBlockViaListen => {
                ...
                node.chain_monitor
                    .chain_monitor
                    .blocks_disconnected(best_block);
            ...
            | ConnectStyle::TransactionsDuplicativelyFirstSkippingBlocks => {
                if i == count - 1 {
                    node.chain_monitor
                        .chain_monitor
                        .best_block_updated(&prev.0.header, prev.1);
            ...
            | ConnectStyle::TransactionsFirstReorgsOnlyTip => {
                for tx in orig.0.txdata {
                    node.chain_monitor
                        .chain_monitor
                        .transaction_unconfirmed(&tx.compute_txid());
...
impl<'a, 'b, 'c> Node<'a, 'b, 'c> {
    pub fn init_features(&self, peer_node_id: PublicKey) -> InitFeatures {
        self.override_init_features
            .borrow()
            .clone()
            .unwrap_or_else(|| {
                self.node.init_features()
                    | self.onion_messenger.provided_init_features(peer_node_id)
            })
    }
}
...
impl<'a, 'b, 'c> Node<'a, 'b, 'c> {
    ...
    fn set_channel_signer_ops(
        ...
        if let Some(context) = chan_lock
            .channel_by_id
            .get_mut(chan_id)
            .map(|chan| chan.context_mut())
        {
        ...
        let entry = unavailable_signers_ops
            .entry(channel_keys_id)
    ...
    fn drop(&mut self) {
        if !std::thread::panicking() {
            ...
            if !msg_events.is_empty() {
                panic!(
                    "Had excess message events on node {}: {:?}",
                    self.logger.id, msg_events
                );
            }
            ...
            if !raa_blockers.is_empty() {
                panic!(
                    "Had excess RAA blockers on node {}: {:?}",
                    self.logger.id, raa_blockers
                );
            }
            ...
            let network_graph = {
                ...
                loop {
                    let orig_announcements = self
                        .gossip_sync
                        .get_next_channel_announcement(chan_progress);
                ...
                loop {
                    let orig_announcements = self
                        .gossip_sync
                        .get_next_node_announcement(node_progress.as_ref());
                    let deserialized_announcements =
            ...
            let broadcaster = test_utils::TestBroadcaster {
                ...
                blocks: Arc::new(Mutex::new(
                    self.tx_broadcaster.blocks.lock().unwrap().clone(),
                )),
            };
...
pub fn remove_first_msg_event_to_node(
    msg_node_id: &PublicKey,
    msg_events: &mut Vec<MessageSendEvent>,
) -> MessageSendEvent {
...
pub fn check_added_monitors<CM: AChannelManager, H: NodeHolder<CM = CM>>(node: &H, count: usize) {
    if let Some(chain_monitor) = node.chain_monitor() {
        ...
        assert_eq!(
            n, count,
            "expected {} monitors to be added, not {}",
            count, n
        );
...
pub fn _reload_node<'a, 'b, 'c>(
    ...
    let (_, node_deserialized) = {
        ...
        for monitor in monitors_read.iter() {
            assert!(channel_monitors
                .insert(monitor.channel_id(), monitor)
                .is_none());
        }
...
macro_rules! reload_node {
    ($node: expr, $new_config: expr, $chanman_encoded: expr, $monitors_encoded: expr, $persister: ident, $new_chain_monitor: ident, $new_channelmanager: ident) => {
        ...
        $node
            .onion_messenger
            .set_offers_handler(&$new_channelmanager);
        $node
            .onion_messenger
            .set_async_payments_handler(&$new_channelmanager);
    };
...
pub fn create_dual_funding_utxos_with_prev_txs(
    ...
    let tx = Transaction {
        ...
        lock_time: LockTime::from_height(
            u32::from_be_bytes(
                node.keys_manager.get_secure_random_bytes()[0..4]
                    .try_into()
                    .unwrap(),
            ) % LOCK_TIME_THRESHOLD,
        )
...
pub fn sign_funding_transaction<'a, 'b, 'c>(
    ...
    node_a
        .node
        .handle_funding_signed(node_b_id, &bs_funding_signed);
    {
    ...
    assert_eq!(
        node_a.tx_broadcaster.txn_broadcasted.lock().unwrap().len(),
        1
    );
...
pub fn exchange_open_accept_zero_conf_chan<'a, 'b, 'c, 'd>(
    ...
    receiver
        .node
        .handle_open_channel(initiator_node_id, &open_channel);
    ...
    initiator
        .node
        .handle_accept_channel(receiver_node_id, &accept_channel);
...
pub fn open_zero_conf_channel_with_value<'a, 'b, 'c, 'd>(
    ...
    receiver
        .node
        .handle_funding_created(initiator_node_id, &funding_created);
    ...
    receiver
        .node
        .handle_channel_ready(initiator_node_id, &as_channel_ready);
    ...
    initiator
        .node
        .handle_channel_update(receiver_node_id, &bs_channel_update);
    receiver
        .node
        .handle_channel_update(initiator_node_id, &as_channel_update);

    assert_eq!(
        initiator.node.list_usable_channels().len(),
        initiator_channels + 1
    );
    assert_eq!(
        receiver.node.list_usable_channels().len(),
        receiver_channels + 1
    );
...
pub fn exchange_open_accept_chan<'a, 'b, 'c>(
    ...
    node_b
        .node
        .handle_open_channel(node_a_id, &open_channel_msg);
    if node_b
        .node
        .get_current_config()
    ...
    assert_eq!(
        accept_channel_msg.common_fields.temporary_channel_id,
        create_chan_id
    );
    node_a
        .node
        .handle_accept_channel(node_b_id, &accept_channel_msg);
    assert_ne!(
        node_b
            .node
            .list_channels()
...
pub fn create_chan_between_nodes_with_value_confirm_first<'a, 'b, 'c, 'd>(
    ...
    node_recv
        .node
        .handle_channel_ready(node_conf.node.get_our_node_id(), &channel_ready);
}
...
pub fn create_chan_between_nodes_with_value_confirm_second<'a, 'b, 'c>(
    ...
    (msgs::ChannelReady, msgs::AnnouncementSignatures),
    ChannelId,
) {
    let channel_id;
    let events_6 = node_conf.node.get_and_clear_pending_msg_events();
    assert_eq!(events_6.len(), 3);
    let announcement_sigs_idx = if let MessageSendEvent::SendChannelUpdate {
        ref node_id,
        msg: _,
    } = events_6[1]
    {
        ...
        2
    } else if let MessageSendEvent::SendChannelUpdate {
        ref node_id,
        msg: _,
    } = events_6[2]
    {
...
pub fn create_chan_between_nodes_with_value_confirm<'a, 'b, 'c: 'd, 'd>(
    ...
    let conf_height = core::cmp::max(
        node_a.best_block_info().1 + 1,
        node_b.best_block_info().1 + 1,
    );
...
pub fn create_channel_manual_funding<'a, 'b, 'c: 'd, 'd>(
    ...
    node_b
        .node
        .handle_funding_created(node_a_id, &funding_created);
    ...
    if zero_conf {
        ...
        node_a
            .node
            .handle_funding_signed(node_b_id, &funding_signed);
        check_added_monitors(node_a, 1)
    }
    ...
    if zero_conf {
        ...
        node_a
            .node
            .handle_channel_update(node_b_id, &bs_channel_update);
        node_b
            .node
            .handle_channel_update(node_a_id, &as_channel_update);
    }
...
pub fn create_chan_between_nodes_with_value_b<'a, 'b, 'c>(
    ...
    node_b
        .node
        .handle_channel_ready(node_a_id, &as_funding_msgs.0);
    ...
    node_b
        .node
        .handle_announcement_signatures(node_a_id, &as_funding_msgs.1);
...
pub fn create_unannounced_chan_between_nodes_with_value<'a, 'b, 'c, 'd>(
    ...
    nodes[b]
        .node
        .handle_funding_created(node_a_id, &as_funding_created);
    ...
    nodes[a]
        .node
        .handle_funding_signed(node_b_id, &cs_funding_signed);
    ...
    assert_eq!(
        nodes[a].tx_broadcaster.txn_broadcasted.lock().unwrap()[0],
        tx
    );
    nodes[a]
        .tx_broadcaster
    ...
    let conf_height = core::cmp::max(
        nodes[a].best_block_info().1 + 1,
        nodes[b].best_block_info().1 + 1,
    );
    ...
    nodes[a]
        .node
        .handle_channel_ready(node_b_id, &bs_channel_ready);
...
pub fn update_nodes_with_chan_announce<'a, 'b, 'c, 'd>(
    ...
    for node in nodes {
        ...
        assert!(node
            .gossip_sync
            .handle_channel_announcement(None, ann)
            .unwrap());
        node.gossip_sync
            .handle_channel_update(Some(node_id_a), upd_1)
            .unwrap();
        node.gossip_sync
            .handle_channel_update(Some(node_id_b), upd_2)
            .unwrap();
...
pub fn check_closed_events(node: &Node, expected_close_events: &[ExpectedCloseEvent]) {
    ...
    let discard_events_count = expected_close_events
        .iter()
        .filter(|e| e.discard_funding)
        .count();
    let splice_events_count = expected_close_events
        .iter()
        .filter(|e| e.splice_failed)
        .count();
    let events = node.node.get_and_clear_pending_events();
    assert_eq!(
        events.len(),
    ...
    assert_eq!(
        events
            .iter()
            .filter(|e| matches!(e, Event::DiscardFunding { .. },))
            .count(),
    ...
    assert_eq!(
        events
            .iter()
            .filter(|e| matches!(e, Event::SpliceFailed { .. },))
            .count(),
        splice_events_count
    );
}
...
pub fn handle_bump_events(node: &Node, expected_close: bool, expected_htlc_count: usize) {
    ...
    assert_eq!(
        ...
        "Expected a bump close event, found {:?}.",
        events
    );
    assert_eq!(
...
pub fn close_channel<'a, 'b, 'c>(
    ...
    node_a
        .close_channel(channel_id, &node_b.get_our_node_id())
        .unwrap();
    let as_shutdown = get_event_msg!(
        struct_a,
...
pub fn expect_htlc_failure_conditions(
    events: Vec<Event>,
    expected_failures: &[HTLCHandlingFailureType],
) {
...
pub fn expect_htlc_forwarding_fails(
    node: &Node<'_, '_, '_>,
    expected_failure: &[HTLCHandlingFailureType],
) {
    expect_and_process_pending_htlcs(node, false);
    let events = node.node.get_and_clear_pending_events();
...
macro_rules! commitment_signed_dance {
    ...
    ($node_a: expr, $node_b: expr, (), $fail_backwards: expr, true /* skip last step */, false /* no extra message */, $incl_claim: expr) => {
        assert!(
            $crate::ln::functional_test_utils::commitment_signed_dance_through_cp_raa(
                &$node_a,
                &$node_b,
                $fail_backwards,
                $incl_claim
...
pub fn do_commitment_signed_dance(
    node_a: &Node<'_, '_, '_>,
    node_b: &Node<'_, '_, '_>,
    commitment_signed: &Vec<msgs::CommitmentSigned>,
    fail_backwards: bool,
    skip_last_step: bool,
) {
    let node_b_id = node_b.node.get_our_node_id();
    ...
    let got_claim = node_a
        .node
        .test_raa_monitor_updates_held(node_b_id, channel_id);
    if fail_backwards {
        assert!(!got_claim);
    }
...
pub fn check_payment_claimable(
    ...
    expected_payment_secret: PaymentSecret,
    expected_recv_value: u64,
    expected_payment_preimage: Option<PaymentPreimage>,
...
macro_rules! expect_payment_claimed {
    ($node: expr, $expected_payment_hash: expr, $expected_recv_value: expr) => {
        ...
        assert_eq!(events.len(), 1);
        match events[0] {
            $crate::events::Event::PaymentClaimed {
                ref payment_hash,
                amount_msat,
                ..
            } => {
...
pub fn expect_payment_sent<CM: AChannelManager, H: NodeHolder<CM = CM>>(
    node: &H,
    expected_payment_preimage: PaymentPreimage,
    expected_fee_msat_opt: Option<Option<u64>>,
...
pub fn expect_payment_forwarded<CM: AChannelManager, H: NodeHolder<CM = CM>>(
    ...
    upstream_force_closed: bool,
    downstream_force_closed: bool,
    allow_1_msat_fee_overpay: bool,
) -> Option<u64> {
    match event {
...
pub fn expect_channel_ready_event<'a, 'b, 'c, 'd>(
    ...
    assert_eq!(events.len(), 1);
    match events[0] {
        crate::events::Event::ChannelReady {
            ref counterparty_node_id,
            ..
        } => {
            assert_eq!(*expected_counterparty_node_id, *counterparty_node_id);
...
pub fn expect_splice_pending_event<'a, 'b, 'c, 'd>(
    ...
    assert_eq!(events.len(), 1);
    match &events[0] {
        crate::events::Event::SplicePending {
            channel_id,
            counterparty_node_id,
            ..
        } => {
...
pub fn expect_probe_successful_events(
    node: &Node,
    mut probe_results: Vec<(PaymentHash, PaymentId)>,
) {
    let mut events = node.node.get_and_clear_pending_events();
...
pub fn fail_payment_along_path<'a, 'b, 'c>(expected_path: &[&Node<'a, 'b, 'c>]) {
    ...
    for (&node, &prev_node) in expected_path
        .iter()
        .rev()
        .zip(expected_path.iter().rev().skip(1))
    {
...
pub fn send_probe_along_route<'a, 'b, 'c>(
    origin_node: &Node<'a, 'b, 'c>,
    expected_route: &[(&[&Node<'a, 'b, 'c>], PaymentHash)],
) {
    ...
    for (path, payment_hash) in expected_route.iter() {
        ...
        let nodes_to_fail_payment: Vec<_> = vec![origin_node]
            .into_iter()
...
pub fn do_claim_payment_along_route(args: ClaimAlongRouteArgs) -> u64 {
    ...
    args.expected_paths[0]
        ...
        .unwrap()
        .node
        .claim_funds(args.payment_preimage);
...
pub fn pass_claimed_payment_along_route(args: ClaimAlongRouteArgs) -> u64 {
    let claim_event = args.expected_paths[0]
        .last()
        .unwrap()
        .node
        .get_and_clear_pending_events();
    ...
    check_added_monitors(
        args.expected_paths[0].last().unwrap(),
        args.expected_paths.len(),
    );
    ...
    let mut events = args.expected_paths[0]
        .last()
        .unwrap()
        .node
...
pub fn pass_claimed_payment_along_route_from_ev(
    ...
    for (i, (expected_route, (path_msgs, next_hop))) in expected_paths
        .iter()
...
pub fn fail_payment_along_route<'a, 'b, 'c>(
    ...
    let expected_destinations: Vec<HTLCHandlingFailureType> =
        repeat(HTLCHandlingFailureType::Receive {
            payment_hash: our_payment_hash,
        })
...
pub fn pass_failed_payment_back<'a, 'b, 'c>(
    origin_node: &Node<'a, 'b, 'c>,
    expected_paths_slice: &[&[&Node<'a, 'b, 'c>]],
    skip_last: bool,
    our_payment_hash: PaymentHash,
    expected_fail_reason: PaymentFailureReason,
) {
    let mut expected_paths: Vec<_> = expected_paths_slice.iter().collect();
    check_added_monitors!(expected_paths[0].last().unwrap(), expected_paths.len());

    let mut per_path_msgs: Vec<(
        (msgs::UpdateFailHTLC, Vec<msgs::CommitmentSigned>),
        PublicKey,
    ...
    expected_paths[0]
        ...
        .node
        .fail_htlc_backwards(&our_payment_hash);
    let pending_events = expected_paths[0]
        .last()
        .unwrap()
        .node
        .get_and_clear_pending_events();
...
pub fn create_chanmon_cfgs_with_legacy_keys(
    node_count: usize,
    predefined_keys_ids: Option<Vec<[u8; 32]>>,
) -> Vec<TestChanMonCfg> {
...
pub fn connect_nodes<'a, 'b: 'a, 'c: 'b>(node_a: &Node<'a, 'b, 'c>, node_b: &Node<'a, 'b, 'c>) {
    ...
    node_a
        .node
        .peer_connected(node_id_b, &init_b, true)
        .unwrap();
    node_b
        .node
        .peer_connected(node_id_a, &init_a, false)
        .unwrap();
    node_a
        .onion_messenger
        .peer_connected(node_id_b, &init_b, true)
...
pub fn connect_dummy_node<'a, 'b: 'a, 'c: 'b>(node: &Node<'a, 'b, 'c>) {
    ...
    node.node
        .peer_connected(node_id_dummy, &init_dummy, true)
        .unwrap();
    node.onion_messenger
        .peer_connected(node_id_dummy, &init_dummy, true)
        .unwrap();
}
...
pub fn handle_announce_close_broadcast_events<'a, 'b, 'c>(
    ...
    for node in nodes {
        node.gossip_sync
            .handle_channel_update(Some(node_a_id), &as_update)
            .unwrap();
        node.gossip_sync
            .handle_channel_update(Some(node_a_id), &bs_update)
            .unwrap();
    }
}
...
macro_rules! get_chan_reestablish_msgs {
    ($src_node: expr, $dst_node: expr) => {{
        ...
        for msg in $src_node.node.get_and_clear_pending_msg_events() {
            if let MessageSendEvent::SendChannelReestablish {
                ref node_id,
                ref msg,
            } = msg
            {
...
macro_rules! handle_chan_reestablish_msgs {
    ($src_node: expr, $dst_node: expr) => {{
        ...
        if let Some(&MessageSendEvent::SendTxAbort {
            ref node_id,
            ref msg,
        }) = msg_events.get(idx)
        {
        ...
        let channel_ready = if let Some(&MessageSendEvent::SendChannelReady {
            ref node_id,
            ref msg,
        }) = msg_events.get(idx)
        {
        ...
        if let Some(&MessageSendEvent::SendAnnouncementSignatures {
            ref node_id,
            ref msg,
        }) = msg_events.get(idx)
        {
        ...
        let mut stfu = None;
        if let Some(&MessageSendEvent::SendStfu {
            ref node_id,
            ref msg,
        }) = msg_events.get(idx)
        {
        ...
        if let Some(&MessageSendEvent::SendTxSignatures {
            ref node_id,
            ref msg,
        }) = msg_events.get(idx)
        {
...
pub fn reconnect_nodes<'a, 'b, 'c, 'd>(args: ReconnectArgs<'a, 'b, 'c, 'd>) {
    ...
    if send_channel_ready.0 {
        ...
        for reestablish in reestablish_1.iter() {
            ...
            assert_eq!(
                n, 0,
                "expected a->b next_remote_commitment_number to be 0, got {}",
                n
            );
        }
    }
    if send_channel_ready.1 {
        ...
        for reestablish in reestablish_2.iter() {
            ...
            assert_eq!(
                n, 0,
                "expected b->a next_remote_commitment_number to be 0, got {}",
                n
            );
        }
    }
    if send_channel_ready.0 || send_channel_ready.1 {
        ...
        for reestablish in reestablish_1.iter() {
            ...
            assert_eq!(
                n, 1,
                "expected a->b next_local_commitment_number to be 1, got {}",
                n
            );
        }
        for reestablish in reestablish_2.iter() {
            ...
            assert_eq!(
                n, 1,
                "expected b->a next_local_commitment_number to be 1, got {}",
                n
            );
        }
    }
    ...
    for mut chan_msgs in resp_1.drain(..) {
        if send_channel_ready.0 {
            node_a
                .node
                .handle_channel_ready(node_b_id, &chan_msgs.0.unwrap());
        ...
        if send_announcement_sigs.0 {
            ...
            node_a
                .node
                .handle_announcement_signatures(node_b_id, &announcement_sigs);
        ...
        if pending_raa.0 {
            ...
            node_a
                .node
                .handle_revoke_and_ack(node_b_id, &chan_msgs.1.unwrap());
            assert!(node_a.node.get_and_clear_pending_msg_events().is_empty());
            check_added_monitors!(node_a, 1);
        } else {
            assert!(chan_msgs.1.is_none());
            ...
            || pending_responding_commitment_signed.0
        {
            ...
            if !pending_responding_commitment_signed.0 {
                ...
                check_added_monitors!(
                    node_b,
                    if pending_responding_commitment_signed_dup_monitor.0 {
                        0
                    } else {
                        1
                    }
    ...
    for mut chan_msgs in resp_2.drain(..) {
        if send_channel_ready.1 {
            node_b
                .node
                .handle_channel_ready(node_a_id, &chan_msgs.0.unwrap());
        ...
        if send_announcement_sigs.1 {
            ...
            node_b
                .node
                .handle_announcement_signatures(node_a_id, &announcement_sigs);
        ...
        if pending_raa.1 {
            ...
            node_b
                .node
                .handle_revoke_and_ack(node_a_id, &chan_msgs.1.unwrap());
            assert!(node_b.node.get_and_clear_pending_msg_events().is_empty());
            check_added_monitors!(node_b, 1);
        } else {
            assert!(chan_msgs.1.is_none());
...
pub fn create_batch_channel_funding<'a, 'b, 'c>(
    ...
    for (other_node, channel_value_satoshis, push_msat, user_channel_id, override_config) in params
    {
        ...
        other_node
            .node
            .handle_open_channel(funding_node_id, &open_channel_msg);
