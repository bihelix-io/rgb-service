...
impl ConstructedTransaction {
    fn new(context: NegotiationContext) -> Result<Self, AbortReason> {
        ...
        let mut inputs: Vec<(TxIn, TxInMetadata)> = context
            .inputs
            .into_values()
            .map(|input| input.into_txin_and_metadata())
            .collect();
        let mut outputs: Vec<(TxOut, TxOutMetadata)> = context
            .outputs
            .into_values()
            .map(|output| output.into_txout_and_metadata())
            .collect();
    ...
    fn funding_outpoint(&self) -> OutPoint {
        OutPoint {
            ...
            index: self.shared_output_index,
        }
    }
    ...
    fn finalize(
        &self,
        holder_tx_signatures: &TxSignatures,
        counterparty_tx_signatures: &TxSignatures,
        ...
        if let Some(shared_input_index) = self.shared_input_index {
            ...
            let counterparty_shared_input_sig = counterparty_tx_signatures
...
impl InteractiveTxSigningSession {
    ...
    pub fn provide_holder_witnesses<C: bitcoin::secp256k1::Verification>(
        ...
        .then(|| {
            ...
            self.holder_tx_signatures
                .clone()
                .expect("Holder tx_signatures were just provided")
        });
    ...
    pub fn shared_input(&self) -> Option<&TxInMetadata> {
        self.unsigned_tx
            .shared_input_index
            .and_then(|shared_input_index| {
                self.unsigned_tx
                    .input_metadata
                    .get(shared_input_index as usize)
            })
    }
    ...
    fn verify_interactive_tx_signatures<C: bitcoin::secp256k1::Verification>(
        ...
        let prev_outputs: Vec<&TxOut> = unsigned_tx
            .input_metadata()
            .map(|input| input.prev_output())
            .collect::<Vec<_>>();
...
impl NegotiationContext {
    ...
    fn remote_inputs_value(&self) -> u64 {
        self.inputs.iter().fold(0u64, |acc, (_, input)| {
            acc.saturating_add(input.remote_value())
        })
    }

    fn remote_outputs_value(&self) -> u64 {
        self.outputs.iter().fold(0u64, |acc, (_, output)| {
            acc.saturating_add(output.remote_value())
        })
    }
    ...
    fn remote_weight_contributed(&self) -> u64 {
        self.remote_inputs_weight()
            ...
            .saturating_add(if !self.holder_is_initiator {
    ...
    fn local_inputs_value(&self) -> u64 {
        self.inputs
            .iter()
            .fold(0u64, |acc, (_, input)| acc.saturating_add(input.value()))
    }
        ...
        let (input, prev_outpoint) = if let Some(shared_txid) = &msg.shared_input_txid {
            ...
            if let Some(shared_funding_input) = &self.shared_funding_input {
                if self
                    .inputs
                    .values()
                    .any(|input| matches!(input.input, InputOwned::Shared(_)))
                {
                    return Err(AbortReason::DuplicateFundingInput);
                }

                let previous_output = BitcoinOutPoint {
                    txid: *shared_txid,
                    vout: msg.prevtx_out,
                };
                ...
                (
                    InputOwned::Shared(shared_funding_input.clone()),
                    previous_output,
                )
            } else {
                return Err(AbortReason::UnexpectedFundingInput);
            ...
            if let Some(tx_out) = prevtx.output.get(msg.prevtx_out as usize) {
                ...
                let prev_outpoint = BitcoinOutPoint {
                    txid,
                    vout: msg.prevtx_out,
                };
                let txin = TxIn {
    ...
    fn received_tx_add_output(&mut self, msg: &msgs::TxAddOutput) -> Result<(), AbortReason> {
        ...
        let output = if txout == self.shared_funding_output.tx_out {
            ...
            if self
                .outputs
                .values()
                .any(|output| matches!(output.output, OutputOwned::Shared(_)))
            {
                return Err(AbortReason::DuplicateFundingOutput);
            }
    ...
    fn sent_tx_add_input(
        &mut self,
        (msg, satisfaction_weight): (&msgs::TxAddInput, Weight),
    ) -> Result<(), AbortReason> {
...
impl SharedOwnedInput {
    ...
    fn remote_owned(&self) -> u64 {
        self.prev_output
            .value
            .to_sat()
            .saturating_sub(self.local_owned)
    }
}
        ...
        match self.next_input_index {
            Some(index) => {
                self.next_input_index = index
                    .checked_add(1)
                    .filter(|index| *index < self.inputs_to_contribute.len());
        ...
        match self.next_output_index {
            Some(index) => {
                self.next_output_index = index
                    .checked_add(1)
                    .filter(|index| *index < self.outputs_to_contribute.len());
...
pub(super) fn calculate_change_output_value(
    ...
    is_splice: bool,
    shared_output_funding_script: &ScriptBuf,
    ...
    let total_output_satoshis = funding_outputs
        .iter()
    ...
    let net_total_less_fees = total_input_satoshis
        .saturating_sub(total_output_satoshis)
...
mod tests {
        ...
        ES::Target: EntropySource,
    {
            ...
            |msg: InteractiveTxMessageSend, for_constructor: &mut InteractiveTxConstructor| {
                match msg {
                    ...
                    InteractiveTxMessageSend::TxAddOutput(msg) => for_constructor
                        ...
                        .map(|msg_send| (Some(msg_send), false)),
                    InteractiveTxMessageSend::TxComplete(msg) => for_constructor
        ...
        let mut message_send_a = constructor_a
            .as_mut()
            .unwrap()
    ...
    fn generate_txout(output: &TestOutput) -> TxOut {
        ...
        let (value, script_pubkey) = match output {
            ...
            TestOutput::P2PKH(value) => (
                *value,
                ScriptBuf::new_p2pkh(&PubkeyHash::from_slice(&[4; 20]).unwrap()),
            ),
        };
