...
impl Display for PaymentAttempts {
    fn fmt(&self, f: &mut Formatter) -> Result<(), fmt::Error> {
        ...
        return write!(
            ...
            self.count,
            Instant::now()
                .duration_since(self.first_attempted_at)
                .as_secs()
        );
    }
}
...
impl OutboundPayments {
            ...
            .entry(payment_id)
        {
            hash_map::Entry::Occupied(mut entry) => match entry.get_mut() {
                PendingOutboundPayment::AwaitingInvoice {
                    ...
                    if invoice
                        .invoice_features()
                        .requires_unknown_bits_from(&features)
                    {
                    ...
                    if duration_since_epoch
                        > invoice
                            .created_at()
                            .saturating_add(invoice.relative_expiry())
                    {
        ...
        L::Target: Logger,
    {
        ...
        log_trace!(
            logger,
            "Failing outbound payment HTLC with payment_hash {payment_hash}"
        );
