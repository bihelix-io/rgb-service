...
pub fn test_async_inbound_update_fee() {
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &payment_event.commitment_msg); // (2)
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_revoke_and_ack); // deliver (2)
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_revoke_and_ack); // deliver (3)
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_update.commitment_signed); // deliver (4)
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &as_update.commitment_signed); // deliver (5)
    let bs_second_revoke = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, node_a_id);
    check_added_monitors(&nodes[1], 1);

    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_second_revoke);
    check_added_monitors(&nodes[0], 1);

    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_second_revoke); // deliver (6)
    check_added_monitors(&nodes[1], 1);
}
...
pub fn test_update_fee_unordered_raa() {
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_revoke_msg); // deliver (2)
    check_added_monitors(&nodes[1], 1);
...
pub fn test_multi_flight_update_fee() {
    ...
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, commitment_signed_1);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_b_id, &bs_revoke_msg);
    ...
    assert_eq!(
        as_second_update.update_fee.as_ref().unwrap().channel_id,
        update_msg_2.channel_id
    );
    assert_eq!(
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_revoke_msg);
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_second_commitment);
    let as_second_revoke = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, node_b_id);
    // No commitment_signed so get_event_msg's assert(len == 1) passes
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &as_second_revoke);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors(&nodes[1], 1);
}
...
pub fn do_test_update_fee_that_funder_cannot_afford(channel_type_features: ChannelTypeFeatures) {
    ...
    nodes[1]
        .node
        .handle_update_fee(node_a_id, &update_msg.update_fee.unwrap());
    ...
    // Confirm that the new fee based on the last local commitment txn is what we expected based on the feerate set above.
    {
        ...
        let mut actual_fee = commitment_tx
            .output
            .iter()
            .fold(0, |acc, output| acc + output.value.to_sat());
    ...
    let remote_point = {
        ...
        chan_signer
            .as_ref()
            .get_per_commitment_point(point_number, &secp_ctx)
            .unwrap()
    };
...
pub fn test_update_fee_with_fundee_update_add_htlc() {
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &commitment_update.update_add_htlcs[0]);
    nodes[0]
        .node
...
pub fn test_update_fee() {
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &commitment_signed_0);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &revoke_msg_0);
    let commitment_update = get_htlc_update_msgs!(nodes[1], node_a_id);
...
pub fn test_chan_init_feerate_unaffordability() {
    ...
    nodes[0]
        .node
        .create_channel(node_b_id, 100_000, push_amt, 42, None, None)
        .unwrap();
...
pub fn accept_busted_but_better_fee() {
    ...
    match events[0] {
        MessageSendEvent::UpdateHTLCs {
            ...
            nodes[1]
                .node
                .handle_update_fee(node_a_id, update_fee.as_ref().unwrap());
...
pub fn do_cannot_afford_on_holding_cell_release(
    ...
    nodes[0]
        .node
        .handle_commitment_signed(node_b_id, &payment_event.commitment_msg[0]);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_commitment_signed(node_a_id, &commitment_signed[0]);
    check_added_monitors(&nodes[1], 1);
...
pub fn do_can_afford_given_trimmed_htlcs(inequality_regions: core::cmp::Ordering) {
    ...
    route_payment(
        &nodes[1],
        &[&nodes[0]],
        node_0_inbound_htlc_amount_sat * 1000,
    );
...
pub fn test_zero_fee_commitments_no_update_fee() {
    ...
    nodes[1]
        .node
        .handle_update_fee(nodes[0].node.get_our_node_id(), &update_fee_msg);
