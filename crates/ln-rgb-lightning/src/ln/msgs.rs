...
pub struct OpenChannel {
    ...
    /// Optional RGB funding amount bound to the channel funding outpoint.
    pub funding_rgb: Option<(ContractId, u64)>,
}
...
pub struct UpdateAddHTLC {
    ...
    pub amount_msat: u64,
    /// Optional RGB asset amount transferred by this HTLC.
...
impl Writeable for OpenChannel {
    fn write<W: Writer>(&self, w: &mut W) -> Result<(), io::Error> {
        ...
        encode_tlv_stream!(w, {
            (0, self.common_fields.shutdown_scriptpubkey.as_ref().map(|s| WithoutLength(s)), option), // Don't encode length twice.
            (1, self.common_fields.channel_type, option),
            (26233_1, self.funding_rgb, option),
        });
        Ok(())
    }
}
...
impl LengthReadable for OpenChannel {
    fn read_from_fixed_length_buffer<R: LengthLimitedRead>(r: &mut R) -> Result<Self, DecodeError> {
        ...
        let mut shutdown_scriptpubkey: Option<ScriptBuf> = None;
        let mut channel_type: Option<ChannelTypeFeatures> = None;
        let mut funding_rgb: Option<(ContractId, u64)> = None;
        decode_tlv_stream!(r, {
            (0, shutdown_scriptpubkey, (option, encoding: (ScriptBuf, WithoutLength))),
            (1, channel_type, option),
            (26233_1, funding_rgb, option),
        });
        Ok(OpenChannel {
            ...
            Self::LegacyBlindedPathEntry {
                ...
                let serialization_length = {
                    ...
                    for current_payment_path in payment_paths {
                        current_payment_path
                            .inner_blinded_path()
                            .write(&mut blinded_path_slice)?;
                        current_payment_path
                            .payinfo
                            .write(&mut blinded_path_slice)?;
                    }
    ...
    NS::Target: NodeSigner,
{
    fn read<R: Read>(r: &mut R, args: (Option<PublicKey>, NS)) -> Result<Self, DecodeError> {
        ...
        if let Some(trampoline_onion_packet) = trampoline_onion_packet {
            ...
            return Ok(Self::TrampolineEntrypoint(
                InboundTrampolineEntrypointPayload {
        ...
        if let Some(blinding_point) = intro_node_blinding_point.or(update_add_blinding_point) {
            ...
            match ChaChaPolyReadAdapter::read(&mut reader, rho)? {
                ...
                ChaChaPolyReadAdapter {
                    ...
                    if tlvs
                        .verify_for_offer_payment(hmac, nonce, &expanded_key)
                        .is_err()
                    {
                        return Err(DecodeError::InvalidValue);
                    }
...
mod tests {
    ...
    fn encoding_announcement_signatures() {
        ...
        let sig_1 = get_sig_on!(
            privkey,
            secp_ctx,
            String::from("01010101010101010101010101010101")
        );
        let sig_2 = get_sig_on!(
            privkey,
            secp_ctx,
            String::from("02020202020202020202020202020202")
        );
    ...
    fn do_encoding_channel_announcement(unknown_features_bits: bool, excess_data: bool) {
        ...
        let sig_3 = get_sig_on!(
            privkey_3,
            secp_ctx,
            String::from("01010101010101010101010101010101")
        );
        let sig_4 = get_sig_on!(
            privkey_4,
            secp_ctx,
            String::from("01010101010101010101010101010101")
        );
    ...
    fn do_encoding_node_announcement(
        unknown_features_bits: bool,
        ipv4: bool,
        ipv6: bool,
        onionv2: bool,
        onionv3: bool,
        hostname: bool,
        excess_address_data: bool,
        excess_data: bool,
    ) {
        let secp_ctx = Secp256k1::new();
        let (privkey_1, pubkey_1) = get_keys_from!(
    ...
    fn do_encoding_open_channelv2(
        ...
        let open_channelv2 = msgs::OpenChannelV2 {
            ...
            require_confirmed_inputs: if require_confirmed_inputs {
                Some(())
            } else {
                None
            },
    ...
    fn do_encoding_shutdown(script_type: u8) {
        ...
        let shutdown = msgs::Shutdown {
            ...
            scriptpubkey: if script_type == 1 {
                ...
                Address::p2sh(&script, Network::Testnet)
                    .unwrap()
                    .script_pubkey()
    ...
    fn do_encoding_commitment_signed(htlcs: bool) {
        ...
        let commitment_signed = msgs::CommitmentSigned {
            channel_id: ChannelId::from_bytes([2; 32]),
            signature: sig_1,
            htlc_signatures: if htlcs {
                vec![sig_2, sig_3, sig_4]
            } else {
                Vec::new()
            },
            funding_txid: Some(
    ...
    fn encoding_final_onion_hop_data_with_trampoline_packet() {
...

        {
            ...
            let decoded_trampoline_packet: TrampolineOnionPacket =
                ...
                .unwrap();
            assert_eq!(
                decoded_trampoline_packet.encode(),
                encoded_trampoline_packet
            );
        }
        ...
        assert_eq!(
            trampoline_length_big_size.0,
            encoded_trampoline_packet.len() as u64
        );
    }
    ...
    fn do_encoding_reply_channel_range(encoding_type: u8) {
        ...
        if encoding_type == 0 {
            ...
            assert!(
                result.is_err(),
                "Expected decode failure with unsupported zlib encoding"
            );
        }
    }
    ...
    fn do_encoding_query_short_channel_ids(encoding_type: u8) {
        ...
        if encoding_type == 0 {
            ...
            assert_eq!(
                query_short_channel_ids.short_channel_ids[0],
                0x000000000000008e
            );
            assert_eq!(
                query_short_channel_ids.short_channel_ids[1],
                0x0000000000003c69
            );
            assert_eq!(
                query_short_channel_ids.short_channel_ids[2],
                0x000000000045a6c4
            );
        } else {
            target_value.append(
    ...
    fn test_socket_address_from_str() {
        ...
        assert_eq!(
            tcpip_v4,
            SocketAddress::from_str(&tcpip_v4.to_string()).unwrap()
        );
        ...
        assert_eq!(
            tcpip_v6,
            SocketAddress::from_str(&tcpip_v6.to_string()).unwrap()
        );
        ...
        assert_eq!(
            hostname,
            SocketAddress::from_str(&hostname.to_string()).unwrap()
        );
        ...
        assert!("invalid-onion-v3-hostname.onion:8080"
            .parse::<SocketAddress>()
            .is_err());
        assert!("b32.example.onion:invalid-port"
            .parse::<SocketAddress>()
            .is_err());
