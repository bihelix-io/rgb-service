...
fn do_test_open_channel(zero_conf: bool) {
...

    {
        ...
        assert!(
            msgs.is_empty(),
            "Expected no message events; got {:?}",
            msgs
        );
    }
...
fn do_test_async_commitment_signature_for_commitment_signed_revoke_and_ack(
    ...
    dst.node
        .handle_update_add_htlc(src_node_id, &payment_event.msgs[0]);
    ...
    dst.node
        .handle_commitment_signed_batch_test(src_node_id, &payment_event.commitment_msg);
...
fn do_test_funding_signed_0conf(signer_ops: Vec<SignerOp>) {
    ...
    assert_eq!(
        accept_channel.common_fields.minimum_depth, 0,
        "Expected minimum depth of 0"
    );
    nodes[0]
        .node
    ...
    nodes[0]
        .node
        .handle_channel_ready(node_b_id, &channel_ready_1);
    expect_channel_ready_event(&nodes[0], &node_b_id);

    nodes[1]
        .node
        .handle_channel_ready(node_a_id, &channel_ready_0);
    ...
    nodes[0]
        .node
        .handle_channel_update(node_b_id, &channel_update_1);
    nodes[1]
        .node
        .handle_channel_update(node_a_id, &channel_update_0);
...
fn do_test_async_raa_peer_disconnect(
    ...
    raa_blocked_by_commit_point: bool,
) {
    ...
    src.node
        .peer_connected(dst_node_id, init_msg, true)
        .unwrap();
    ...
    dst.node
        .peer_connected(src_node_id, init_msg, false)
        .unwrap();
    ...
    dst.node
        .handle_channel_reestablish(src_node_id, &reestablish_1[0]);
...
fn do_test_async_commitment_signature_ordering(monitor_update_failure: bool) {
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &bs_second_commitment_signed);
...
fn do_test_async_holder_signatures(keyed_anchors: bool, p2a_anchor: bool, remote_commitment: bool) {
    ...
    config
        ...
        .negotiate_anchors_zero_fee_htlc_tx = keyed_anchors;
    config
        .channel_handshake_config
    ...
    let closing_node = if remote_commitment {
        &nodes[1]
    } else {
        &nodes[0]
    };
    ...
    assert!(
        txn.is_empty(),
        "expected no transaction to be broadcast, got {:?}",
        txn
    );
...
fn do_test_closing_signed(extra_closing_signed: bool, reconnect: bool) {
    ...
    if extra_closing_signed {
        ...
        nodes[0]
            .node
            .handle_closing_signed(node_b_id, &node_1_closing_signed_2_bad);
...
fn test_no_disconnect_while_async_revoke_and_ack_expecting_remote_commitment_signed() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_a_id, &update.update_add_htlcs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_a_id, &update.commitment_signed);
    check_added_monitors(&nodes[1], 1);

    let update = get_htlc_update_msgs!(&nodes[1], node_a_id);
    nodes[0]
        .node
        .handle_update_add_htlc(node_b_id, &update.update_add_htlcs[0]);
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_b_id, &update.commitment_signed);
    check_added_monitors(&nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_a_id, &revoke_and_ack);
    check_added_monitors(&nodes[1], 1);
...
fn test_no_disconnect_while_async_commitment_signed_expecting_remote_revoke_and_ack() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_b_id, update.update_fulfill_htlcs.remove(0));
