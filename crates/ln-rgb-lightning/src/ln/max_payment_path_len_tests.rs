...
fn blinded_path_with_custom_tlv() {
    ...
    create_announced_chan_between_nodes(&nodes, 0, 1);
    create_announced_chan_between_nodes(&nodes, 1, 2);
    let chan_upd_2_3 = create_announced_chan_between_nodes_with_value(&nodes, 2, 3, 1_000_000, 0)
        .0
        .contents;
...
fn bolt12_invoice_too_large_blinded_paths() {
    ...
    nodes[0]
        .onion_messenger
        .handle_onion_message(nodes[1].node.get_our_node_id(), &invoice_om);
