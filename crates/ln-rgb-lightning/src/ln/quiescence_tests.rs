...
fn test_quiescence_tie() {
    ...
    nodes[0]
        .node
        .maybe_propose_quiescence(&nodes[1].node.get_our_node_id(), &chan_id)
        .unwrap();
    nodes[1]
        .node
        .maybe_propose_quiescence(&nodes[0].node.get_our_node_id(), &chan_id)
        .unwrap();

    let stfu_node_0 = get_event_msg!(
        nodes[0],
    ...
    nodes[1]
        .node
        .handle_stfu(nodes[0].node.get_our_node_id(), &stfu_node_0);

    let stfu_node_1 = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendStfu,
    ...
    nodes[0]
        .node
        .handle_stfu(nodes[1].node.get_our_node_id(), &stfu_node_1);

    assert!(stfu_node_0.initiator && stfu_node_1.initiator);

    assert!(nodes[0]
        .node
        .exit_quiescence(&nodes[1].node.get_our_node_id(), &chan_id)
        .unwrap());
    assert!(nodes[1]
        .node
        .exit_quiescence(&nodes[0].node.get_our_node_id(), &chan_id)
        .unwrap());
}
...
fn test_quiescence_shutdown_ignored() {
    ...
    if let Err(e) = nodes[0]
        .node
        .close_channel(&chan_id, &nodes[1].node.get_our_node_id())
    {
        assert_eq!(
            e,
    ...
    nodes[1]
        .node
        .close_channel(&chan_id, &nodes[0].node.get_our_node_id())
        .unwrap();
    let shutdown = get_event_msg!(
        nodes[1],
    ...
    nodes[0]
        .node
        .handle_shutdown(nodes[1].node.get_our_node_id(), &shutdown);
...
fn allow_shutdown_while_awaiting_quiescence(local_shutdown: bool) {
    ...
    remote_node
        .node
        .handle_update_add_htlc(local_node_id, &update_add.update_add_htlcs[0]);
    remote_node
        .node
    ...
    local_node
        .node
        .handle_revoke_and_ack(remote_node_id, &revoke_and_ack);
    check_added_monitors(local_node, 1);
    ...
    local_node
        .node
        .maybe_propose_quiescence(&remote_node_id, &chan_id)
        .unwrap();
    assert!(local_node
        .node
    ...
    let (closer_node, closee_node) = if local_shutdown {
    ...
    closer_node
        .node
        .close_channel(&chan_id, &closee_node_id)
        .unwrap();
    ...
    remote_node
        .node
        .handle_revoke_and_ack(local_node_id, &last_revoke_and_ack);
    check_added_monitors(remote_node, 1);
    ...
    local_node
        .node
        .handle_update_fail_htlc(remote_node_id, &update_fail.update_fail_htlcs[0]);
    local_node
        .node
    ...
    remote_node
        .node
        .handle_revoke_and_ack(local_node_id, &revoke_and_ack);
    check_added_monitors(remote_node, 1);
    remote_node
        .node
        .handle_commitment_signed_batch_test(local_node_id, &commit_sig);
    check_added_monitors(remote_node, 1);
    ...
    local_node
        .node
        .handle_revoke_and_ack(remote_node_id, &last_revoke_and_ack);
    ...
    assert!(!local_node
        .node
        .exit_quiescence(&remote_node_id, &chan_id)
        .unwrap());
...
fn test_quiescence_waits_for_async_signer_and_monitor_update() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(node_id_1, update.update_fulfill_htlcs.remove(0));
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_id_1, &update.commitment_signed);
    ...
    nodes[1]
        .node
        .maybe_propose_quiescence(&node_id_0, &chan_id)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(node_id_0, &revoke_and_ack);
    check_added_monitors(&nodes[1], 1);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_id_0, &commit_sig);
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(node_id_1, &revoke_and_ack);
    ...
    // We have two updates pending:
    {
        ...
        let (_, latest_update) = test_chain_mon
        ...
        chain_monitor
            .channel_monitor_updated(chan_id, latest_update)
            .unwrap();
        expect_payment_sent(&nodes[0], preimage, None, false, true);

        let (_, new_latest_update) = test_chain_mon
            .latest_monitor_update_id
            .lock()
            .unwrap()
            .get(&chan_id)
        ...
        chain_monitor
            .channel_monitor_updated(chan_id, new_latest_update)
            .unwrap();
...
fn test_quiescence_on_final_revoke_and_ack_pending_monitor_update() {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(node_id_0, &update_add.update_add_htlcs[0]);
    nodes[1]
        .node
        .handle_commitment_signed_batch_test(node_id_0, &update_add.commitment_signed);
    check_added_monitors(&nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_commitment_signed_batch_test(node_id_1, &commit_sig);
    check_added_monitors(&nodes[0], 1);
...
fn quiescence_updates_go_to_holding_cell(fail_htlc: bool) {
    ...
    nodes[1]
        .node
        .send_payment_with_route(route1, payment_hash1, onion1, payment_id1)
        .unwrap();
    ...
    nodes[0]
        .node
        .send_payment_with_route(route2, payment_hash2, onion2, payment_id2)
        .unwrap();
    check_added_monitors!(&nodes[0], 1);
    ...
    if fail_htlc {
        ...
        let failed_payment = HTLCHandlingFailureType::Receive {
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(node_id_1, &update.update_add_htlcs[0]);
    if fail_htlc {
        nodes[0]
            .node
            .handle_update_fail_htlc(node_id_1, &update.update_fail_htlcs[0]);
    } else {
    ...
    if fail_htlc {
        assert!(events
            .iter()
            .find(|e| matches!(e, Event::PaymentFailed { .. }))
            .is_some());
        assert!(events
            .iter()
            .find(|e| matches!(e, Event::PaymentPathFailed { .. }))
            .is_some());
    } else {
        assert!(events
            .iter()
            .find(|e| matches!(e, Event::PaymentSent { .. }))
            .is_some());
        assert!(events
            .iter()
            .find(|e| matches!(e, Event::PaymentPathSuccessful { .. }))
            .is_some());
        check_added_monitors(&nodes[0], 1);
    }
    ...
    if fail_htlc {
        nodes[1]
            .node
            .handle_update_fail_htlc(node_id_0, &update.update_fail_htlcs[0]);
    } else {
        expect_payment_claimed!(nodes[0], payment_hash1, payment_amount);
        nodes[1]
            .node
            .handle_update_fulfill_htlc(node_id_0, update.update_fulfill_htlcs.remove(0));
    }
...
fn do_test_quiescence_during_disconnection(with_pending_claim: bool, propose_disconnected: bool) {
    ...
    if !propose_disconnected {
        nodes[1]
            .node
            .maybe_propose_quiescence(&node_a_id, &chan_id)
            .unwrap();
    }
    ...
    let reestab_a = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendChannelReestablish,
        node_b_id
    );
    let reestab_b = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelReestablish,
        node_a_id
    );
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(node_b_id, &reestab_b);
    get_event_msg!(nodes[0], MessageSendEvent::SendChannelUpdate, node_b_id);

    nodes[1]
        .node
        .handle_channel_reestablish(node_a_id, &reestab_a);
    ...
    let stfu = if with_pending_claim {
        ...
        if let MessageSendEvent::UpdateHTLCs { mut updates, .. } = msg {
            ...
            nodes[1]
                .node
                .handle_commitment_signed_batch_test(node_a_id, &cs);
            check_added_monitors(&nodes[1], 1);
...
fn do_test_quiescence_termination_on_disconnect(reload: bool) {
    ...
    nodes[0]
        .node
        .maybe_propose_quiescence(&node_id_1, &channel_id)
        .unwrap();
