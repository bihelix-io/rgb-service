...
impl AsyncPaymentsMessageHandler for TestAsyncPaymentsMessageHandler {
    fn handle_offer_paths_request(
        &self,
        _message: OfferPathsRequest,
        _context: AsyncPaymentsContext,
        _responder: Option<Responder>,
...
impl TestCustomMessageHandler {
    ...
    fn expect_message(&self, message: TestCustomMessage) {
        let expectation = OnHandleCustomMessage {
            expect: message,
    ...
    fn get_next_expectation(&self) -> OnHandleCustomMessage {
        self.expectations
            ...
            .unwrap()
            .pop_front()
            .expect("No expectations remaining")
    }
}
...
fn create_nodes_using_cfgs(cfgs: Vec<MessengerCfg>) -> Vec<MessengerNode> {
    ...
    for (i, cfg) in cfgs.into_iter().enumerate() {
        let secret_key = cfg
            .secret_override
            .unwrap_or(SecretKey::from_slice(&[(i + 1) as u8; 32]).unwrap());
...
fn connect_peers(node_a: &MessengerNode, node_b: &MessengerNode) {
    ...
    node_a
        .messenger
        .peer_connected(node_b.node_id, &init_msg.clone(), true)
        .unwrap();
    node_b
        .messenger
        .peer_connected(node_a.node_id, &init_msg.clone(), false)
        .unwrap();
}
...
fn add_channel_to_graph(
    node_a: &MessengerNode,
    node_b: &MessengerNode,
...
fn pass_along_path(path: &Vec<MessengerNode>) {
    ...
    for node in path.iter().skip(1) {
        ...
        node.messenger
            .handle_onion_message(prev_node.node_id, &onion_msg);
        prev_node = node;
    }
}
...
fn one_unblinded_hop() {
    ...
    nodes[0]
        .messenger
        .send_onion_message(test_msg, instructions)
        .unwrap();
    nodes[1]
        .custom_message_handler
        .expect_message(TestCustomMessage::Pong);
...
fn two_unblinded_hops() {
    ...
    nodes[0]
        .messenger
        .send_onion_message_using_path(path, test_msg, None)
        .unwrap();
    nodes[2]
        .custom_message_handler
        .expect_message(TestCustomMessage::Pong);
...
fn two_unblinded_two_blinded() {
    ...
    let secp_ctx = Secp256k1::new();
    let intermediate_nodes = [MessageForwardNode {
...
fn async_response_over_one_blinded_hop() {
    ...
    let response_instruction = nodes[0]
        .custom_message_handler
        .handle_custom_message(message, None, responder);
    ...
    assert_eq!(
        nodes[0]
            .messenger
            .handle_onion_message_response(msg, instructions),
        Ok(SendSuccess::Buffered),
    );

    bob.custom_message_handler
        .expect_message(TestCustomMessage::Pong);
...
fn async_response_with_reply_path_succeeds() {
    ...
    alice
        .custom_message_handler
        .expect_message_and_response(message.clone());
    let response_instruction =
        alice
            .custom_message_handler
            .handle_custom_message(message, None, Some(responder));
    ...
    alice
        .custom_message_handler
        .expect_message(TestCustomMessage::Ping);
    nodes.reverse();
    pass_along_path(&nodes);
}
...
fn test_blinded_path_no_padding_for_compact_path() {
    ...
    let intermediate_nodes = [
        MessageForwardNode {
            node_id: nodes[1].node_id,
            short_channel_id: Some(24),
        },
        MessageForwardNode {
            node_id: nodes[2].node_id,
            short_channel_id: Some(25),
        },
    ];
    // Update the context to create a larger final receive TLVs, ensuring that
    // the hop sizes vary before padding.
...
fn we_are_intro_node() {
    ...
    nodes[0]
        .messenger
        .send_onion_message(test_msg.clone(), instructions)
        .unwrap();
    nodes[2]
        .custom_message_handler
        .expect_message(TestCustomMessage::Pong);
...
fn peer_buffer_full() {
    ...
    for _ in 0..super::messenger::MAX_PER_PEER_BUFFER_SIZE / 1401 + 1 {
        nodes[0]
            .messenger
            .send_onion_message(test_msg.clone(), instructions.clone())
            .unwrap();
    }
    let err = nodes[0]
        .messenger
        .send_onion_message(test_msg, instructions.clone())
        .unwrap_err();
...
fn requests_peer_connection_for_buffered_messages() {
    ...
    nodes[0]
        .messenger
        .send_onion_message(message.clone(), instructions.clone())
        .unwrap();
    assert!(release_events(&nodes[0]).is_empty());
    assert!(nodes[0]
        .messenger
        .next_onion_message_for_peer(nodes[1].node_id)
        .is_some());
    assert!(nodes[0]
        .messenger
        .next_onion_message_for_peer(nodes[1].node_id)
    ...
    assert!(nodes[0]
        .messenger
        .next_onion_message_for_peer(nodes[2].node_id)
        .is_some());
}
...
fn intercept_offline_peer_oms() {
    ...
    let err = nodes[1]
        .messenger
        .forward_onion_message(onion_message.clone(), next_node_id)
        .unwrap_err();
    ...
    nodes[1]
        .messenger
        .forward_onion_message(onion_message, &final_node_vec[0].node_id)
        .unwrap();
    final_node_vec[0]
        .custom_message_handler
        .expect_message(TestCustomMessage::Pong);
...
fn spec_test_vector() {
    ...
    nodes[0]
        .messenger
        .handle_onion_message(sender_dummy_node_id, &sender_to_alice_om);
    let alice_to_bob_om = nodes[0]
        .messenger
        .next_onion_message_for_peer(nodes[1].node_id)
        .unwrap();
    ...
    nodes[1]
        .messenger
        .handle_onion_message(nodes[0].node_id, &alice_to_bob_om);
    let bob_to_carol_om = nodes[1]
        .messenger
        .next_onion_message_for_peer(nodes[2].node_id)
        .unwrap();
    ...
    nodes[2]
        .messenger
        .handle_onion_message(nodes[1].node_id, &bob_to_carol_om);
    let carol_to_dave_om = nodes[2]
        .messenger
        .next_onion_message_for_peer(nodes[3].node_id)
        .unwrap();
    ...
    nodes[3]
        .messenger
        .handle_onion_message(nodes[2].node_id, &carol_to_dave_om);
}
