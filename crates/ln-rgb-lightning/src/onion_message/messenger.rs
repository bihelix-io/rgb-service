    ...
    >(
        ...
        let mut peer_info = peers
            .map(|peer| MessageForwardNode {
                short_channel_id: if compact_paths {
                    peer.short_channel_id
                } else {
                    None
                },
                ..peer
        ...
        peer_info.sort_unstable_by(|(_, a_tor_only, a_channels), (_, b_tor_only, b_channels)| {
            a_tor_only
                .cmp(b_tor_only)
                .then(a_channels.cmp(b_channels).reverse())
        });
        ...
        if !compact_paths {
            debug_assert!(paths
                .iter()
                .all(|path| path.blinded_hops().len() == PADDED_PATH_LENGTH));
        }
    ...
    NL::Target: NodeIdLookUp,
{
    ...
    let (first_node_id, blinding_point) = if let Some(first_node_id) = intermediate_nodes.first() {
        (
            *first_node_id,
            PublicKey::from_secret_key(&secp_ctx, &blinding_secret),
        )
    } else {
        match &destination {
        ...
        Ok((
            ...
            (
                ...
                if !control_tlvs_authenticated {
                    log_trace!(
                        logger,
                        "Received an unauthenticated async payments onion message"
                    );
                    return Err(());
                }
            ...
            (
                ...
                if !control_tlvs_authenticated {
                    log_trace!(
                        logger,
                        "Received an unauthenticated DNS resolver onion message"
                    );
                    return Err(());
                }
    ...
    CMH::Target: CustomOnionMessageHandler,
{
    ...
    fn send_onion_message_internal<T: OnionMessageContents>(
        ...
        let is_forward = matches!(
            instructions,
            MessageSendInstructions::ForwardedMessage { .. }
        );
        ...
        .map_err(|e| {
            log_warn!(
                logger,
                "Failed to create onion message with {:?} {}",
                e,
                log_suffix
            );
            e
    ...
    fn enqueue_intercepted_event(&self, event: Event) {
        ...
        let total_buffered_bytes: usize = pending_intercepted_msgs_events
            .iter()
    ...
    >(
...

        {
            ...
            if !futures.is_empty() {
                ...
                log_trace!(
                    self.logger,
                    "Done handling events async, results: {:?}",
                    res
                );
...
fn outbound_buffer_full(
    peer_node_id: &PublicKey,
    buffer: &HashMap<PublicKey, OnionMessageRecipient>,
) -> bool {
        ...
        for (node_id, recipient) in self.message_recipients.lock().unwrap().iter_mut() {
            if let OnionMessageRecipient::PendingConnection(_, addresses, _) = recipient {
                if let Some(addresses) = addresses.take() {
                    ...
                    log_trace!(
                        self.logger,
                        "Done handling event, ignoring result: {:?}",
                        res
                    );
                }
            }
    ...
    fn peer_connected(
        ...
        if init.features.supports_onion_messages() {
            ...
            if self.intercept_messages_for_offline_peers {
                let mut pending_peer_connected_events =
                    self.pending_peer_connected_events.lock().unwrap();
                pending_peer_connected_events.push(Event::OnionMessagePeerConnected {
            ...
            Ok(PeeledOnion::AsyncPayments(message, context, reply_path)) => {
                ...
                match message {
                    ...
                    AsyncPaymentsMessage::OfferPaths(msg) => {
                        let response_instructions = self
                            .async_payments_handler
                            .handle_offer_paths(msg, context, responder);
                        if let Some((msg, instructions)) = response_instructions {
                            let _ = self.handle_onion_message_response(msg, instructions);
                        }
                    }
                    ...
                    AsyncPaymentsMessage::StaticInvoicePersisted(msg) => {
                        self.async_payments_handler
                            .handle_static_invoice_persisted(msg, context);
                    }
                    ...
                    AsyncPaymentsMessage::ReleaseHeldHtlc(msg) => {
                        self.async_payments_handler
                            .handle_release_held_htlc(msg, context);
                    }
                }
            }
...
>(
    ...
    unblinded_path: Vec<PublicKey>,
    destination: Destination,
    message: T,
    mut reply_path: Option<BlindedMessagePath>,
    ...
    let (mut intro_node_id_blinding_pt, num_blinded_hops) = match &destination {
        ...
        Destination::BlindedPath(path) => {
            ...
            (
                Some((*introduction_node_id, path.blinding_point())),
...
fn construct_onion_message_packet<T: OnionMessageContents>(
    payloads: Vec<(Payload<T>, [u8; 32])>,
    onion_keys: Vec<onion_utils::OnionKeys>,
    prng_seed: [u8; 32],
) -> Result<Packet, ()> {
    // Spec rationale:
