...
impl OMNameResolver {
    ...
    pub fn new_best_block(&self, height: u32, time: u32) {
        self.latest_block_time
            .store(time as usize, Ordering::Release);
    ...
    pub fn expire_pending_resolution(&self, name: &HumanReadableName, payment_id: PaymentId) {
        let dns_name = Name::try_from(format!(
    ...
    pub fn resolve_name<ES: EntropySource + ?Sized>(
        ...
        context
            .nonce
            .copy_from_slice(&entropy_source.get_secure_random_bytes()[..16]);
    ...
    pub fn handle_dnssec_proof_for_uri(
        ...
        if let hash_map::Entry::Occupied(entry) = pending_resolves.entry(answer_name) {
            ...
            let validated_rrs = parsed_rrs
                .as_ref()
                .and_then(|rrs| verify_rr_stream(rrs).map_err(|_| &()));
...
mod tests {
    ...
    fn test_expiry() {
        ...
        resolver
            .resolve_name(PaymentId([0; 32]), name.clone(), &keys)
            .unwrap();
        ...
        resolver
            .resolve_name(PaymentId([1; 32]), name.clone(), &keys)
            .unwrap();
        assert_eq!(resolver.pending_resolves.lock().unwrap().len(), 1);
        ...
        resolver
            .resolve_name(PaymentId([2; 32]), name.clone(), &keys)
            .unwrap();
        resolver
            .resolve_name(PaymentId([3; 32]), name.clone(), &keys)
            .unwrap();
        assert_eq!(resolver.pending_resolves.lock().unwrap().len(), 1);
        assert_eq!(
            resolver
