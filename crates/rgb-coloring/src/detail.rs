...
use rgbstd::bitcoin::Transaction as BitcoinTx;
use rgbstd::opret::OpretProof;
use rgbstd::rgbcore::commit_verify::CommitId as _;
use rgbstd::rgbcore::commit_verify::TryCommitVerify;
use rgbstd::rgbcore::commit_verify::mpc::{self, MPC_MINIMAL_DEPTH};
use rgbstd::{
    Txid,
    ...
    for (&contract_id, rgb_assignment) in &rgb_assignments.0 {
        ...
        let prev_outputs = {
            let state: BTreeMap<_, Vec<Amount>> = contract
                .fungible("assetOwner", available_utxos)
                .unwrap()
                .fold(bmap![], |mut set, a| {
...
pub(crate) fn rgb_compose<S: StashProvider, H: StateProvider, P: IndexProvider>(
    ...
    for (contract_id, rgb_assignment) in rgb_assignments {
        let mut main_builder = stock.transition_builder(contract_id, "transfer")?;

        let assignment_id = main_builder.assignment_type("assetOwner");
    ...
    for (id, list) in spent_state {
        let mut blank_builder_opret = stock.transition_builder(id, "transfer")?;
...
impl PartialFascia {
    ...
    pub fn complete_with_tx(self, consensus_serialized_tx: &[u8]) -> Fascia {
        let tx: BitcoinTx = deserialize(consensus_serialized_tx).unwrap();
        let seal_witness = SealWitness::new(PubWitness::Tx(tx), self.merkle_block, self.dbc_proof);
        Fascia::new(seal_witness, self.bundles)
    }
    ...
    pub fn complete_with_txid(self, txid: impl Into<crate::types::Txid>) -> Fascia {
        let txid = txid.into();
        let seal_witness =
            SealWitness::new(PubWitness::Txid(txid), self.merkle_block, self.dbc_proof);
        Fascia::new(seal_witness, self.bundles)
    }
}
...
pub(crate) fn rgb_commit(
    ...
    let merkle_tree = {
        let mpc_messages: BTreeMap<mpc::ProtocolId, mpc::Message> = contract_bundles
            .iter()
            .map(|(cid, bundle)| {
                let protocol_id = mpc::ProtocolId::from(cid.to_byte_array());
                let message = mpc::Message::from(bundle.bundle_id().to_byte_array());
                (protocol_id, message)
            })
            .collect();
