...
use crate::resolvers::{FasciaResolver, LnResolver};
...
fn test_rgb_workflow() {
    ...
    stock.consume_fascia(fascia, FasciaResolver).unwrap();
    ...
    let validation = ValidationConfig {
        chain_net: rgbstd::ChainNet::BitcoinTestnet4,
        trusted_typesystem: consign.types.clone(),
        ..Default::default()
    };
    let validated = consign.validate(&resolver, &validation).unwrap();
    dbg!(validated.validation_status());

    dbg!(rgb_balance(&stock, contract_id, &outputs).unwrap());
...
fn basic_transfer(
    ...
    stock.consume_fascia(fascia.clone(), FasciaResolver).unwrap();
    ...
    let valid_transfer = transfer.validate(&resolver, &validation).unwrap();
    dbg!(valid_transfer.validation_status());
