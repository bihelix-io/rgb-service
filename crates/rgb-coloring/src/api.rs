use std::collections::HashMap;
use std::str::FromStr;

use amplify::confinement::Confined;
use rand::{Rng, SeedableRng};
use rgb_strict_encoding::{StrictDeserialize, StrictSerialize};
use rgbinvoice::{RgbInvoice, RgbInvoiceBuilder};
use rgbstd::containers::{Contract, Transfer, ValidContract};
use rgbstd::containers::ValidTransfer;
use rgbstd::persistence::{IndexProvider, StashProvider, StateProvider, Stock};
use rgbstd::OutputSeal;
use rgbstd::rgbcore::commit_verify::Conceal;

use crate::types::*;

use crate::detail;
use crate::detail::PartialFascia;
use crate::error::Result;


pub fn rgb_issue(
    issuer: &str,
    ticker: &str,
    name: &str,
    details: Option<&str>,
    precision: u8,
    allocations: impl IntoIterator<Item = (String, u64)>,
    chain_net: &str,
) -> ValidContract {
    let chain_net = match chain_net {
        "bitcoin" => rgbstd::ChainNet::BitcoinMainnet,
        "regtest" => rgbstd::ChainNet::BitcoinRegtest,
        "testnet" => rgbstd::ChainNet::BitcoinTestnet3,
        "testnet4" => rgbstd::ChainNet::BitcoinTestnet4,
        "signet" => rgbstd::ChainNet::BitcoinSignet,
        _ => todo!(),

    };
    detail::rgb_issue(issuer, ticker, name, details, precision, allocations, chain_net)
}

pub fn rgb_balance<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    contract_id: ContractId,
    utxos: &[Outpoint],
) -> Result<u64> {
    let utxos = utxos.iter().copied().map(public_outpoint_to_rgb).collect::<Vec<_>>();
    detail::rgb_balance(stock, contract_id.to_raw(), &utxos)
}

pub fn rgb_assignments<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    utxos: &[Outpoint],
) -> HashMap<ContractId, HashMap<Outpoint, u64>> {
    let utxos = utxos.iter().copied().map(public_outpoint_to_rgb).collect::<Vec<_>>();
    detail::rgb_assignments(stock, &utxos)
        .into_iter()
        .map(|(cid, map)| {
            let cid = ContractId::from(cid);
            let map = map
                .into_iter()
                .map(|(outpoint, amount)| (rgb_outpoint_to_public(outpoint), amount))
                .collect();
            (cid, map)
        })
        .collect()
}

pub fn filter_rgb_outpoints<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    utxos: &[Outpoint],
) -> Vec<Outpoint> {
    let utxos = utxos.iter().copied().map(public_outpoint_to_rgb).collect::<Vec<_>>();
    detail::filter_rgb_outpoints(stock, &utxos)
        .into_iter()
        .map(rgb_outpoint_to_public)
        .collect()
}

pub fn rgb_coin_select<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    available_utxos: &[Outpoint],
    rgb_assignments: &RgbAssignments,
) -> Result<Vec<Outpoint>> {
    // let available_utxos: Vec<RawOutpoint> =
    //     available_utxos.iter().copied().map(ToRaw::to_raw).collect();

    let available_utxos = available_utxos
        .iter()
        .copied()
        .map(public_outpoint_to_rgb)
        .collect::<Vec<_>>();

    let coins = detail::rgb_coin_select(stock, &available_utxos, rgb_assignments)?;
    Ok(coins
        .into_iter()
        .map(|coin| {
            let outpoint = coin.to_outpoint();
            rgb_outpoint_to_public(outpoint)
        })
        .collect())
}

pub fn rgb_compose<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    prev_outputs: impl IntoIterator<Item = Outpoint>,
    rgb_assignments: RgbAssignments,
    change_seal: Option<Beneficiary>,
) -> Vec<Transition> {
    let prev_outputs = prev_outputs
        .into_iter()
        .collect::<Vec<_>>();

    let mut rng = {
        // TODO(fy): use a stable hash function, std hasher algorithm is not specified.
        let blinding_seed = {
            use std::hash::{DefaultHasher, Hash, Hasher};

            #[derive(Debug, Hash, Clone)]
            struct ColoringInfo {
                prev_outputs: Vec<Outpoint>,
                rgb_assignments: RgbAssignments,
                change_seal: Option<Beneficiary>,
            }

            let mut hasher = DefaultHasher::new();
            let coloring_info = ColoringInfo {
                prev_outputs: prev_outputs.clone(),
                rgb_assignments: rgb_assignments.clone(),
                change_seal: change_seal.clone(),
            };
            coloring_info.hash(&mut hasher);
            hasher.finish()
        };
        rand_chacha::ChaCha20Rng::seed_from_u64(blinding_seed)
    };

    let prev_outputs = prev_outputs
        .into_iter()
        .map(|o| {
            let o = public_outpoint_to_rgb(o);
            OutputSeal::with(o.txid, o.vout)
        });

    let rgb_assignments = rgb_assignments.to_raw_with_blinding_rng(&mut rng);
    let change_seal = change_seal.map(|s| s.to_raw_with_blinding(rng.gen()));
    
    let transition_list = detail::rgb_compose(
        stock,
        prev_outputs,
        rgb_assignments,
        change_seal,
    )
    .unwrap();

    transition_list
        .into_iter()
        .map(Transition)
        .collect()
}

pub fn rgb_commit(
    // The order of txins must not be changed after `rgb_commit`
    finalized_txins: &[Outpoint],
    transition_list: Vec<Transition>,
) -> ([u8; 32], PartialFascia) {
    // let finalized_txins = finalized_txins
    //     .iter()
    //     .copied()
    //     .map(ToRaw::to_raw)
    //     .collect::<Vec<_>>();

    let finalized_txins = finalized_txins
        .iter()
        .copied()
        .map(public_outpoint_to_rgb)
        .collect::<Vec<_>>();

    let transition_list = transition_list
        .into_iter()
        .map(ToRaw::to_raw)
        .collect();

    let (commitment, partial_fascia) = detail::rgb_commit(&finalized_txins, transition_list);

    (commitment.to_byte_array(), partial_fascia)
}

pub fn rgb_transfer<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    contract_id: ContractId,
    outputs: &[Outpoint],
    secret_seals: &[[u8; 32]],
    witness_tx: Option<Txid>,
) -> Result<Transfer> {
    let outputs = outputs
        .into_iter()
        .cloned()
        .map(public_outpoint_to_rgb)
        .map(|o| rgbstd::OutputSeal::with(o.txid, o.vout))
        .collect::<Vec<_>>();

    let secret_seals = secret_seals
        .iter()
        .cloned()
        .map(|s| SecretSeal::from(s))
        .collect::<Vec<_>>();
    detail::rgb_transfer(stock, contract_id.to_raw(), &outputs, &secret_seals, witness_tx)
}

fn public_outpoint_to_rgb(outpoint: Outpoint) -> rgbstd::Outpoint {
    rgbstd::Outpoint::from_str(&outpoint.to_string()).expect("valid RGB outpoint")
}

fn rgb_outpoint_to_public(outpoint: rgbstd::Outpoint) -> Outpoint {
    Outpoint::from_str(&outpoint.to_string()).expect("valid bp outpoint")
}

pub fn get_empty_stock() -> Stock {
    use schemata::NonInflatableAsset;
    use rgbstd::contract::IssuerWrapper;

    let mut stock = Stock::in_memory();
    stock.import_kit(NonInflatableAsset::kit()).unwrap();

    stock
}

pub fn rgb_export_contract<S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &Stock<S, H, P>,
    contract_id: ContractId,
) -> Contract {
    stock.export_contract(contract_id.to_raw()).unwrap()
}

pub fn rgb_build_invoice<'a, S: StashProvider, H: StateProvider, P: IndexProvider>(
    stock: &mut Stock<S, H, P>,
    contract_id: ContractId,
    amount: u64,
    beneficiary: Beneficiary,
    transports: impl IntoIterator<Item = &'a str>,
    expiry_secs: Option<u32>,
    chain_net: rgbstd::ChainNet,
) -> RgbInvoice {
    use rgbstd::GraphSeal;

    let beneficiary = {
        let b = match beneficiary {
            Beneficiary::WitnessVout(vout) => {
                let seal = GraphSeal::new_random_vout(vout);
                stock.store_secret_seal(seal).unwrap();
                // TODO: check
                rgbinvoice::Beneficiary::BlindedSeal(seal.conceal())
            }
            Beneficiary::Outpoint(outpoint) => {
                let outpoint = public_outpoint_to_rgb(outpoint);
                let seal = GraphSeal::new_random(outpoint.txid, outpoint.vout);
                stock.store_secret_seal(seal).unwrap();
                rgbinvoice::Beneficiary::BlindedSeal(seal.conceal())
            }
            Beneficiary::SecretSeal(secret_seal) => {
                // TODO: check
                rgbinvoice::Beneficiary::BlindedSeal(SecretSeal::from(secret_seal))
            }
        };
        rgbinvoice::XChainNet::with(chain_net, b)
    };

    let expiry = {
        let timestamp = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        (timestamp + expiry_secs.unwrap_or(600) as u64) as i64
    };
    RgbInvoiceBuilder::with(contract_id.to_raw(), beneficiary)
        .set_expiry_timestamp(expiry)
        .set_amount_raw(amount)
        .add_transports(transports)
        .unwrap()
        .finish()
}

pub fn to_rgb_chain_net(bitcoin_network: &str) -> rgbstd::ChainNet {
    match bitcoin_network {
        "bitcoin" => rgbstd::ChainNet::BitcoinMainnet,
        "regtest" => rgbstd::ChainNet::BitcoinRegtest,
        "testnet" => rgbstd::ChainNet::BitcoinTestnet3,
        "testnet4" => rgbstd::ChainNet::BitcoinTestnet4,
        "signet" => rgbstd::ChainNet::BitcoinSignet,
        _ => todo!(),
    }
}

pub fn consensus_serialize_btc_tx(tx: &bitcoin::Transaction) -> Vec<u8> {
    use bitcoin::consensus::Encodable;

    let mut buf = Vec::new();
    tx.consensus_encode(&mut buf).expect("should encode");
    buf
}

pub fn create_stock_from_funding_rgb_transfer(funding_rgb_transfer: crate::ValidTransfer) -> Stock {
    let mut stock = get_empty_stock();
    let mut resolver = crate::LocalResolver::new();
    resolver.add_terminals(&funding_rgb_transfer);

    stock.accept_transfer(funding_rgb_transfer, &resolver).unwrap();
    stock
}

pub fn transfer_to_bytes(transfer: &Transfer) -> Vec<u8> {
    transfer
        .to_strict_serialized::<{ u32::MAX as usize }>()
        .expect("transfer strict serialization")
        .to_unconfined()
}

pub fn transfer_from_bytes(serialized: Vec<u8>) -> Result<Transfer> {
    Transfer::from_strict_serialized(Confined::<Vec<u8>, 0, { u32::MAX as usize }>::from_checked(
        serialized,
    ))
    .map_err(|e| crate::error::Error::transfer(e.to_string()))
}

pub fn valid_transfer_to_bytes(transfer: &ValidTransfer) -> Vec<u8> {
    transfer
        .to_strict_serialized::<{ u32::MAX as usize }>()
        .expect("valid transfer strict serialization")
        .to_unconfined()
}

pub fn stock_to_parts(stock: &Stock) -> (Vec<u8>, Vec<u8>, Vec<u8>) {
    let stash = stock
        .as_stash_provider()
        .to_strict_serialized::<{ u32::MAX as usize }>()
        .expect("stash strict serialization")
        .to_unconfined();
    let state = stock
        .as_state_provider()
        .to_strict_serialized::<{ u32::MAX as usize }>()
        .expect("state strict serialization")
        .to_unconfined();
    let index = stock
        .as_index_provider()
        .to_strict_serialized::<{ u32::MAX as usize }>()
        .expect("index strict serialization")
        .to_unconfined();
    (stash, state, index)
}

pub fn stock_from_parts(
    stash_serialized: Vec<u8>,
    state_serialized: Vec<u8>,
    index_serialized: Vec<u8>,
) -> Result<Stock> {
    let stash = rgbstd::persistence::MemStash::from_strict_serialized(
        Confined::<Vec<u8>, 0, { u32::MAX as usize }>::from_checked(stash_serialized),
    )
    .map_err(|e| crate::error::Error::transfer(e.to_string()))?;
    let state = rgbstd::persistence::MemState::from_strict_serialized(
        Confined::<Vec<u8>, 0, { u32::MAX as usize }>::from_checked(state_serialized),
    )
    .map_err(|e| crate::error::Error::transfer(e.to_string()))?;
    let index = rgbstd::persistence::MemIndex::from_strict_serialized(
        Confined::<Vec<u8>, 0, { u32::MAX as usize }>::from_checked(index_serialized),
    )
    .map_err(|e| crate::error::Error::transfer(e.to_string()))?;

    Ok(Stock::with(stash, state, index))
}


// TODO: use a more effecient implementation to convert between bp and bitcoin types
pub mod bp2btc {
    pub fn outpoint_from_bp(bp_outpoint: &bp::Outpoint) -> bitcoin::OutPoint {
        bp_outpoint.to_string().parse().expect("should be valid bitcoin outpoint")
    }
}

pub mod btc2bp {
    pub fn outpoint_to_bp(btc_outpoint: &bitcoin::OutPoint) -> bp::Outpoint {
        btc_outpoint.to_string().parse().expect("should be valid bp outpoint")
    }

    pub fn tx_to_bp(tx: &bitcoin::Transaction) -> bp::Tx {
        use bitcoin::consensus::Encodable;
        use bp::ConsensusDecode;

        let mut buf = vec![];
        tx.consensus_encode(&mut buf).expect("should be serialized");
        bp::Tx::consensus_deserialize(&buf).expect("should be deserialized")
    }

    pub fn txid_to_bp(btc_txid: &bitcoin::Txid) -> bp::Txid {
        btc_txid.to_string().parse().expect("should be valid bp txid")
    }
}
