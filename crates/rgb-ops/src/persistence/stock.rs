...
impl<S: StashProvider, H: StateProvider, P: IndexProvider> CloneNoPersistence for Stock<S, H, P> {
    fn clone_no_persistence(&self) -> Self {
        Self {
            ...
            store_lock: Arc::clone(&self.store_lock),
        }
    }
}
...
impl<S: StashProvider, H: StateProvider, I: IndexProvider> Stock<S, H, I> {
    ...
    where P: Clone
            ...
            + 'static {
        let _ = autosave;
        let stash = S::load(provider.clone(), false)?;
        let state = H::load(provider.clone(), false)?;
        let index = I::load(provider, false)?;
            ...
            + 'static,
    {
        ...
        let a = self
            ...
            .make_persistent(provider.clone(), false)?;
        let b = self
    ...
    pub fn store(&mut self) -> Result<(), PersistenceError> {
        ...
        let _guard = lock.write().map_err(|err| {
            PersistenceError::with(io::Error::new(
                io::ErrorKind::Other,
                format!("RGB stock store lock poisoned: {err}"),
            ))
        })?;
